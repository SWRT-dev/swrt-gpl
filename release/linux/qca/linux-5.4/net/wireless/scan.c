...
static size_t cfg80211_gen_new_ie(const u8 *ie, size_t ielen,
				  ...
				  u8 *new_ie, size_t new_ie_len, u8 bssid_index)
{
	...
	for_each_element(parent, ie, ielen) {
		...
		if (!sub) {
			if (id == WLAN_EID_REDUCED_NEIGHBOR_REPORT) {
				pos +=
				  cfg80211_handle_rnr_ie_for_mbssid(parent,
								    bssid_index,
								    pos);
			} else if (cfg80211_is_element_inherited(parent,
								 non_inherit_elem)) {
	...
	for_each_element_id(elem, WLAN_EID_MULTIPLE_BSSID, ie, ielen) {
		...
		for_each_element(sub, elem->data + 1, elem->datalen - 1) {
			...
			new_ie_len = cfg80211_gen_new_ie(ie, ielen,
							 profile,
							 profile_len, new_ie,
							 IEEE80211_MAX_DATA_LEN, bssid_index);
			if (!new_ie_len)
				continue;
