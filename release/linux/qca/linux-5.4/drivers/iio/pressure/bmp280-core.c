 ...
 */
static int bmp390_read_calib(struct bmp280_data *data, struct bmp390_calib *calib)
{
	int ret;
			...
			int irq)
{
	...
	case BMP390_CHIP_ID:
		indio_dev->num_channels = 2;
		data->chip_info = &bmp390_chip_info;
		data->oversampling_press = ilog2(16);
		data->oversampling_temp = ilog2(2);
		data->start_up_time = 2000;
		break;
	...
	if (chip_id  == BMP180_CHIP_ID) {
		...
		ret = bmp390_read_calib(data, &data->calib.bmp390);
		if (ret < 0) {
			dev_err(data->dev,
				"failed to read calibration coefficients\n");
			goto out_disable_vdda;
