...
static bool match(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	htinfo_pt = kmalloc(sizeof(httpinfo_t), GFP_KERNEL);
	if (htinfo_pt == NULL)
		return 0;

	/* Get the http header info */
	if (get_http_info(skb, flags, htinfo_pt) < 1) {
		kfree (htinfo_pt);
		return 0;
	}
	...
	if (info->type == IPT_WEBSTR_HOST || info->type == IPT_WEBSTR_URL) {
		...
		haystack = htinfo_pt->host;
		hlen = htinfo_pt->hostlen;
	    }
	    else {
		haystack = htinfo_pt->url;
		hlen = htinfo_pt->urllen;
	    }
	    ...
	    if (opt & BLK_JAVA) {
		vicelen = sizeof(".js") - 1;
		if (strncasecmp(htinfo_pt->url + htinfo_pt->urllen - vicelen, ".js", vicelen) == 0) {
		    SPARQ_LOG("%s: MATCH....java\n", __FUNCTION__);
		    found = 1;
		    goto match_ret;
		}
		vicelen = sizeof(".class") - 1;
		if (strncasecmp(htinfo_pt->url + htinfo_pt->urllen - vicelen, ".class", vicelen) == 0) {
		    SPARQ_LOG("%s: MATCH....java\n", __FUNCTION__);
		    found = 1;
		    goto match_ret;
		}
		...
		if (strncasecmp(htinfo_pt->url + htinfo_pt->urllen - vicelen, ".ocx", vicelen) == 0) {
		    SPARQ_LOG("%s: MATCH....activex\n", __FUNCTION__);
		    found = 1;
		    goto match_ret;
		}
		vicelen = sizeof(".cab") - 1;
		if (strncasecmp(htinfo_pt->url + htinfo_pt->urllen - vicelen, ".cab", vicelen) == 0) {
		    SPARQ_LOG("%s: MATCH....activex\n", __FUNCTION__);
		    found = 1;
		    goto match_ret;
		}
	    ...
	    if (opt & BLK_PROXY){
		if (strncasecmp(htinfo_pt->url + htinfo_pt->hostlen, "http://", sizeof("http://") - 1) == 0) {
...
static bool match6(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	/* Get the http header info */
	if (get_http_info6(skb, flags, htinfo_pt) < 1) {
		kfree (htinfo_pt);
		return 0;
	}
