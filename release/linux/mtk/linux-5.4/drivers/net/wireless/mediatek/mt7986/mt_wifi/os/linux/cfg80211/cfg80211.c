	...
	IN struct cfg80211_connect_params	*pSme)
{
	...
	if (pSme->bssid != NULL) {
		CFG80211DBG(DBG_LVL_TRACE, ("80211> Connect bssid "MACSTR"\n",
								  MAC2STR(pSme->bssid)));
...
#endif
{
	...
	if (dev) {
		...
		CFG80211DBG(DBG_LVL_TRACE, ("80211> %s ==> for bssid ("MACSTR")\n",
		...
		IN struct cfg80211_qos_map *qos_map)
{
	...
	if (qos_map == NULL) {
		MTWF_DBG(pAd, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Qos Map NULL\n");
		return 0;
	}
	...
	if (apidx != WDEV_NOT_FOUND) {
		...
		MTWF_DBG(pAd, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "AP Index for setting qos map not found\n");
