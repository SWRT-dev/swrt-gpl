...
VOID RTMP_CFG80211_AddVifEntry(VOID *pAdSrc, PNET_DEV pNewNetDev, UINT32 DevType)
{
	...
	if (pNewVifDev) {
		...
		MTWF_DBG(pAd, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	...
	if (dev_p) {
		...
		if (isGoOn) {
			...
			if (pAd->Mlme.bStartMcc == TRUE) {
				MTWF_DBG(pAd, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Group remove stop mcc\n");
				Stop_MCC(pAd, 0);
			...
			if (pAd->flg_apcli_init) {
				...
				if (pAd->Mlme.bStartMcc == TRUE) {
					...
					MTWF_DBG(pAd, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "GC remove stop mcc\n");
...
VOID RTMP_CFG80211_VirtualIF_CancelP2pClient(VOID *pAdSrc)
{
	...
	while (pDevEntry != NULL) {
		if (pDevEntry->devType == RT_CMD_80211_IFTYPE_P2P_CLIENT) {
			MTWF_DBG(pAd, DBG_CAT_P2P, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "==> RTMP_CFG80211_VirtualIF_CancelP2pClient HIT.\n");
...
{
	MTWF_DBG(pAd, DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "\n");
