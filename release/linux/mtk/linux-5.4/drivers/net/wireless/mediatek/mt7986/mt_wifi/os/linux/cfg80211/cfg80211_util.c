...
VOID CFG80211OS_UnRegister(VOID *pCB, VOID *pNetDevOrg)
{
	...
	if (pCfg80211_CB->pCfg80211_Wdev != NULL) {
		MTWF_DBG(NULL, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "80211> unregister/free wireless device\n");
		/*
	...
	IN CFG80211_BAND * pBandInfo)
{
	...
	if (pWiphy != NULL) {
		MTWF_DBG(NULL, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "80211> re-init bands...\n");
		/* re-init bands */
...
#endif
		MTWF_DBG(NULL, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "80211> cfg80211_scan_done\n");
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	...
	IN UCHAR					FlgIsSuccess)
{
	...
	if (FlgIsSuccess) {
		MTWF_DBG(NULL, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "APCLI: ReqIeLen %d, RspIeLen, %d\n", ReqIeLen, RspIeLen);
