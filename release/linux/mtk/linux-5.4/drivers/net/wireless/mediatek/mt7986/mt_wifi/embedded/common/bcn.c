	...
	BCN_UPDATE_REASON reason)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_BCN, DBG_LVL_TRACE,
	...
	if (reason == BCN_UPDATE_INIT) {
		...
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_BCN, DBG_LVL_DEBUG, "BCN_UPDATE_INIT, OmacIdx = %x (%s)\n",
		...
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_BCN, DBG_LVL_DEBUG, "BCN_UPDATE_ENABLE_TX, OmacIdx = %x (%s)\n",
				 wdev->OmacIdx, wdev->if_dev->name);
...
#ifdef CONVERTER_MODE_SWITCH_SUPPORT
	{
			...
			if (pMbss->APStartPseduState != AP_STATE_ALWAYS_START_AP_DEFAULT) {
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_BCN, DBG_LVL_TRACE, "StopBeaconing:\n");
				return;
			}
	}
