...
)
{
	...
	if ((pentry->repting_bmap ^ self_bmap_backup) || bforce)	/* self bitmap changed */
...
)
{
	...
	MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO, "%s: NetDev[%d], Cmd(%d), Len(%d)\n",
			 (pNetDev != NULL) ? RtmpOsGetNetIfIndex(pNetDev) : -1,
	...
	UINT8 *f_buf)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO, "%s, wdev(%d) f_buf = 0x%p\n",
			 wdev->wdev_idx, f_buf);

	frame_len = build_fils_discovery_action(wdev, f_buf);

	MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO, "%s, Build FILS_DISCOVERY, Len = %ld\n",
			 frame_len);
		...
		struct wifi_dev *wdev, UINT8 *f_buf)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO, "%s, Build QOS_NULL, Len = %ld\n",
			 frame_len);
	...
	struct wifi_dev *wdev)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO, "%s, Type(%d), Len(%ld), mode:%d, mcs:%d\n",
