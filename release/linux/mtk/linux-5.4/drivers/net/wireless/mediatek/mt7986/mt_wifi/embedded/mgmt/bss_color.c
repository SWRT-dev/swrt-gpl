...
static BOOLEAN bss_color_acquire(struct wifi_dev *wdev, UINT8 *color)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE, "wdev(idx:%d) acquire BSS color %d\n", wdev->wdev_idx, *color);
	return TRUE;
}
#endif
		...
		struct bss_color_ctrl *bss_color)
{
	...
	MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE,
...
void set_bss_color_info(struct wifi_dev *wdev, BOOLEAN disabled, UINT8 color)
{
	...
	if ((bss_color->collision_detected == TRUE) && (bss_color->disabled != disabled)) {
		...
		MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE, "collision is alleviated\n");
	}
...
static void bss_color_trigger_collision(struct wifi_dev *wdev, UINT8 OP_MODE)
{
		...
		for (i = 0; i < WDEV_NUM_MAX; i++) {
			...
			if (curr_wdev && (band_idx == HcGetBandByWdev(curr_wdev))
				...
				if (bss_color->collision_detected == FALSE) {
					...
					MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE, "collision is detected\n");
					if (!bss_color->u.ap_ctrl.trigger_timer_running) {
						...
						MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE, "timer is triggered\n");
					}
				}
	...
	if (OP_MODE == BSS_COLOR_OPMODE_SINGLE) {
		...
		if (wdev->wdev_type == WDEV_TYPE_AP) {
			...
			if (bss_color->collision_detected == FALSE) {
				bss_color->collision_detected = TRUE;
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE,
		...
		IN PVOID SystemSpecific3)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE, "enter\n");
	band_idx = HcGetBandByWdev(wdev);
	...
	if (bss_color->collision_detected) {
		...
		if (IS_BSS_COLOR_MANUAL_ACTIVE(ad, band_idx)) {
			...
			if (!ad->ApCfg.bss_color_cfg.bss_color_next[band_idx]) {
				...
				MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE, "Got new color after collision:%d\n", next_color);
				...
				MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE, "Update color to MBSS(%d):%d\n",
		...
		IN PVOID SystemSpecific3)
{
	...
	MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_COLOR, DBG_LVL_TRACE, "enter\n");
