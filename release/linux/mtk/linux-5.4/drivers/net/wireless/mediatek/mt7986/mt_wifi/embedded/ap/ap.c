...
VOID MacTableMaintenance(RTMP_ADAPTER *pAd)
{
...
#ifdef IGMP_SNOOPING_NON_OFFLOAD
		UCHAR band_id = HcGetBandByWdev(pEntry->wdev);
	...
	call_traffic_notifieriers(TRAFFIC_NOTIFY_TPUT_DETECT, pAd, pEntry);
		...
		*/
		...
		if (pEntry->NoDataIdleCount > (pEntry->StaIdleTimeout * 1024) / 1000) {
...
#endif
			{
				...
				*/
				...
				|| (pEntry->per_err_times >= CONTD_PER_ERR_CNT_MC
			...
			*/
...
#endif
			{
				...
				MTWF_DBG(pAd, DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_WARN,
						...
						pEntry->per_err_times, tr_entry->token_cnt, atomic_read(&que->free_token_cnt));
#endif
				MTWF_DBG(pAd, DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_WARN,
		...
		if ((pEntry && pEntry->agg_err_flag) || pAd->aggManualEn) {
			MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"STA(%d) hit agg err, need adjust agg num!\n", pEntry->wcid);
			/*send cmd to fw, update sta per(pEntry->tx_per);
			* then according sta per, set wtbl(BA_WIN_SIZE) to per sta limit agg.
			*/
			RTMP_SET_UPDATE_PER(pAd, wcid);
		}
#ifdef PS_STA_FLUSH_SUPPORT
...
VOID ApIdleStateCheck(RTMP_ADAPTER *pAd)
{
	...
	for (band_idx = 0; band_idx < band_num; band_idx++) {
		...
		if (!chip_dbg->get_data_traffic_cnt(pAd, band_idx, &tx_cnt, &rx_cnt)) {
			MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"fail to get data traffic cnt for band %d\n", band_idx);
			continue;
		}
		...
		if (tx_cnt <= AP_IDLE_TRAFFIC_THRESHOLD && rx_cnt <= AP_IDLE_TRAFFIC_THRESHOLD) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"data traffic not exist, tx_cnt=%d, rx_cnt=%d, increase idle counter = %d for band %d\n",
					tx_cnt, rx_cnt, pAd->ApIdleCount[band_idx], band_idx);
		} else {
			/* data traffic exist, reset idle counter */
			pAd->ApIdleCount[band_idx] = 0;
			MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"data traffic exist, tx_cnt=%d, rx_cnt=%d, reset idle counter to 0 for band %d\n",
					tx_cnt, rx_cnt, band_idx);
		}
		...
		MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"idle timeout happens for band %d\n", band_idx);
		sta_connected = FALSE;
		for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
		...
		if (!wdev) {
			MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					"invalid wifi device for band %d\n", band_idx);
			continue;
		}
...
VOID APOverlappingBSSScan(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	for (index = chStartIdx; index <= chEndIdx; index++) {
        ...
        if (pAd->bIsLowPower == TRUE) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO, "WaitBeaconTime=%d!\n",
				pAd->WaitBeaconTime);
