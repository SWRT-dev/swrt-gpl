	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	Ret = snprintf(TMR_Value, sizeof(TMR_Value), "%d", value);
	if (os_snprintf_error(sizeof(TMR_Value), Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"TMR_Value snprintf error!\n");
		return FALSE;
	}

	Ret = snprintf(TMR_HW_Version, sizeof(TMR_HW_Version), "%d", version);
	if (os_snprintf_error(sizeof(TMR_HW_Version), Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
...
static INT32 HQA_TxBfProfileTagInValid(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%u", invalid);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
		return FALSE;
...
static INT32 HQA_TxBfProfileTagPfmuIdx(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%u", pfmuidx);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagBfType(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%u", bftype);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagBw(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%u", tag_bw);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagSuMu(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%u", su_mu);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagMemAlloc(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
...
static INT32 HQA_TxBfProfileTagMatrix(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (len < 24) {
		...
		memset(cmd, 0x00, HQA_BF_STR_SIZE);
		Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x:%02x:%02x", matrix->nrow, matrix->ncol, matrix->ngroup, matrix->LM, matrix->code_book, matrix->htc_exist);
		if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"cmd snprintf error!\n");
			os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagSnr(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (len < 16) {
		...
		memset(cmd, 0x00, HQA_BF_STR_SIZE);
		Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x", snr->snr_sts0, snr->snr_sts1, snr->snr_sts2, snr->snr_sts3);
		if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"cmd snprintf error!\n");
			os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagSmtAnt(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", smt_ant);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagSeIdx(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", se_idx);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagRmsdThrd(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", rmsd_thrd);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagMcsThrd(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (len < 24) {
		...
		memset(cmd, 0x00, HQA_BF_STR_SIZE);
		Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x:%02x:%02x", mcs_thrd->mcs_lss0, mcs_thrd->mcs_sss0, mcs_thrd->mcs_lss1, mcs_thrd->mcs_sss1, mcs_thrd->mcs_lss2, mcs_thrd->mcs_sss2);
		if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"cmd snprintf error!\n");
			os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagTimeOut(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", bf_tout);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagDesiredBw(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", desire_bw);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagDesiredNc(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", desire_nc);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagDesiredNr(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", desire_nr);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagWrite(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", idx);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfProfileTagRead(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (idx < 256 && isBFer < 256) {
		Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x", idx, isBFer);
		if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"cmd snprintf error!\n");
			os_free_mem(cmd);
...
static INT32 HQA_StaRecCmmUpdate(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x", rec->wlan_idx, rec->bss_idx, rec->aid, PRINT_MAC(rec->mac));
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_StaRecBfUpdate(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
...
static INT32 HQA_BFProfileDataRead(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (subcarr_end <= 65535) {
		for (subcarrIdx = subcarr_start; subcarrIdx <= subcarr_end; subcarrIdx++) {
			...
			Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x", idx, fgBFer, SubIdx[1], SubIdx[0]);
			if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
				MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					"cmd snprintf error!\n");
				os_free_mem(cmd);
...
static INT32 HQA_BFProfileDataWrite(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:", profile->pfmuid);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%03x:", profile->subcarrier);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
		return FALSE;
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%03x:", profile->phi11);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x:", profile->psi21);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%03x:", profile->phi21);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x:", profile->psi31);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%03x:", profile->phi31);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x:", profile->psi41);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%03x:", profile->phi22);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x:", profile->psi32);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%03x:", profile->phi32);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x:", profile->psi42);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%03x:", profile->phi33);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x:", profile->psi43);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), HQA_BF_STR_SIZE - strlen(cmd), "%02x:", profile->snr00);
	if (os_snprintf_error(HQA_BF_STR_SIZE, (strlen(cmd) + Ret))) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
		return FALSE;
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x:", profile->snr01);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x:", profile->snr02);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
	...
	cmd_left = HQA_BF_STR_SIZE - strlen(cmd);
	Ret = snprintf(cmd + strlen(cmd), cmd_left, "%02x", profile->snr03);
	if (os_snprintf_error(cmd_left, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_BFQdRead(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", subcarrier_idx);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_BFSounding(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x:%02x:%02x:%02x", param->su_mu, param->mu_num, param->snd_interval, param->wlan_id0, param->wlan_id1, param->wlan_id2, param->wlan_id3);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TXBFProfileDataWriteAllExt(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%x:%x", profile_idx, bw);
	if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
...
static INT32 HQA_TxBfTxApply(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (wlan_id < DBDC_BAND_NUM) {
		...
		Ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x:%02x", wlan_id, eBF_enable, iBF_enable, MuTx_enable, iBFPhaseCali);
		if (os_snprintf_error(HQA_BF_STR_SIZE, Ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"cmd snprintf error!\n");
			os_free_mem(cmd);
		...
		if (ATEOp->SetICapStart) {
			{
				...
				MTWF_DBG(pAd, DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO, "MACTriggerEvent:%d\n", MACTriggerEvent);
...
static INT32 hqa_iBFSetValue_ext(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *wrq, struct _HQA_CMD_FRAME *cmd_frame)
{
	...
	case ATE_TXBF_INIT:
		memset(cmd, 0x00, HQA_BF_STR_SIZE);
		ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d", (UCHAR)u4InArg[0]);
		if (os_snprintf_error(HQA_BF_STR_SIZE, ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"cmd snprintf error!\n");
			os_free_mem(cmd);
	...
	case ATE_CHANNEL:
		...
		if (u4InArg[1] == 1) {
			ret = snprintf(cmd, HQA_BF_STR_SIZE, "%d:1", (UCHAR)u4InArg[0]);
			if (os_snprintf_error(HQA_BF_STR_SIZE, ret)) {
				MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					"cmd snprintf error!\n");
				os_free_mem(cmd);
	...
	case ATE_RX_FRAME:
		SetATE(pAd, "TXSTOP");
		memset(cmd, 0x00, HQA_BF_STR_SIZE);
		ret = snprintf(cmd, HQA_BF_STR_SIZE, "%s", "RXFRAME");
		if (os_snprintf_error(HQA_BF_STR_SIZE, ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"cmd snprintf error!\n");
			os_free_mem(cmd);
	...
	case ATE_IBF_PHASE_COMP:
		...
		ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x:%02x:%02x",
				u4InArg[0], u4InArg[1],
				u4InArg[2], u4InArg[3],
				u4InArg[4]);
		if (os_snprintf_error(HQA_BF_STR_SIZE, ret)) {
	...
	case ATE_IBF_TX:
		...
		memset(cmd, 0x00, HQA_BF_STR_SIZE);
		/* fgBf:WLAN idx:Txcnt */
		ret = snprintf(cmd, HQA_BF_STR_SIZE, "%02x:%02x:%02x",
				u4InArg[0], u4InArg[1],
				u4InArg[2]);
		if (os_snprintf_error(HQA_BF_STR_SIZE, ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	case ATE_IBF_PROF_UPDATE:
		...
		ret = snprintf(cmd, HQA_BF_STR_SIZE, "01:00:01:00:01");
		if (os_snprintf_error(HQA_BF_STR_SIZE, ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"cmd snprintf error!\n");
			os_free_mem(cmd);
...
static INT32 hqa_iBFChanProfUpdate_ext(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *wrq, struct _HQA_CMD_FRAME *cmd_frame)
{
	...
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	ret = snprintf(cmd, HQA_BF_STR_SIZE, "%03x:%03x:%03x:%03x:%03x:%03x:%03x:%03x:%03x:%03x:%03x",
...
static INT32 hqa_iBFProfileRead_ext(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *wrq, struct _HQA_CMD_FRAME *cmd_frame)
{
	...
	EthGetParamAndShiftBuff(TRUE, sizeof(UINT32), &data, (UCHAR *)&u4Subcarr);
	memset(cmd, 0x00, HQA_BF_STR_SIZE);
	ret = snprintf(cmd, HQA_BF_STR_SIZE, "%03x:%03x", (UCHAR)u4PfmuId, (UCHAR)u4Subcarr);
	if (os_snprintf_error(HQA_BF_STR_SIZE, ret)) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"cmd snprintf error!\n");
		os_free_mem(cmd);
