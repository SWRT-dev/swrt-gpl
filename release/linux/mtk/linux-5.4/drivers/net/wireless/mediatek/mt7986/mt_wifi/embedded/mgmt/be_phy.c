...
VOID phy_oper_init(struct wifi_dev *wdev, struct phy_op *obj)
{
	...
	if (ad->CommonCfg.dbdc_mode) {
		...
		MTWF_DBG(ad, DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Swap TX/RX Stream number to (%d,%d) since DBDC_MODE EN\n",
				 ucTxPath, ucRxPath);
	...
	MTWF_DBG(ad, DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "obj->tx_stream = %d, obj->rx_stream = %d\n",
	...
	if (!ATE_ON(ad)) {
		...
		MTWF_DBG(ad, DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "normal mode - set to new T/RX\n");
	}
#endif /* CONFIG_ATE */
	MTWF_DBG(ad, DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Operate TxStream = %d, RxStream = %d\n",
			  obj->tx_stream, obj->rx_stream);
