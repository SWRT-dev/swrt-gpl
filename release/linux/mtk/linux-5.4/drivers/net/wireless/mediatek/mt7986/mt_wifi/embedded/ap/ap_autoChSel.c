...
#define CCA_THRESHOLD (100)
	...
	for (channel_idx = 0; channel_idx < pChCtrl->ChListNum; channel_idx++) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, "Channel %d : Dirty = %ld, False CCA = %u, Busy Time = %u, Skip Channel = %s\n",
	...
	if (candidate_ch >= 0) {
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, "Rule 1 CCA value : Min Dirtiness (Include extension channel) ==> Select Channel %d\n", ch);
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, "Min Dirty = %u\n", min_dirty);
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, "ExChannel = %d , %d\n", candidate_ExChannel[0], candidate_ExChannel[1]);
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, "BW        = %s\n", (cfg_ht_bw == BW_40) ? "40" : "20");
		return ch;
	}
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, "Rule 3 CCA value : Randomly Select ==> Select Channel %d\n", ch);
	return ch;
}
...
static inline UCHAR SelectClearChannelApCnt(RTMP_ADAPTER *pAd, struct wifi_dev *pwdev)
{
	...
	for (channel_index = 0; channel_index < pChCtrl->ChListNum; channel_index++)
		/* debug messages */
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, "Channel %d : Dirty = %ld, ApCnt=%ld, Busy Time = %d, Skip Channel = %s\n",
	...
	for (dirty = 30; dirty <= 32; dirty++) {
		...
		if (candidate_num) {
			...
			if (final_channel != 0) {
				...
				MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, " Dirtiness = %d ,  Min ApCnt = %d\n", dirty, min_ApCnt);
	...
	IN BOOLEAN IsABand)
{
...
#endif
	{
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO,
				"BandIdx = %d, pre_pcca_time = %u\n", BandIdx, pAutoChCtrl->AutoChSelCtrl.pre_pcca_time);
	...
	IN struct wifi_dev *pwdev)
{
	...
	if (pAutoChCtrl->AutoChSelCtrl.ScanChIdx == -1) {
		...
		if (cap->uni_cmd_support)
			...
			MtCmdMultipleMibRegAccessRead(pAd, BandIdx, Reg, 1);
		pAutoChCtrl->AutoChSelCtrl.pre_pcca_time = Reg[0].Value;
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_ACS, DBG_LVL_INFO, "BandIdx %d, pre_pcca_time %u\n",
			BandIdx, pAutoChCtrl->AutoChSelCtrl.pre_pcca_time);
