	...
	IN int			index)
{
	...
	for (; startIdx <=  endIdx; startIdx++) {
		...
		while (pHead) {
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "IPMac[%d]:\n", startIdx);
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "\t:IP=0x%x,Mac="MACSTR", lastTime=0x%lx, next=%p\n",
					 pHead->ipAddr, MAC2STR(pHead->macAddr), pHead->lastTime, pHead->pNext);
			pHead = pHead->pNext;
		}
	}

	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "\t----EndOfDump!\n");
}
	...
	IN UINT *dstIP)
{
	...
	/*	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "Get the dstIP=0x%x\n", *dstIP); */
	return TRUE;
}
	...
	IN UINT   *pSrcIP)
{
	...
	/*	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "Get the srcIP=0x%x\n", *pSrcIP); */
	return TRUE;
}
	...
	IN UINT				ipAddr)
{
	...
	while (pEntry) {
		...
		if (ipAddr == pEntry->ipAddr) {
			/*	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "Got the Mac("MACSTR") of mapped IP(%d.%d.%d.%d)\n",
	...
	IN UINT			ipAddr)
{
	...
	while (pEntry) {
		if (pEntry->ipAddr == ipAddr) {
			/*			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "dstMac="MACSTR" for mapped dstIP(%d.%d.%d.%d)\n",
	...
	IN PUCHAR			pMacAddr)
{
	...
	*/
	...
	/*
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "ARP Pkt=>senderIP=%d.%d.%d.%d, senderMac="MACSTR"\n",
				...
				if (*(pLayerHdr + 9) == 0x11) {
					...
					if (srcPort == 67 && dstPort == 68) { /*It's a DHCP packet */
						...
						if (pReptEntry)
							...
							MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE,
							"MatchAPCLITabIdx = %u\n",
							pEntry->func_tb_idx);
						bHdrChanged = TRUE;
					}
	...
	IN PUCHAR			pDevMacAdr)
{
	...
	if (*(pLayerHdr + 9) == 0x11) {
		...
		if (srcPort == 68 && dstPort == 67) { /*It's a DHCP packet */
			...
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "is bootp packet! bootpFlag=0x%x\n", bootpFlag);
			dhcpHdr = bootpHdr + 236;
#if defined(DHCP_UC_SUPPORT) && defined(MAC_REPEATER_SUPPORT)
...
#endif
			{
				MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "the DHCP flag is a unicast, dhcp_magic=%02x:%02x:%02x:%02x\n",
						 dhcpHdr[0], dhcpHdr[1], dhcpHdr[2], dhcpHdr[3]);

				if (NdisEqualMemory(dhcpHdr, DHCP_MAGIC, 4)) {
					MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "dhcp magic macthed!\n");
