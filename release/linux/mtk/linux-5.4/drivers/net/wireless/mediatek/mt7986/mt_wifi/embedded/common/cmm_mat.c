...
VOID dumpPkt(PUCHAR pHeader, int len)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "--StartDump\n");

	for (i = 0; i < len; i++) {
		if ((i % 16 == 0) && (i != 0))
			MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "\n");

		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "%02x ", tmp[i] & 0xff);
	}
	...
	IN RTMP_ADAPTER * pAd)
{
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++) {
		...
		if ((pHandle != NULL) && (pHandle->init != NULL)) {
			...
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_TRACE, "MATEngine Init Protocol (0x%04x) success!\n", MATProtoTb[i].protocol);
		}
	}
