...
int main_virtual_if_close(IN struct net_device *net_dev)
{
	...
	MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "===> %s\n",
...
int mt_wifi_open(VOID *dev)
{
	...
	if (load_dev_l1profile(pAd) == NDIS_STATUS_SUCCESS)
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "load l1profile succeed!\n");
	else {
...
int virtual_if_up_handler(VOID *dev)
{
	...
	if (IS_SUPPORT_V10_DFS(pAd)
		...
		MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "pAd->CommonCfg.v10_bw = %d, IS_V10_W56_VHT80_SWITCHED(pAd)=%d\n", pAd->CommonCfg.v10_bw, IS_V10_W56_VHT80_SWITCHED(pAd));
