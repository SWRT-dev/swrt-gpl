	...
	UINT32 u4EnableFeature)
{
	...
	for (i = 0; i < DevInfoTabSize; i++) {
		...
		if (Ret != NDIS_STATUS_SUCCESS)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("%s(): The hanlder of tag (0x%08x) return fail!\n", __func__, UniCmdDevInfoTab[i].u2CmdFeature));
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): Active = %d, OwnMacIdx = %d, band = %d (%02x-%02x-%02x-%02x-%02x-%02x), TLV Num = %d, CmdNeedMaxBufSize = %d, u4RealUseBufSize = %d\n",
		__func__, Active, OwnMacIdx, BandIdx, PRINT_MAC(OwnMacAddr), u2TLVNumber, u4CmdNeedMaxBufSize, u4RealUseBufSize));
...
static INT32 UniCmdBssInfoBasic(struct _RTMP_ADAPTER *pAd, BSS_INFO_ARGUMENT_T *bss_info, VOID *pHandle)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s, ucDbdcIdx = %d, u4ConnectionType = %d, ucActive = %d, u2BcnInterval = %d, ucWmmIdx = %d,ucDtimPeriod = %d, u2BcMcWlanidx = %d, ucPhyMode=%x, BSSID = %02x:%02x:%02x:%02x:%02x:%02x\n",
			  __func__,
...
static INT32 UniCmdBssInfoRLM(struct _RTMP_ADAPTER *pAd, BSS_INFO_ARGUMENT_T *bss_info, VOID *pHandle)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s, ucPrimaryChannel = %d, ucCenterChannelSeg0 = %d, ucCenterChannelSeg1 = %d, ucBandwidth = %d, ucTxStream = %d, ucRxStream = %d\n",
			  __func__,
...
static INT32 UniCmdBssInfoBcnContent(struct _RTMP_ADAPTER *pAd, BSS_INFO_ARGUMENT_T *bss_info, VOID *pHandle)
{
	...
	if ((bcn_buf->bBcnSntReq == TRUE) && (bcn_buf->BeaconPkt)) {
		...
		MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, BSS(%d), BcnLength = %d, IE offset(TIM/CSA/BCC) = %d/%d/%d\n",
				__func__, bss_info->ucBssIndex, le2cpu16(pBssInfoBcnContent->u2PktLength), le2cpu16(pBssInfoBcnContent->u2TimIeOffset),
...
static INT32 UniCmdBssInfoBcnCSA(struct _RTMP_ADAPTER *pAd, BSS_INFO_ARGUMENT_T *bss_info, VOID *pHandle)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,	("%s, BSS(%d), CsaCount = %d\n", __func__,
...
static INT32 UniCmdBssInfoBcnBCC(struct _RTMP_ADAPTER *pAd, BSS_INFO_ARGUMENT_T *bss_info, VOID *pHandle)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, BSS(%d), BccCount = %d\n", __func__,
...
static INT32 UniCmdBssInfoBcnMBSSID(struct _RTMP_ADAPTER *pAd, BSS_INFO_ARGUMENT_T *bss_info, VOID *pHandle)
{
	...
	for (IdBss = MAIN_MBSSID; IdBss < pAd->ApCfg.BssidNum; IdBss++) {
		...
		if (DbdcIdx == bss_info->ucBandIdx) {
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, DbdcIdx(%d), BSS(%d), IE Offset = %d\n",
				__func__, DbdcIdx, IdBss, pMbss->wdev.bcn_buf.TimIELocationInBeacon));
	...
	BSS_INFO_ARGUMENT_T *bss_info)
{
	...
	for (i = 0; i < BssInfoTabSize; i++) {
		...
		default:
			Ret = NDIS_STATUS_SUCCESS;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("%s(): The hanlder of tag (0x%08x) not support!\n", __func__, bss_info->u4BssInfoFeature));
			break;
		}
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s(): OwnMacIdx = %d, Band = %d, BssIndex = %d (%02x-%02x-%02x-%02x-%02x-%02x), TLV Num = %d, CmdNeedMaxBufSize = %d, u4RealUseBufSize = %d\n",
			__func__, bss_info->OwnMacIdx, bss_info->ucBandIdx, bss_info->ucBssIndex,
...
INT32 MtUniCmdPmStateCtrl(struct _RTMP_ADAPTER *pAd, MT_PMSTAT_CTRL_T PmStatCtrl)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::BssidIdx(%d),ucPsProfile(%d)\n",
			  __func__, UniCmdBssInfo.ucBssInfoIdx, UniCmdBssInfoPowerSave.ucPsProfile));
	...
	STA_REC_CFG_T *pStaRecCfg)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s(): TLV Num = %d, CmdNeedMaxBufSize = %d, u4RealUseBufSize = %d\n",
			__func__, u2TLVNumber, u4CmdNeedMaxBufSize, u4RealUseBufSize));
	...
	MT_EDCA_CTRL_T EdcaParam)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): TLV Num = %d, CmdNeedMaxBufSize = %d, u4RealUseBufSize = %d\n",
			__func__, u2TLVNumber, u4CmdNeedMaxBufSize, u4RealUseBufSize));
...
INT32 MtUniCmdSmeshConfigSet(struct _RTMP_ADAPTER *pAd, UCHAR *pdata, P_UNI_CMD_SMESH_PARAM_T prSmeshResult)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s::ucEntryEnable(%d), fgSmeshRxA2(%d), fgSmeshRxA1(%d), fgSmeshRxData(%d), fgSmeshRxMgnt(%d), fgSmeshRxCtrl(%d)\n",
			  __func__, CmdSmeshParam.ucEntryEnable, CmdSmeshParam.fgSmeshRxA2, CmdSmeshParam.fgSmeshRxA1, CmdSmeshParam.fgSmeshRxData,
...
INT32 UniCmdMuarConfigSet(struct _RTMP_ADAPTER *pAd, UCHAR *pdata)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s(): BAND = %d, TLV Num = %d, CmdNeedMaxBufSize = %d, u4RealUseBufSize = %d\n",
			__func__, pUniCmdMuar->ucBand, u2TLVNumber, u4CmdNeedMaxBufSize, u4RealUseBufSize));
...
INT32 UniCmdMib(struct _RTMP_ADAPTER *pAd, UCHAR ChIdx, RTMP_MIB_PAIR *RegPair, UINT32 Num)
{
	...
	pNextHeadBuf += u4ComCmdSize;
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): ucBand=%d\n", __func__, pUniCmdMib->ucBand));
	...
	for (Index = 0; Index < Num; Index++) {
		...
		MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): Tag=%d, Length=%d, Counter=%d\n", __func__,
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, u2StaID=%d, u4CtrlFieldID=%d, u4ComValue=%d, aucAirTimeQuantum=%d-%d-%d-%d-%d-%d-%d-%d\n",
		__func__, le2cpu16(pUniCmdVowDrrCtrl->u2Tag), le2cpu16(pUniCmdVowDrrCtrl->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d\n",
		__func__, le2cpu16(pUniCmdVowFeatureCtrl->u2Tag), le2cpu16(pUniCmdVowFeatureCtrl->u2Length)));
...
INT uni_cmd_vow_set_group(PRTMP_ADAPTER pad, UINT8 group_id, UINT32 subcmd)
{
	...
	case ENUM_BSSGROUP_CTRL_MIN_RATE_TOKEN_CFG_ITEM:
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(SubCmd %x, Value = 0x%x)\n", __func__, subcmd, pVowBssgroupTokenCfg->u4SingleFieldIDValue));
		break;
	...
	case ENUM_BSSGROUP_CTRL_ALL_ITEM_FOR_ALL_GROUP: {
		...
		for (i = 0; i < VOW_MAX_GROUP_NUM; i++) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(min rate token = 0x%x)\n", __func__, pad->vow_bss_cfg[i].min_rate_token));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(max rate token = 0x%x)\n", __func__, pad->vow_bss_cfg[i].max_rate_token));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(min airtime token = 0x%x)\n", __func__, pad->vow_bss_cfg[i].min_airtime_token));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(max airtime token = 0x%x)\n", __func__, pad->vow_bss_cfg[i].max_airtime_token));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(min rate bucket = 0x%x)\n", __func__, pad->vow_bss_cfg[i].min_ratebucket_size));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(max rate bucket = 0x%x)\n", __func__, pad->vow_bss_cfg[i].max_ratebucket_size));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(min airtime bucket = 0x%x)\n", __func__, pad->vow_bss_cfg[i].min_airtimebucket_size));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(max airtime bucket = 0x%x)\n", __func__, pad->vow_bss_cfg[i].max_airtimebucket_size));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(max baclog size = 0x%x)\n", __func__, pad->vow_bss_cfg[i].max_backlog_size));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(max wait time = 0x%x)\n", __func__, pad->vow_bss_cfg[i].max_wait_time));
		}
	}
	break;
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucBssGroupID=%d\n",
		__func__, le2cpu16(pUniCmdVowBss->u2Tag), le2cpu16(pUniCmdVowBss->u2Length), pUniCmdVowBss->ucBssGroupID));
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucBssGroupID=%d, u4SingleFieldIDValue=%d, u4CfgItemId=%d\n",
		__func__, le2cpu16(pUniCmdVowBss->u2Tag), le2cpu16(pUniCmdVowBss->u2Length), pUniCmdVowBss->ucBssGroupID,
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d\n",
		__func__, le2cpu16(pUniCmdVowBss->u2Tag), le2cpu16(pUniCmdVowBss->u2Length)));
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucBssGroupQuantumID=%u, ucBssGroupQuantumTime=%u\n",
		__func__, le2cpu16(pUniCmdVowBss->u2Tag), le2cpu16(pUniCmdVowBss->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, fgAtEstimateOnOff=%d\n",
		__func__, le2cpu16(pUniCmdVowATProc->u2Tag), le2cpu16(pUniCmdVowATProc->u2Length), pUniCmdVowATProc->fgAtEstimateOnOff));
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, u2AtEstMonitorPeriod=%d\n",
		__func__, le2cpu16(pUniCmdVowATProc->u2Tag), le2cpu16(pUniCmdVowATProc->u2Length), le2cpu16(pUniCmdVowATProc->u2AtEstMonitorPeriod)));
...
INT uni_cmd_vow_set_at_estimator_group(PRTMP_ADAPTER pad, UINT32 subcmd, UINT8 group_id)
{
	...
	case ENUM_AT_PROC_EST_GROUP_RATIO_CTRL:
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(group %d, val = 0x%x/0x%x)\n",
				 __func__, group_id,
				 pad->vow_bss_cfg[group_id].max_airtime_ratio,
				 pad->vow_bss_cfg[group_id].min_airtime_ratio));
		break;
	...
	case ENUM_AT_PROC_EST_GROUP_TO_BAND_MAPPING:
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(group %d, val = 0x%x)\n",
				 __func__, group_id, pad->vow_bss_cfg[group_id].band_idx));
		break;
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, u4GroupRatioBitMask=%d\n",
		__func__, le2cpu16(pUniCmdVowATProc->u2Tag), le2cpu16(pUniCmdVowATProc->u2Length), le2cpu32(pUniCmdVowATProc->u4GroupRatioBitMask)));
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucGrouptoSelectBand=%d, ucBandSelectedfromGroup=%d\n",
		__func__, le2cpu16(pUniCmdVowATProc->u2Tag), le2cpu16(pUniCmdVowATProc->u2Length),
...
INT uni_cmd_vow_set_rx_airtime(PRTMP_ADAPTER pad, UINT8 cmd, UINT32 subcmd)
{
	...
	case ENUM_RX_AT_BITWISE_CTRL:
		...
		case ENUM_RX_AT_BITWISE_SUB_TYPE_AIRTIME_CLR: /* clear all RX airtime counters */
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(cmd = 0x%x, subcmd = 0x%x, value = 0x%x)\n",
					 __func__, cmd, subcmd, pVowRxAtAirtimeClrEn->fgRxAirTimeClrEn));
			break;
...
INT uni_cmd_vow_set_wmm_selection(PRTMP_ADAPTER pad, UINT8 om)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(cmd = 0x%x, subcmd = 0x%x, OM = 0x%x, Map = 0x%x)\n",
			 __func__, pVowRxAtStaWmmCtrl->ucOwnMacID,
...
INT uni_cmd_vow_set_mbss2wmm_map(PRTMP_ADAPTER pad, UINT8 bss_idx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(bss_idx = 0x%x, Map = 0x%x)\n",
			 __func__, bss_idx, pad->vow_rx_time_cfg.bssid2wmm_set[bss_idx]));
...
INT uni_cmd_vow_set_backoff_time(PRTMP_ADAPTER pad, UINT8 target)
{
	...
	case ENUM_RX_AT_WMM_GROUP_3:
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(group = 0x%x, BK = 0x%x, BE = 0x%x, VI = 0x%x, VO = 0x%x)\n",
				 __func__, target,
				 pad->vow_rx_time_cfg.wmm_backoff[target][WMM_AC_BK],
				 pad->vow_rx_time_cfg.wmm_backoff[target][WMM_AC_BE],
				 pad->vow_rx_time_cfg.wmm_backoff[target][WMM_AC_VI],
	...
	case ENUM_RX_AT_NON_QOS:
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(group = 0x%x, backoff time = 0x%x)\n",
				 __func__, target, pad->vow_rx_time_cfg.non_qos_backoff));
		break;
	...
	case ENUM_RX_AT_OBSS:
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(group = 0x%x, backoff time = 0x%x)\n",
				 __func__, target, pad->vow_rx_time_cfg.obss_backoff));
		break;
...
INT uni_cmd_vow_get_rx_time_counter(PRTMP_ADAPTER pad, UINT8 target, UINT8 band_idx)
{
	...
	case ENUM_RX_AT_REPORT_SUB_TYPE_RX_NONWIFI_TIME:
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(target = 0x%x, band_idx = 0x%x)\n",
				 __func__, target, band_idx));
		break;
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		...
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, fgRxMibTimeEn=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, fgRxEarlyEndEn=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, fgRxAirTimeClrEn=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucOwnMacID=%d, fgtoApplyWm00to03MibCfg=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucMbssGroup=%d, ucWmmGroup=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucEdOffsetValue=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucCompensateMode=%d, ucRxBand=%d, ucSwCompensateTimeValue=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ACBackoff=%d-%d-%d-%d, ucRxATBackoffWmmGroupIdx=%d, ucRxAtBackoffAcQMask=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucRxNonWiFiBandIdx=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucRxObssBandIdx=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucRxMibObssBandIdx=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, u2StaId=%d\n",
		__func__, le2cpu16(pUniCmdVowRxAt->u2Tag), le2cpu16(pUniCmdVowRxAt->u2Length),
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucRedEnable=%d\n",
		__func__, le2cpu16(pUniCmdVowRed->u2Tag), le2cpu16(pUniCmdVowRed->u2Length),pUniCmdVowRed->ucRedEnable));
	...
	UINT32 *u4RespStructSize)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): u2Tag=%d, u2Length=%d, ucWordlen=%d\n",
		__func__, le2cpu16(pUniCmdVowRed->u2Tag), le2cpu16(pUniCmdVowRed->u2Length),pUniCmdVowRed->ucWordlen));
	...
	VOID *pResult)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): Operation=%s, TLV Num = %d, CmdNeedMaxBufSize = %d, u4RealUseBufSize = %d, u4RespStructSize=%d\n",
		__func__, (isSet || u4RespStructSize == 0) ? "Set" : "Query",
