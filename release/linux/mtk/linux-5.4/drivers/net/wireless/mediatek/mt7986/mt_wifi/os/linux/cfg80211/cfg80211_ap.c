...
#endif
	MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "80211> pBeacon->privacy = %d\n", pBeacon->privacy);
	...
	defined(MT7986) || defined(MT7916) || defined(MT7981)
				if (IS_MT7615(pAd) || IS_MT7626(pAd) || IS_MT7622(pAd) ||
...
#ifdef RT_CFG80211_P2P_MULTI_CHAN_SUPPORT
					{
						...
						MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "InfraCh=%d, pWdev->channel=%d\n", pAd->MlmeAux.InfraChannel, pWdev->channel);
						...
						if (INFRA_ON(pAd) &&
							...
							Start_MCC(pAd);
							MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "infra => GO test\n");
						} else if ((op_ht_bw2 != op_ht_bw1) && ((pAd->StaCfg[0].wdev.channel == pWdev->channel))) {
							MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "start bw !=  && SCC\n");
			...
			MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "PMF IGTK pKeyInfo->KeyId=%d\n", pKeyInfo->KeyId);
#if defined(MT7615) || defined(MT7626) || defined(MT7622) || defined(MT7915) || \
	defined(MT7986) || defined(MT7916) || defined(MT7981)
					if (IS_MT7615(pAd) || IS_MT7626(pAd) || IS_MT7622(pAd) ||
						IS_MT7915(pAd) || IS_MT7986(pAd) || IS_MT7916(pAd) ||
...
)
{
	...
	MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Set Ap Default Key: %d\n", Data);
#ifdef RT_CFG80211_P2P_SUPPORT
...
)
{
	...
	if ((Data == 4) || (Data == 5)) {
		...
		MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Set Ap Default Mgmt KeyId: %d\n", Data);
	} else {
