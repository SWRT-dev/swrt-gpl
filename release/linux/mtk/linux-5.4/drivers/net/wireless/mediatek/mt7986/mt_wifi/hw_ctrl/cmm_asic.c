...
INT AsicSetReptFuncEnable(RTMP_ADAPTER *pAd, BOOLEAN enable, UCHAR band_idx)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		"caller:%pS\n", OS_TRACE);
...
VOID dynamic_agg_adjust_per_sta(struct _RTMP_ADAPTER *ad, struct _MAC_TABLE_ENTRY *entry)
{
	UINT8 per;
	UINT32 ori_idx, winsize, WinSizeThr;
	...
	if (ori_idx == 0 || !pBAEntry){
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
			("%s: STA(%d) doesn't establish BA session, return!\n", __func__, entry->wcid));
		return;
	}
	...
	if (chip_dbg->set_ba_winsize) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
			("%s: STA(%d) - per/dnth/upth = %d/%d/%d, winsize/limit/kpth/WinSizeThr = %d/%d/%d/%d!\n",
			__func__, entry->wcid, per, ad->per_dn_th, ad->per_up_th,
...
VOID LinkTestStaLinkUpHandler(RTMP_ADAPTER *pAd, struct _MAC_TABLE_ENTRY *pEntry)
{
	...
	if (pAd->CommonCfg.LinkTestSupport) {
		...
		MTWF_DBG(pAd, DBG_CAT_CMW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Build Association: Link Up!!\n");
	}
}
...
VOID LinkTestChannelBandUpdate(RTMP_ADAPTER *pAd, UINT8 u1BandIdx, UINT8 u1ControlChannel)
{
	if (pAd->CommonCfg.LinkTestSupport) {
		...
		if (u1ControlChannel <= 14) {
			...
			MTWF_DBG(pAd, DBG_CAT_CMW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, " 2G Channel Band !!\n");
		} else {
			pAd->ucCmwChannelBand[u1BandIdx] = CHANNEL_BAND_5G;
			MTWF_DBG(DBG_CAT_CMW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, " 5G Channel Band !!\n");
		}
