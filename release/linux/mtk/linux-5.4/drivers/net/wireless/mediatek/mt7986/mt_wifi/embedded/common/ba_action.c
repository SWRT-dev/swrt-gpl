...
static VOID ba_free_ori_entry(RTMP_ADAPTER *pAd, ULONG Idx)
{
	...
	if (pBAEntry->ORI_BA_Status != Originator_NONE) {
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "%s: Wcid = %d, TID = %d\n", __func__, pBAEntry->Wcid, pBAEntry->TID);

		if (pBAEntry->ORI_BA_Status == Originator_Done) {
			NdisAcquireSpinLock(&ba_ctl->BATabLock);
			ba_ctl->numDoneOriginator -= 1;
			pEntry->TXBAbitmap &= (~(1 << (pBAEntry->TID)));
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "ba_free_ori_entry numAsOriginator= %ld\n", ba_ctl->numDoneOriginator);
	...
	PVOID SystemSpecific3)
{
	...
	if ((pBAEntry->ORI_BA_Status == Originator_WaitRes) && (pBAEntry->Token < ORI_SESSION_MAX_RETRY)) {
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "BA Ori Session Timeout(%d) : Send ADD BA again\n", pBAEntry->Token);
		pBAEntry->Token++;
		RTMPSetTimer(&pBAEntry->ORIBATimer, ORI_BA_SESSION_TIMEOUT);
	} else {
...
BOOLEAN ba_reordering_resource_init(RTMP_ADAPTER *pAd, int num)
{
	...
	for (i = 0; i < 2; i++) {
		...
		freelist->next = NULL;
		freelist->qlen = 0;
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "Allocate %d memory for BA reordering\n", (UINT32)(num * sizeof(struct reordering_mpdu)));
		...
		UINT16 cfg_ori_wsize, UINT16 peer_rec_wsize)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE,
	...
	UINT16 wcid, UCHAR tid)
{
	...
	pBAEntry = &ba_ctl->BAOriEntry[Idx];
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "%s():Idx=%d, Wcid=%d, TID=%d, ORI_BA_Status=%d\n",
	...
	UCHAR tid, USHORT timeout, UINT16 ba_buffer_size)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "%s(%ld): Idx = %d, BAWinSize = %d\n",
	...
	if (pBAEntry != NULL) {
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "MACEntry[%d]RXBAbitmap = 0x%x. BARecWcidArray=%d\n",
				 pEntry->wcid, pEntry->RXBAbitmap, pEntry->BARecWcidArray[tid]);
	} else {
		Status = FALSE;
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "Can't Accept ADDBA for "MACSTR" TID = %d\n",
...
VOID peer_addba_req_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "%s ==> (Wcid = %d)\n", __func__, Elem->Wcid);
...
VOID peer_addba_rsp_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "\t\t StatusCode = %d\n", pFrame->StatusCode);
	...
	MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "%s ==>\n", __func__);
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "Initiator = %d, Reason = %d\n", pDelFrame->DelbaParm.Initiator, pDelFrame->ReasonCode);
...
BOOLEAN bar_process(RTMP_ADAPTER *pAd, UINT16 Wcid, ULONG MsgLen, PFRAME_BA_REQ pMsg)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "%s(): BAR-Wcid(%d), Tid (%d)\n", __func__, Wcid, TID);
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, "BAR(%d) : Tid (%d) - %04x:%04x\n",
