	...
	IN INT input)
{
	...
	MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "== CCMP test vector ==\n");
	/* Check AAD */
	...
	if (res_len == 22 && NdisEqualMemory(res_buf, AAD, res_len))
		MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Construct AAD is OK!!!\n");
	else {
	...
	if (res_len == 13 && NdisEqualMemory(res_buf, CCM_NONCE, res_len))
		MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Construct NONCE is OK!!!\n");
	else {
	...
	if (NdisEqualMemory(res_buf, CCMP_HDR, 8))
		MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Construct CCMP_HDR is OK!!!\n");
	else {
	...
	if (AES_CCM_Encrypt(res_buf, sizeof(P_TEXT_DATA),
		...
		if (res_len == sizeof(C_TEXT_DATA) &&
			...
			MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "CCM_Encrypt is OK!!!\n");
		else {
	...
	if (AES_CCM_Decrypt(res_buf, sizeof(C_TEXT_DATA), TK, 16,
		...
		if (res_len == sizeof(P_TEXT_DATA) &&
			...
			MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "CCM_Decrypt is OK!!!\n");
		else {
