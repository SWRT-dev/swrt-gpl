...
INT scan_ch_restore(RTMP_ADAPTER *pAd, UCHAR OpMode, struct wifi_dev *pwdev)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_DEBUG, "scan ch restore   ch %d  p2p_wdev->CentralChannel%d\n", ch, p2p_wdev->CentralChannel);
	...
	ch = wlan_operate_get_cen_ch_1(wdev);
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_DEBUG, "End of SCAN(TYPE: %d, BandIdx: %d), restore to BW(%d) channel %d, Total BSS[%02d]\n",
	...
	if (OpMode == OPMODE_STA) {
		...
		*/
		...
		if ((ScanCtrl->Channel != 0) &&
			...
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_DEBUG, "bFastRoamingScan ~~~ Get back to send data ~~~\n");
		}
	}
...
set_stay_time:
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_DEBUG, "SYNC - stay_time:%d\n", stay_time);
	return TRUE;
