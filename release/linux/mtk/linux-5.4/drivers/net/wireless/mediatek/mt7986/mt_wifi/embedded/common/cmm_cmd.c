...
static NTSTATUS ApCliPbcApFoundHandler(IN PRTMP_ADAPTER pAd, IN PCmdQElmt CMDQelmt)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "cmd> channel=%d CMDTHREAD_APCLI_PBC_AP_FOUND!\n", channel);
	/* XXX: Check if channel change is required */
	rtmp_set_channel(pAd, &pApCliTab->wdev, channel);

	/* Bring down ApCli If */
	apcliEn = pApCliTab->ApcliInfStat.Enable;
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "cmd>  CMDTHREAD_APCLI_PBC_AP_FOUND!apcliEn=%d\n", apcliEn);
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	MTWF_DBG(pAd, DBG_CAT_HIF, CATHIF_USB, DBG_LVL_TRACE, "\n pAd=%p\n", pAd);
