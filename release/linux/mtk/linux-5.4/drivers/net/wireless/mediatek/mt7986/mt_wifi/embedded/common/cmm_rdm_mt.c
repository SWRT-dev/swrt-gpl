	...
	IN RTMP_STRING * arg)
{
	...
	PDFS_PARAM pDfsParam = &pAd->CommonCfg.DfsParameter;
	struct DOT11_H *pDot11h = NULL;
	MTWF_PRINT("[%s][RDM]: DFS channel info\n", __func__);
	MTWF_PRINT("=========================================\n ");
...
/*Link down only after sending the radar detect notification*/
		...
		MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Channel=%d, timestamp=%lums, ifname=%s\n",
		...
		if (pDfsParam->RadarDetected[band_idx] && pDfsParam->DfsChBand[band_idx]) {
			...
			if (((pDfsParam->bDedicatedZeroWaitSupport == TRUE) &&
				...
				&& ((IS_ADJ_BW_ZERO_WAIT(pDfsParam->BW160ZeroWaitState) == FALSE))
					...
					tempCh = WrapDfsRandomSelectChannel(pAd, FALSE, 0, DBDC_BAND1);
					MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO,
...
#endif
			{
       			...
       			{
       				pDfsParam->band_ch[band_idx] = WrapDfsRandomSelectChannel(pAd, FALSE, 0, band_idx);
       			}
       			else
#endif
...
#endif
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_TRACE, "[RDM]:ZeroWaitState:%d\n",
