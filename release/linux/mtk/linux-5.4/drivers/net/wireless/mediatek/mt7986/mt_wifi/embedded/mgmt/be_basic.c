...
VOID dump_ht_cap(struct wifi_dev *wdev)
{
	...
	if (wdev) {
		...
		MTWF_DBG(NULL, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "ht_cap: ht_cap->HtCapInfo, \n");
		MTWF_DBG(NULL, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "ldpc=%d,ch_width=%d,gf=%d,sgi20=%d,sgi40=%d,tx_stbc=%d,rx_stbc=%d,amsdu_size=%d\n",
		...
		MTWF_DBG(NULL, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "ht_cap: ht_cap->HtCapParm, \n");
		MTWF_DBG(NULL, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "mdpu_density=%d, ampdu_factor=%d\n",
...
static VOID radio_operate_init(struct wifi_dev *wdev)
{
	...
	if (CheckNonOccupancyChannel(ad, wdev, RDD_CHECK_NOP_BY_WDEV) == FALSE) {
		...
		MTWF_DBG(NULL, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "NOP channel %d grp %d\n", channel, chGrp);
			...
			if (channel && (DfsV10CheckChnlGrp(ad, wdev, channel) == W56_160_UB)) {
				if (ad->CommonCfg.bCh144Enabled == TRUE) {
					...
					MTWF_DBG(NULL, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Set BW to VHT80.\n");
				} else {
					AutoChSelUpdateChannel(ad, channel, TRUE, wdev);
					wlan_config_set_ht_bw(wdev, HT_BW_20);
					wlan_config_set_vht_bw(wdev, VHT_BW_2040);
					MTWF_DBG(NULL, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Set BW to HT20.\n");
				}
			}
			...
			if (!channel && chGrp == W53)
				...
				MTWF_DBG(NULL, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "calling DfsV10W56APDownStart\n");
