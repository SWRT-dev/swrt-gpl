...
#ifdef CFG_SUPPORT_FALCON_MURU
	MTWF_DBG(ad, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_TRACE,
			"%s: val:%d\n", __func__, ppdu_type);
...
VOID ap_set_wireless_mumimo(VOID *ad, VOID *param)
{
	...
	MTWF_DBG(adapt, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_TRACE,
			"cmd:%s\n", str);
...
VOID ap_set_he_fixed_gi_ltf_by_wcid_or_bss(RTMP_ADAPTER *pAd, UINT8 gi, UINT8 mode, UINT32 wcid, struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_TRACE, ("%s:: Enter.\n", __func__));
	if (!pAd)
		return;
	if (gi >= GI_MASK) {
	...
	switch (mode) {
		case GI_BY_WCID:
			if (VALID_UCAST_ENTRY_WCID(pAd, wcid)) {
				...
				MTWF_LOG(DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_TRACE, ("%s:: Invalid wcid value(%d)!!!.\n", __func__, wcid));
			break;
		case GI_BY_BSS:
			if (!wdev) {
...
VOID sta_set_wireless_bcc_ldpc(VOID *ad, VOID *param)
{
	...
	MTWF_DBG(adapt, DBG_CAT_TEST, CATTEST_RFEATURE, DBG_LVL_TRACE, "arg:%s\n", (UCHAR *)param);
