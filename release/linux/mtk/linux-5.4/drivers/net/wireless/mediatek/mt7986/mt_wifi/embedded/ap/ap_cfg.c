	...
	case OID_GET_AIR_MONITOR_RESULT:
		{
			...
			if (wdev) {
				...
				MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "Query::OID_GET_AIR_MONITOR_RESULT success!\n");
				os_free_mem(pEntry);
			} else
...
case OID_GET_SECURITY_TYPE:
	...
	case OID_GET_PASSWORD:
	{
		...
		if (wdev) {
			...
			MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Password %s\n",
				pSecConfig->PSK);
		}
		break;
	}
...
INT Set_StaPowerOffDetect_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			"set STA power off detect with per_err_total = %d, tx_contd_fail_total = %d\n",
			...
			if (subcmd == RT_OID_GET_PHY_MODE) {
				if (pData != NULL) {
					...
					MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "Query::Get phy mode (%02X)\n",
							 pAd->ApCfg.MBSSID[apidx].wdev.PhyMode);
					modetmp = (UINT) pAd->ApCfg.MBSSID[apidx].wdev.PhyMode;
#else
					MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "Query::Get phy mode (%02X)\n",
		...
		if ( subcmd == ASUS_SUBCMD_CHLIST) {
			...
			if (pObj->ioctl_if_type == INT_APCLI && wdev && ((GetAssociatedAPByWdev(pAd, wdev)) != NULL) && (pAd->StaCfg[pObj->ioctl_if].SsidLen != 0)){
				for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
					...
					if (IS_ENTRY_PEER_AP(pEntry) && (pEntry->Sst == SST_ASSOC) && (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED) && (pEntry->wdev == wdev)) {
						pCurrState = 6;
						break;
					}
				}
	...
	char *arg)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		"%s: MAP R4, MAP_R3 and MAP_R2 is %s, %s, %s\n",
	...
	for_each_netdev(&init_net, ndev) {
		if (RTMP_OS_NETDEV_STATE_RUNNING(ndev)) {
			...
			MTWF_DBG(pAd, DBG_CAT_HIF, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"priv_flags(0x%lx), ndev->name(%s)\n", priv_flags, ndev->name);
			...
			if ((priv_flags == INT_MAIN) ||
				...
				if (pAd == NULL) {
					MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
						"ndev->name(%s) interface has closed!\n", ndev->name);
					return FALSE;
				}
				...
				dev_close(ndev);
				MTWF_PRINT("%s iface close success\n", ndev->name);
			}
		} else {
...
#endif
	if (ret < 0) {
		MTWF_PRINT("%s(): line(%d), iface %s cannot be opened, ret(%d).\n",
			__func__, __LINE__, ndev->name, ret);
	} else {
		MTWF_PRINT("%s(): line(%d), %s iface open success, ret(%d).\n",
			__func__, __LINE__, ndev->name, ret);
	}
...
INT	set_resume_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for_each_netdev(&init_net, ndev) {
		if (!(RTMP_OS_NETDEV_STATE_RUNNING(ndev))) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"ndev->name(%s) interface has opened!\n", ndev->name);
		}
	}
...
INT	show_resume_info(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	MTWF_PRINT("%s(): line(%d), bIsLowPower(%d)\n",
...
INT set_ap_idle_timeout(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "IdleTimeout=%d\n",
			pAd->ApCfg.ApIdleTimeout);
