	...
	IN RTMP_STRING * arg)
{
	...
	if (arg != NULL) {
		...
		if (apidx >= pAd->ApCfg.BssidNum) {
			...
			tr_entry = &tr_ctl->tr_entry[Wcid];
			MTWF_PRINT("BSS(%d), WCID(%d), AuthMode(%s, %x), PairwiseCipher(%s)\nGroupCipher(%s), GroupKeyId(%d), PortSecured(%s)\n",
				 apidx,
				 Wcid,
	...
	MTWF_PRINT("BSS\tWCID\tAuthMode\tPairwiseCipher\tPortSecured\n");
	...
	MTWF_PRINT("BSS\t\t\tAID\tWCID\tAuthMode\tPairwiseCipher\tPortSecured\n");
