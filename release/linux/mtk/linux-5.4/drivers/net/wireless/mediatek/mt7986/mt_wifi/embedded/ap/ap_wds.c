...
INT WdsEntryAlloc(RTMP_ADAPTER *pAd, UCHAR band_idx, UCHAR *pAddr)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE,
	...
	if (i == MAX_WDS_ENTRY)
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s: Unable to allocate WdsEntry.\n", __func__);
	else
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "Alloced WdsEntry[%d]\n", i);
...
VOID WdsEntryDel(RTMP_ADAPTER *pAd, UCHAR *pAddr)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s(): "MACSTR"\n", __func__,
			 MAC2STR(pAddr));
	...
	UINT WdsTabIdx)
{
	...
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s(): WdsTabIdx = %d, Addr "MACSTR"\n", __func__,
	...
	do {
		...
		if (pEntry) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "  OmacIdx = %d, wcid = %d, phymode = 0x%x\n",
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "phy_mode=0x%x\n", phy_mode);
			...
			if (WMODE_CAP_N(phy_mode)) {
				if (wdev->DesiredTransmitSetting.field.MCS != MCS_AUTO) {
					MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "IF-wds%d : Desired MCS = %d\n", WdsTabIdx,
			...
			if (WMODE_CAP_AC(phy_mode) && WMODE_CAP_5G(wdev->PhyMode)) {
				...
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s(): Peer's PhyCap=>Mode:%s, BW:%s, MCS: 0x%x (Word = 0x%x)\n",
							__func__,
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "PhyMode = 0x%x, Channel = %d\n",
					 wdev->PhyMode, wdev->channel);
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s() - allocate entry #%d(link to WCID %d)\n",
...
struct wifi_dev *wdev_search_by_address_for_WDS(RTMP_ADAPTER *pAd, UCHAR *address)
{
	...
	NdisReleaseSpinLock(&pAd->WdevListLock);
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s: can not find registered wdev\n", __func__);
	return NULL;
}
	...
	IN struct _RX_BLK *pRxBlk)
{
	...
	if (pAd->WdsTab.Mode[pRxBlk->band]) {
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s(): Wcid = %d, PhyMode = 0x%x\n", __func__,
...
VOID WdsTableMaintenance(RTMP_ADAPTER *pAd, UCHAR band_idx)
{
	...
	for (idx = 0; idx < MAX_WDS_ENTRY; idx++) {
		...
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s(): Entry[%d], Wcid = %d, "MACSTR"\n",
		...
		if (peer->NoDataIdleCount >= MAC_TABLE_AGEOUT_TIME) {
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "ageout "MACSTR" from WDS #%d after %d-sec silence\n",
	...
	IN BCN_IE_LIST * ie_list)
{
	...
	if ((WMODE_CAP_N(cmm_phy_mode) && WMODE_CAP_N(pWdsEntry->phy_mode))
		...
		if (HAS_HT_CAPS_EXIST(cmm_ies->ie_exists)) {
			...
			if (wlan_operate_get_ht_bw(wdev) != new_bw) {
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "%s(): set new_bw = %d\n", __func__, new_bw);
	...
	if ((pEntry->HTPhyMode.field.MODE != pEntry->MaxHTPhyMode.field.MODE) ||
		...
		pEntry->HTPhyMode.field.BW = pEntry->MaxHTPhyMode.field.BW;
		pEntry->HTPhyMode.field.MCS = pEntry->MaxHTPhyMode.field.MCS;
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, " bRaReInit, New Mode %d, MCS %d, STBC %d SGI %d, BW %d\n",
				 pEntry->HTPhyMode.field.MODE,
				 pEntry->HTPhyMode.field.MCS,
				 pEntry->HTPhyMode.field.STBC,
				 pEntry->HTPhyMode.field.ShortGI,
	...
	RTMP_STRING *buffer)
{
	...
	if (buffer == NULL) {
		...
		MTWF_DBG(ad, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE,
			 "WDS registering.\n");
...
VOID rtmp_read_wds_from_file(RTMP_ADAPTER *pAd, RTMP_STRING *tmpbuf, RTMP_STRING *buffer)
{
...
#ifdef DBDC_MODE
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "WDS Number: band[0]=%d, band[1]=%d\n",
		 pAd->WdsTab.wds_num[DBDC_BAND0], pAd->WdsTab.wds_num[DBDC_BAND1]);
#else
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "WDS Number: band[0]=%d\n",
	...
	if (RTMPGetKeyParameter("WdsEnable", tmpbuf, 10, buffer, TRUE)) {
		...
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); (macptr && i < DBDC_BAND_NUM); macptr = rstrtok(NULL, ";"), i++) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "band%d WDS-Enable mode=%d\n", i, pAd->WdsTab.Mode[i]);
		}
	}
	...
	if (RTMPGetKeyParameter("WdsPhyMode", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); (macptr && i < MAX_WDS_ENTRY); macptr = rstrtok(NULL, ";"), i++) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "If/wds%d - PeerPhyMode=0x%x\n", i, pWdsEntry->phy_mode);
		}
	}
	...
	if (RTMPGetKeyParameter("WdsTxMode", tmpbuf, 25, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); (macptr && i < MAX_WDS_ENTRY); macptr = rstrtok(NULL, ";"), i++) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "I/F(wds%d) Tx Mode = %d\n", i,
					 wdev->DesiredTransmitSetting.field.FixedTxMode);
		}
	}
	...
	if (RTMPGetKeyParameter("WdsTxMcs", tmpbuf, 50, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); (macptr && i < MAX_WDS_ENTRY); macptr = rstrtok(NULL, ";"), i++) {
			...
			if (wdev->DesiredTransmitSetting.field.MCS == MCS_AUTO)
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "I/F(wds%d) Tx MCS = AUTO\n", i);
			else {
				MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "I/F(wds%d) Tx MCS = %d\n", i,
						 wdev->DesiredTransmitSetting.field.MCS);
			}
		}
	}
	...
	if (RTMPGetKeyParameter("WDS_VLANID", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";"); (macptr && i < MAX_WDS_ENTRY); macptr = rstrtok(NULL, ";"), i++) {
			...
			MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "If/wds%d - WdsVlanId=%d\n", i, pAd->WdsTab.WdsEntry[i].wdev.VLAN_VID);
		}
	}
...
VOID WDS_Init(RTMP_ADAPTER *pAd, UCHAR band_idx, RTMP_OS_NETDEV_OP_HOOK *pNetDevOps)
{
	...
	MTWF_DBG(pAd, DBG_CAT_ALL, CATAP_WDS, DBG_LVL_TRACE,
...
VOID WDS_Remove(RTMP_ADAPTER *pAd)
{
	...
	for (index = 0; index < MAX_WDS_ENTRY; index++) {
		...
		MTWF_DBG(pAd, DBG_CAT_ALL, CATAP_WDS, DBG_LVL_TRACE, "%s: index:%d\n", __func__, index);
...
INT wds_inf_open(struct wifi_dev *wdev)
{
	...
	if (wdev) {
		...
		MTWF_DBG(pAd, DBG_CAT_AP, CATAP_WDS, DBG_LVL_TRACE, "WDS inf up for wds_%x(func_idx) OmacIdx=%d\n",
