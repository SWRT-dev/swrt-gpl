		...
		if (IS_CIPHER_CCMP128(GroupCipher) || IS_CIPHER_CCMP256(GroupCipher) ||
			...
			if (pEntry->Init_CCMP_BC_PN_Passed[key_idx] == FALSE) {
					if (rx_blk->CCMP_PN >= pEntry->CCMP_BC_PN[key_idx]) {
						MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "BC, %s (%d)(%d) OK: 1st come-in the %llu and now is %llu\n",
				...
				if (rx_blk->CCMP_PN <= pEntry->CCMP_BC_PN[key_idx]) {
					...
					MTWF_DBG(pAd, DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "BC, %s (%d)(%d) OK: come-in the %llu and now is %llu\n",
						__FUNCTION__, pEntry->wcid, key_idx, rx_blk->CCMP_PN, pEntry->CCMP_BC_PN[key_idx]);
					pEntry->CCMP_BC_PN[key_idx] = rx_blk->CCMP_PN;
				}
			}
	...
	IN UCHAR MgmtQid)
{
	...
	*/
	...
	if (pMacEntry->bAPSDDeliverEnabledPerAC[MgmtQid]) {
		...
		MTWF_DBG(pAd, DBG_CAT_PS, CATPS_UAPSD, DBG_LVL_TRACE, "ps> mgmt to UAPSD queue %d ... (IsDelts: %d)\n",
...
Label_Legacy_PS:
#endif /* UAPSD_CC_FUNC_PS_MGMT_TO_LEGACY */
		MTWF_DBG(pAd, DBG_CAT_PS, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
...
BOOLEAN is_gratuitous_arp(UCHAR *pData)
{
	...
	if (ProtoType == ETH_P_ARP) {
		...
		if (NdisCmpMemory(SenderIP, TargetIP, 4) == 0) {
			MTWF_DBG(NULL, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
...
BOOLEAN is_dad_packet(RTMP_ADAPTER *pAd, UCHAR *pData)
{
	...
	*/
	if (
		...
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
...
/* for MSP check always 0xFF for Rx log easily check PASS / FAIL */
		...
		if (pAd->auto_dvt && pAd->auto_dvt->apps.block_packet & ICMP_PACKET) {
			MTWF_DBG(pAd, DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Automation Block ICMP Packet\n");
			return FALSE;
		}
#endif /* !AUTOMATION */
			pSrcBuf += IP_HDR_LEN;
...
/* for MSP check always 0xFF for Rx log easily check PASS / FAIL */
	...
	case ETH_TYPE_ARP: {
			...
			if (pMbss->WNMCtrl.ProxyARPEnable) {
				...
				if (IsIPv4ProxyARPCandidate(pAd, pSrcBuf - 2)) {
					...
					if (!FoundProxyARPEntry)
						MTWF_DBG(pAd, DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Can not find proxy entry\n");
		...
		if (pAd->auto_dvt && pAd->auto_dvt->apps.block_packet & ARP_PACKET) {
			MTWF_DBG(pAd, DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Automation Block ARP Packet\n");
			return FALSE;
		}
#endif /* !AUTOMATION */
		...
		if (wdev->wdev_type == WDEV_TYPE_AP) {
			...
			if (pMbss->WNMCtrl.ProxyARPEnable) {
				...
				if (IsIPv6ProxyARPCandidate(pAd, pSrcBuf - 2)) {
					...
					if (!FoundProxyARPEntry)
						MTWF_DBG(pAd, DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Can not find IPv6 proxy entry\n");
...
INT TxOPUpdatingAlgo(RTMP_ADAPTER *pAd)
{
	...
	if ((TxTotalByteCnt == 0) || (RxTotalByteCnt == 0)) {
		...
		if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RDG_ACTIVE) == TRUE) {
			...
			MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Current TP=%lu < Threshold(%lu), turn-off TxOP\n",
	...
	if ((pAd->CommonCfg.bEnableTxBurst == FALSE)
		...
		if (WMM_UP2AC_MAP[UserPriority] == QID_AC_BK) {
			...
			if (pAd->flg_be_adjust == 0) {
				...
				MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "wmm> adjust be!\n");
			}
		} else {
			if (pAd->flg_be_adjust != 0) {
				QUEUE_HEADER *pQueue;
				...
				if ((pQueue == NULL) ||
					...
					if ((now - pAd->be_adjust_last_time) > TIME_ONE_SECOND) {
						...
						/* RTMP_SET_TX_BURST(pAd, wdev); */
						pAd->flg_be_adjust = 0;
						MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "wmm> recover be!\n");
					}
				} else
					NdisGetSystemUpTime(&pAd->be_adjust_last_time);
			}
...
VOID indicate_amsdu_pkt(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, UCHAR wdev_idx)
{
	...
	if (check_rx_pkt_pn_allowed(pAd, pRxBlk) == FALSE) {
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s:drop packet by PN mismatch!\n", __FUNCTION__);
	...
	UCHAR op_mode)
{
	...
	if (to_os == TRUE) {
		...
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s(): No need to send to OS!\n", __func__);
		tr_cnt->rx_to_os_drop++;
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
	}
}
...
VOID indicate_ampdu_pkt(RTMP_ADAPTER *pAd, RX_BLK *rx_blk, UCHAR wdev_idx)
{
	...
	if (!RX_BLK_TEST_FLAG(rx_blk, fRX_AMSDU) &&
		...
		if (err_size > 20) {
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE,
				"AMPDU DataSize = %d\n", rx_blk->DataSize);
...
VOID de_fragment_data_pkt(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "de_fragment_data_pkt: SN:%d,"
	...
	IN UCHAR wdev_idx)
{
	...
	if ((pEntry)
		...
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "CFG80211 EAPOL indicate_802_11_pkt\n");
#ifdef CONFIG_STA_SUPPORT
		...
		*/
		if (pStaCfg && (pEntry->wdev == &pStaCfg->wdev)) {
			if (pStaCfg->wdev.bLinkUpDone)
				...
				if (pEapBlk) {
					MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "CFG80211(%s) bLinkUpDone = %d, Q this eapol packet\n",
	...
	if (pEntry && IS_ENTRY_PEER_AP(pEntry)) {
		...
		if ((sup_info != NULL) && (sup_info->WpaSupplicantUP)) {
			...
			*/
			to_mlme = FALSE;
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "eapcode=%d\n", eapcode);
			...
			if ((wdev->SecConfig.IEEE8021X == TRUE) && IS_CIPHER_WEP(wdev->SecConfig.PairwiseCipher) &&
				...
				MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Receive EAP-SUCCESS Packet\n");
#ifdef CONFIG_AP_SUPPORT
	...
	IN UCHAR wdev_idx)
{
	...
	if (pAd == NULL) {
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "indicate_eapol_pkt: invalid pAd.\n");
		RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
		return;
	}

	if (!VALID_UCAST_ENTRY_WCID(pAd, pRxBlk->wcid)) {
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "indicate_eapol_pkt: invalid wcid.\n");
		RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
		return;
	}
	...
	if (pEntry == NULL) {
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "indicate_eapol_pkt: drop and release the invalid packet.\n");
		RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
		return;
	}
...
VOID dev_rx_mgmt_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (pRxBlk->DataSize > MAX_MGMT_PKT_LEN) {
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "DataSize=%d\n", pRxBlk->DataSize);
...
VOID dev_rx_ctrl_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s - subtype:%d\n", __func__, FC->SubType);
	...
	RX_BLK *rx_blk)
{
	...
	if ((rx_info->U2M) && VALID_UCAST_ENTRY_WCID(ad, rx_blk->wcid) && (IS_CIPHER_TKIP_Entry(entry))) {
		...
		/* bmc case */
		MTWF_DBG(ad, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Rx bc/mc Cipher Err(MPDUsize=%d, WCID=%d, CipherErr=%d)\n",
...
static INT rtmp_chk_rx_err(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (pRxInfo->Decrypted && pRxInfo->CipherErr) {
		if (pRxInfo->CipherErr == 2)
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "RxErr: ICV ok but MICErr");
		else if (pRxInfo->CipherErr == 1)
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "RxErr: ICV Err");
		else if (pRxInfo->CipherErr == 3)
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "RxErr: Key not valid");
		else
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "RxErr: CipherErr 0x%x", pRxInfo->CipherErr);
...
#endif /* CONFIG_STA_SUPPORT */
		rtmp_handle_mic_error_event(pAd, pRxBlk);
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s(): %d (len=%d, Mcast=%d, MyBss=%d, Wcid=%d, KeyId=%d)\n",
				...
				if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 0) &&
					...
					*/
					...
					if (NdisEqualMemory(EAPOL, pData, 2)) {
						...
						MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s():  Not my bss! pRxInfo->MyBss=%d\n", __func__, pRxInfo->MyBss);
						return FALSE;
					}
				}
			} else
	...
	if (wdev->UapsdInfo.bAPSDCapable
		...
		if ((*pData >> 4) & 0x01) {
			...
			if (pStaCfg->FlgPsmCanNotSleep == TRUE)
				MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "tdls uapsd> Rcv EOSP frame but we can not sleep!\n");
			else
#endif /* CONFIG_STA_SUPPORT */
		...
		if ((pFmeCtrl->MoreData) && (pAd->CommonCfg.bInServicePeriod))
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "MoreData bit=1, Sending trigger frm again\n");
	}
			...
			if ((CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_APSD_CAPABLE)) &&
				...
				*/
				...
				if ((*(pRxBlk->pData + LENGTH_802_11) & 0x10) == 0) {
					...
					MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "This is a EOSP frame, not a trigger frame!\n");
			}
		}
	...
	if (pEntry) {
		if (!RTMPCheckWPAframe(pAd, pEntry, pData, data_len, wdev_idx, eth_frame)) {
			if (RX_BLK_TEST_FLAG(pRxBlk, fRX_CM) && FC && (FC->Type == FC_TYPE_DATA) && (pRxBlk->DataSize) && (FC->Wep)) {
				MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s: CM, wcid=%d\n",
							__func__, pRxBlk->wcid);
				MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Addr1="MACSTR"\t",
							MAC2STR(pRxBlk->Addr1));
				MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Addr2="MACSTR"\n",
				...
				if (pEntry) {
					...
					if (IS_ENTRY_CLIENT(pEntry) && pEntry->pMbss) {
						if (((*(pData + 1) == EAPOLStart) || (*(pData + 1) == EAPPacket)) && (pMbss->HotSpotCtrl.HotSpotEnable == 1) &&  IS_AKM_WPA2(pMbss->wdev.SecConfig.AKMMap) && (pEntry->hs_info.ppsmo_exist == 1)) {
							...
							MTWF_DBG(pAd, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s: hotspot rcv eapol start, %x:%x:%x:%x\n",
...
INT rx_chk_amsdu_invalid_frame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, struct wifi_dev *wdev)
{
	...
	if (RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU) && (pRxBlk->AMSDU_ADDR)) {
		...
		if ((AmsduState == FIRST_AMSDU_FORMAT) &&
			...
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s: AMSDU ATTACK,	wcid=%d SN-AN(%d,%d)\n",
					__func__, pRxBlk->wcid, pRxBlk->SN, AmsduState);
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "80211 Addr=%02x:%02x:%02x:%02x:%02x:%02x\t",
					PRINT_MAC(pRxBlk->AMSDU_ADDR));
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "8023 Addr=%02x:%02x:%02x:%02x:%02x:%02x\n",
					PRINT_MAC(CmpAddr));
...
VOID rx_802_3_data_frm_announce(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, RX_BLK *pRxBlk, struct wifi_dev *wdev)
{
	...
	if (RX_BLK_TEST_FLAG(pRxBlk, fRX_CM) && FC   &&	(FC->Type == FC_TYPE_DATA)) {
		...
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Addr1="MACSTR"\t", MAC2STR(pRxBlk->Addr1));
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Addr2="MACSTR"\n", MAC2STR(pRxBlk->Addr2));
		RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
		return;
	}
	...
	RX_BLK *pRxBlk)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		if ((pRxBlk->DataSize == 0) && (pRxPacket)) {
			RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_SUCCESS);
			MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s():Packet Length is zero!\n", __func__);
			return NDIS_STATUS_INVALID_DATA;
		}
	}
	...
	if (rtmp_chk_rx_err(pAd, pRxBlk) != NDIS_STATUS_SUCCESS) {
		...
		RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
		MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s(): CheckRxError!\n", __func__);
		return NDIS_STATUS_INVALID_DATA;
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (fgHitMUTxOPCondition) {
		MTWF_DBG(pAd, DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "value of pAd->MUMIMO_TxOP_Value = %d\n", pAd->MUMIMO_TxOP_Value);
		...
		for (idx = 0; idx < TX_TYPE_MAX; idx++) {
			for (cpu = 0; cpu < NR_CPUS; cpu++) {
				if (pAd->tx_cnt.tx_pkt_to_hw[idx][cpu] != 0) {
					...
					if (pAd->ixia_ctl.debug_lvl >= IXIA_ERROR)
						MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "*%s-to_hw_cpu[%d]: %d .\n",
						txType[idx], cpu, pAd->tx_cnt.tx_pkt_to_hw[idx][cpu]);
				}
			}

			if (tx_type_total != 0)
				MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%s-tx_pkt_to_hw  : %d .\n",
