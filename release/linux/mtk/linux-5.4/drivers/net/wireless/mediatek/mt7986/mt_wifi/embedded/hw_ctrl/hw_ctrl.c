...
static NTSTATUS HwCtrlUpdateTxPer(RTMP_ADAPTER *pAd, HwCmdQElmt *CMDQelmt)
{
	...
	chip_get_sta_per(pAd, wlanid, &pEntry->tx_per);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
		("%s: STA(%d) update tx per(%d)!\n",
		__func__, pEntry->wcid, pEntry->tx_per));

	/* set wtbl BA_WIN_SIZE to limit sta agg num */
	dynamic_agg_adjust_per_sta(pAd, pEntry);
