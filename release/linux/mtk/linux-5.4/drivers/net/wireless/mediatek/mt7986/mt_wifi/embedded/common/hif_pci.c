...
{
	UINT i = 0;
	MTWF_PRINT("%s: current rxd_log_idx=%u, rxd_total_drop_cnt=%u, rxd_unkown_drop_cnt=%u\n", __func__, pAd->rxd_log_idx, pAd->rxd_total_drop_cnt, pAd->rxd_unkown_drop_cnt);
	...
	*pPktSize = 0;
	...
	do {
		...
		*pPktSize += pCurRxD->SDL0;
		...
		if ((LoopCnt == 1)
			&& ((*dw3 & RXDMAD_PPE_VLD) || (*dw1 & RXDMAD_TO_HOST_A)
		...
		if (cp_skb_pkt) {
			...
			*/
			/* if (scatterCnt > 1) */
			{
				...
				/* early check the type and dump record */
				if (check_rx_pkt_type(rx_pkt_type) == FALSE) {
					MTWF_DBG(pAd, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
							"@@ scatterCnt =%u, pRxD->LS0=%u, ,tkn_rx_id=%u\n", scatterCnt, pRxD->LS0, tkn_rx_id);
