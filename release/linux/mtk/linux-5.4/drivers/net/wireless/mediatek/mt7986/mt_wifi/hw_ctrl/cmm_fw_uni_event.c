...
VOID UniCmdResultRsp(struct cmd_msg *msg, char *payload, UINT16 payload_len)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: UniCmdResult.ucCID = 0x%x\n",
			__func__, UniCmdResult->u2CID));
	...
	if (UniCmdResult->u4Status != 0 || (UniCmdResult->u2CID != msg->attr.type)) {
		...
		MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::UniCmdResult.u4Status = 0x%x\n",
				  __func__, UniCmdResult->u4Status));
	}
}
	...
	struct cmd_msg *msg)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			...
			__func__, TlvData->ucEntryEnable, TlvData->fgSmeshRxA2, TlvData->fgSmeshRxA1,
...
VOID UniEventMibHandler(struct cmd_msg *msg, char *payload, UINT16 payload_len)
{
	...
	TAG_FOR_EACH(tag, tags_len, offset) {
		...
		case UNI_EVENT_MIB_DATA:
			...
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:Tag=0x%x, Counter=%d, Value=%lld\n",
					__func__, pUniEventMibData->u2Tag, pMibResult->Counter, pMibResult->Value));
			pMibResult++;
			break;
