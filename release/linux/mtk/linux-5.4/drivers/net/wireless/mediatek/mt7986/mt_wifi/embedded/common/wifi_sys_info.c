...
BOOLEAN wifi_sys_op_lock(struct wifi_dev *wdev)
{
	...
	if (!ret) {
		snprintf(wdev->dbg_wdev_op_lock_caller, sizeof(wdev->dbg_wdev_op_lock_caller), "%pS", OS_TRACE);
		MTWF_DBG(wdev->sys_handle, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%pS(%d): get wdev_op_lock.\n", OS_TRACE, wdev->wdev_idx);
...
{
	MTWF_DBG(wdev->sys_handle, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "%pS(%d): release wdev_op_lock.\n", OS_TRACE, wdev->wdev_idx);
...
INT wifi_sys_open(struct wifi_dev *wdev)
{
	...
	MTWF_DBG(NULL, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		"wdev idx = %d\n", wdev->wdev_idx);
...
INT wifi_sys_close(struct wifi_dev *wdev)
{
	...
	os_zero_mem(wsys, sizeof(struct WIFI_SYS_CTRL));
	MTWF_DBG(NULL, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "wdev idx = %d\n", wdev->wdev_idx);
...
INT wifi_sys_disconn_act(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry)
{
	...
	os_zero_mem(wsys, sizeof(struct WIFI_SYS_CTRL));
	MTWF_DBG(ad, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, " wdev_idx=%d\n", wdev->wdev_idx);
	...
	if (entry->EntryState == ENTRY_STATE_SYNC) {
		...
		MTWF_DBG(ad, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, " Not enqueue!! entry->EntryState=%d\n",
			entry->EntryState);
		wifi_sys_op_unlock(wdev);
	}
...
INT wifi_sys_conn_act(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry)
{
	...
	MTWF_DBG(ad, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "wdev idx = %d\n", wdev->wdev_idx);
