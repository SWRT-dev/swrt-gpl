	...
	UINT						Data)
{
	...
	peer = (UCHAR *)pData;
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_TRACE,
			"=====>  peer: "MACSTR" ,op: %d\n", MAC2STR(peer), Data);
	...
	if (Data == tdls_insert_entry) {
		...
		if (pMacEntry && IS_ENTRY_TDLS(pMacEntry))
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_TRACE, "MacTable Entry exist !!!\n");
		else
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_TRACE, "op: %d  DO NOTHING\n", Data);

	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_TRACE, "<====== out\n");
	return TRUE;
}
#ifdef UAPSD_SUPPORT
...
VOID cfg_tdls_rx_parsing(PRTMP_ADAPTER pAd, RX_BLK *pRxBlk)
{
	...
	if ((pRxBlk->DataSize >= (LENGTH_802_1_H + LENGTH_TDLS_H))
		...
		if (TDLSType == TDLS_ACTION_CODE_SETUP_REQUEST || TDLSType == TDLS_ACTION_CODE_SETUP_RESPONSE) {
			...
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_TRACE, "GOT CHSW RESP! %ld !!!\n", (jiffies * 1000) / OS_HZ);
			peerMAC = pRxBlk->pHeader->Addr2;
...
)
{
	...
	pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.IsentCHSW = TRUE;
	{
		...
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_TRACE, "Send CHSW REQ! %ld !!!\n", (jiffies * 1000) / OS_HZ);
		RTMPToWirelessSta(pAd, pMacEntry, Header802_3,
						  LENGTH_802_3, pOutBuffer, (UINT)FrameLen, FALSE);
	}
	return 0;
...
)
{
	...
	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_TRACE, "CHSW Resp Status:%d , SwitchTime:%d , SwitchTimeout:%d\n"
