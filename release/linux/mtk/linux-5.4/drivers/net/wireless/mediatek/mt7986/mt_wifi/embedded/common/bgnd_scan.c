...
VOID BuildBgndScanChList(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	for (channel_idx = 0; channel_idx < pAd->BgndScanCtrl.ChannelListNum; channel_idx++) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "Support channel: PrimCh=%d, CentCh=%d, DFS=%d\n",
...
VOID GenerateGroupChannelList(PRTMP_ADAPTER pAd)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "ChannelListNum=%d\n",
	...
	IN struct wifi_dev *wdev)
{
	...
	*/
	...
	if (pAd->BgndScanCtrl.BgndScanSupport) {
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "Background scan doesn't support in current settings....\n");
}
...
VOID BackgroundScanStartAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "ScanType=%d\n", bgnd_scan_type);
	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "ScanTypeInfo=%d\n", bgnd_band_scan_info);
	...
	MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "ChannelIdx [%d].PCCA_TIME=%x, SCCA_TIEM=%x, EDCCA_TIME=%x, Band0TxTime=%x Mdrdy=%x ===============>\n", pAd->BgndScanCtrl.ChannelIdx, Reg[0].Value, Reg[1].Value, Reg[2].Value, Reg[3].Value, Reg[4].Value);
#endif
	...
	MLME_QUEUE_ELEM *Elem)
{
	...
	if (pAd->BgndScanCtrl.ScanChannel == 0) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "First time\n");
		FirstBgndScanChannel(pAd);
#ifdef GREENAP_SUPPORT
		greenap_suspend(pAd, GREENAP_REASON_AP_BACKGROUND_SCAN);
#endif /* GREENAP_SUPPORT */
...
VOID BackgroundChannelSwitchAnnouncementAction(
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "Trigger Channel Switch Announcemnet IE\n");
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "ch_stat %d\n", *ch_stat);
#endif
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "Get new outband DFS channel\n");
	...
	IN RTMP_ADAPTER * pAd, BOOLEAN apply_cur_ch)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "start.\n");
#if (RDD_2_SUPPORTED == 0)
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "end.\n");
	...
	IN PVOID SystemSpecific3)
{
	...
	case DFS_INB_DFS_RADAR_OUTB_CAC_DONE:
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_SCAN, DBG_LVL_TRACE, "New inband channel %d bandidx %d\n",
