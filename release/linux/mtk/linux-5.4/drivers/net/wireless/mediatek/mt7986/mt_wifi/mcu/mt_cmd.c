	...
	IN UINT16 rsp_len)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: FuncIndex = %d\n", __func__, FuncIndex)); /* only use funcIdx = 3 */
	...
	IN UINT8 *pData)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s :\n"
			...
			" pPhyIcsInfo->u4PhyIcsTimer = %d\n", __func__, pPhyIcsInfo->fgTrigger, pPhyIcsInfo->fgRingCapEn,
	...
	IN UINT16 rsp_len)
{
	...
	Status = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, (Status != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_TRACE,
			"(Status = %d)\n", Status);
...
error:
	...
	MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, (Status != CAP_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_TRACE,
			"(Status = %d)\n", Status);
...
INT32 MtCmdChannelSwitch(RTMP_ADAPTER *pAd, MT_SWITCH_CHANNEL_CFG SwChCfg)
{
...

	{
		...
		if (SwChCfg.ap_bw) {
			MTWF_DBG(pAd, DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "AP BW = %d, AP central_chn = %d\n", SwChCfg.ap_bw, SwChCfg.ap_central_channel);
		}
	}
...
INT32 MtCmdSetForceTxPowerCtrl(RTMP_ADAPTER *pAd, UINT8 ucBandIdx, INT8 cTxPower, UINT8 ucPhyMode, UINT8 ucTxRate, UINT8 ucBW)
{
	...
	struct _CMD_ATTRIBUTE attr = {0};
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Band(%d), TxMode(%d), MCS(%d), BW(%d), TxPower(%d)\n",
	...
	UINT_8        u1OwnMacIdx)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: Aid:%u, Band:%u, OwnMac:%u\n", __func__, u2Aid, u1BandIdx, u1OwnMacIdx));
	...
	PUINT8 pucData)
{
	...
	MTWF_DBG(pAd, DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	...
	EXT_CMD_ETXBF_PFMU_SW_TAG_T rEBfPfmuSwTag)
{
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_FW, DBG_SUBCAT_ALL, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_TRACE, "(ret = %d)\n", ret);
	return ret;
}
	...
	PUINT8 pucData)
{
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_FW, DBG_SUBCAT_ALL, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_TRACE,
			 "(ret = %d)\n", ret);
	return ret;
}
...
static VOID CmdHwcfgRDRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_DBG(NULL, DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 "Len = 0x%x\n",
			  Len);
	...
	PSTA_ADMIN_CONFIG pStaCfg)
{
	...
	if (IS_AKM_PSK(pEntry->SecConfig.AKMMap)) {
		...
		MTWF_DBG(pAd, DBG_CAT_P2P, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	...
	ret = AndesSendCmdMsg(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_TRACE,
			 "(ret = %d)\n", ret);
}
...
INT32 MtCmdBcnV2OffloadSet(RTMP_ADAPTER *pAd, CMD_BCN_OFFLOAD_T_V2 *bcn_offload_v2)
{
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_AP, CATAP_BCN, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_TRACE,
			 "(ret = %d)\n", ret);
	return ret;
}
#endif
						  ...
						  struct _EXT_CMD_SNIFFER_MODE_T *param)
{
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_TEST, DBG_SUBCAT_ALL, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_TRACE,
						 ...
						 UINT32 len, UINT32 data_mode)
{
	...
	MTWF_DBG(ad, DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	...
	ret = chip_cmd_tx(ad, msg);
error:
	MTWF_DBG(ad, DBG_CAT_FW, DBG_SUBCAT_ALL, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_ERROR:DBG_LVL_TRACE,
			 "(ret = %d)\n", ret);
	return ret;
}
...
)
{
	...
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_DBG(pAd, DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "fgTpcUtGo: %d \n", fgTpcUtGo);
...
)
{
	...
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_DBG(pAd, DBG_CAT_FW, DBG_SUBCAT_ALL, (ret != NDIS_STATUS_SUCCESS) ? DBG_LVL_TRACE:DBG_LVL_ERROR,
			 "(ret = %d)\n",  ret);
	return ret;
}
...
INT32 MtCmdPsStaFlushCtrl(struct _RTMP_ADAPTER *pAd)
{
	...
	Ret = chip_cmd_tx(pAd, msg);
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			"%s: fgPsSTAFlushEnable(%d), u2FlushThldTotalMsduNum(%d), u2PerStaMaxMsduNum(%d)\n",
			__func__, ps_ctrl.fgPsSTAFlushEnable, ps_ctrl.u2FlushThldTotalMsduNum, ps_ctrl.u2PerStaMaxMsduNum);
...
INT SetHeraIara_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Enable=%d \n", u4Value);
...
INT SetHeraOptionFrequecyDup_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (!arg) {
		MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "Null Parameters\n");
		return FALSE;
	}
	...
	UINT16 user_idx)
{
	...
	MTWF_DBG(pAd, DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "band_idx: %d, user_idx: 0x%x\n", band_idx, user_idx);
