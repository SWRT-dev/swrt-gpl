			       ...
			       const struct phylink_link_state *state)
{
	...
	/* Assert PHYA power down state */
#if 0 /* SGMII no power workaround */
	regmap_write(ss->regmap_sgmii[id], SGMSYS_QPHY_PWR_STATE_CTRL, SGMII_PHYA_PWD);
#endif
	...
	regmap_write(ss->regmap_sgmii[id], SGMSYS_QPHY_PWR_STATE_CTRL, 0);
#else
	regmap_read(ss->regmap_sgmii[id], SGMSYS_QPHY_PWR_STATE_CTRL, &val);
	val &= ~SGMII_PHYA_PWD;
	regmap_write(ss->regmap_sgmii[id], SGMSYS_QPHY_PWR_STATE_CTRL, val);
#endif
