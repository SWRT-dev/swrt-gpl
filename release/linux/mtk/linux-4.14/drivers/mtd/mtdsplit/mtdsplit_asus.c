				   ...
				   struct mtd_part_parser_data *data)
{
	...
	if (kernel_size == 0) {
		printk(KERN_INFO "no asus header found in \"%s\"\n", master->name);
		ret = -ENODEV;
		goto err_free_buf;
	}
...
static const struct of_device_id asus_parser_of_match_table[] = {
	{ .compatible = "swrt,asus" },
	{},
};
#endif
...
static struct mtd_part_parser asus_parser = {
	...
	.name = "asus-fw",
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 9, 0)
	.of_match_table = asus_parser_of_match_table,
#endif
