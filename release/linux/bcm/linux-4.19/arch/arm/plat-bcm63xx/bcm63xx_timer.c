...
static notrace u64 bcm63xx_read_timer_count(struct clocksource *cs)
{
	int ret;
	uint64_t count; /* u64 is 64 bit integer */
...
static int periph_timer_starting_cpu(unsigned int cpu)
{
	int *timer_ce = this_cpu_ptr(&timer_ce_used);
...
static int periph_timer_dying_cpu(unsigned int cpu)
{
	struct clock_event_device *evt = this_cpu_ptr(&clockevent_timer);
...
static int __init bcm63xx_timer_of_register(struct device_node *np)
{
	...
	err = cpuhp_setup_state(CPUHP_AP_ARM_ARCH_TIMER_STARTING,
		...
		periph_timer_starting_cpu, periph_timer_dying_cpu);
	...
	{
		printk("bcm63xx_timer_init: cpuhp_setup_state failed\n");
	}

	return err;
}

TIMER_OF_DECLARE(bcm63xx, "brcm,bcm6xxx-timer", bcm63xx_timer_of_register);
