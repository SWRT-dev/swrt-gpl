...
static int sdhci_bcm63xx_setup_baseclk(struct platform_device *pdev)
{
	struct sdhci_host *host = platform_get_drvdata(pdev);
        struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	unsigned int base_clk_freq;
	void __iomem *top_cfg_base;
        struct resource *res;
	...
	if( pltfm_host->clock != (base_clk_freq) )
	{
		...
		/* Option 1: override baseclock in override register */
	 	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "sdhci-topcfg" );
                if (!res) {
	        	dev_err(&pdev->dev, "Platform resource sdhci-topcfg is missing\n");
		        return -EINVAL;
	        }
		top_cfg_base = devm_ioremap_resource(&pdev->dev, res);
                if (IS_ERR(top_cfg_base))
			return PTR_ERR(top_cfg_base);
...
static int sdhci_bcm63xx_probe(struct platform_device *pdev)
{
	...
	writel(SDHCI_BCM63XX_TOPCFG_SD_PIN_SEL_VAL, reg_base 
	...
	if( pltfm_host->clock )
		if( sdhci_bcm63xx_setup_baseclk(pdev) )
