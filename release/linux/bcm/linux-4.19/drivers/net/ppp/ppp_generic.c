...
/* Added by Foxconn start. */
int g_disable_idle=0;
EXPORT_SYMBOL(g_disable_idle);
int (*acos_ppp_check_function)(struct ppp *ppp, struct sk_buff *skb) = NULL;
EXPORT_SYMBOL(acos_ppp_check_function);
int (*acos_ppp_check_function2)(struct ppp *ppp, struct sk_buff *skb) = NULL;
EXPORT_SYMBOL(acos_ppp_check_function2);
/* Added by Foxconn end. */
...
static long ppp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case PPPIOCGIDLE:
		...
		/* Modified by Foxconn start. */
		if(g_disable_idle)
		    idle.xmit_idle = 0;
		//idle.recv_idle = (jiffies - ppp->last_recv) / HZ;
		idle.recv_idle = idle.xmit_idle;
		/* Modified by Foxconn end. */
...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (proto < 0x8000) {
		...
		*(u8 *)skb_push(skb, 2) = 1;
		      ...
		      BPF_PROG_RUN(ppp->active_filter, skb) == 0))
		{
		    ...
		    if(!acos_ppp_check_function2 || (acos_ppp_check_function2 && acos_ppp_check_function2(ppp, skb)))
		    {
			    ppp->last_xmit = jiffies;
			}
	...
	if (ppp->flags & SC_LOOP_TRAFFIC) {
	    /* Added by Foxconn start. */
		if(acos_ppp_check_function)
		{
            if(!acos_ppp_check_function(ppp, skb))
            {
                goto drop;
            }
        }
