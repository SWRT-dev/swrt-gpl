    ...
    if (sock) 
    {
        ...
        {   
            ...
            } else if (opt->seq_recv && ((*rcv_pktSeq) > opt->seq_recv)) 
            {
                ...
                printk(KERN_INFO "bcm_pptp_rcv_check():[BLOG_PPTP_RCV_OOS_LT] current seq_recv is %d \n", opt->seq_recv);
                ret = BLOG_PPTP_RCV_OOS_LT;
            } else {
                printk(KERN_INFO "bcm_pptp_rcv_check():[BLOG_PPTP_RCV_OOS_GT] current seq_recv is %d \n", opt->seq_recv);
                opt->seq_recv = (*rcv_pktSeq);
...
int bcm_pptp_xmit_get(uint16_t call_id, uint32_t* seqNum, uint32_t* ackNum, uint32_t daddr)
{
    struct pppox_sock *sock;
    struct pptp_opt *opt;
...
void __bcm_pptp_xmit_update(uint16_t call_id, uint32_t seqNum, uint32_t ackNum, uint32_t daddr)
{
    struct pppox_sock *sock;
    struct pptp_opt *opt;
    int i;
...
void bcm_pptp_xmit_update(uint16_t call_id, uint32_t seqNum, uint32_t ackNum, uint32_t daddr)
{
    rcu_read_lock();
    __bcm_pptp_xmit_update(call_id, seqNum, ackNum, daddr);
    rcu_read_unlock();
    return;
}

blog_pptp_rcv_check_t blog_pptp_rcv_check_fn = (blog_pptp_rcv_check_t) bcm_pptp_rcv_check;
blog_pptp_xmit_get_t blog_pptp_xmit_get_fn = (blog_pptp_xmit_get_t) bcm_pptp_xmit_get;
