...
#if IS_ENABLED(CONFIG_BCM_KF_MAP)
...
int blog_mcast_add_rx_dev(Blog_t *mblog_p, Blog_t *cblog_p)
{
    ...
    for (i=0; (i<MAX_VIRT_DEV) && (mdev_info_p->num_dev < mdev_info_p->max_dev); i++)
    {
                ...
                void * net_p, uint32_t param1, uint32_t param2 )
{
    ...
    switch ( entity_type )
    {
        case FLOWTRACK:
        {
            ...
            blog_p->ct_ver[idx] = ((param2 == BLOG_PARAM2_GRE_IPV4) || (param2 == BLOG_PARAM2_L2TP_IPV4)) ?
            ...
            blog_print( "idx<%d> ct_p<%px> ct_ver<%d>\n",
        ...
        case MAP_TUPLE:
        {
            ...
            blog_print( "map_p<%px> net<%px>", blog_p->map_p, net_p );
        ...
        case MEGA:
        {
            ...
            blog_print( "mega_p<%px> net<%px>", blog_p->mega_p, net_p );
            break;
        }
#endif
...
void blog_nfct_dump( struct nf_conn * ct, uint32_t dir )
{
    ...
    printk("\tNFCT: ct<0x%px>, master<0x%px>\n"
           ...
           "\t\thelp<0x%px> helper<%s> status=%lx refcnt=%d zone=%d\n",
            ct, 
            ct->master,
            nat_p, 
...
void blog_ct_dump(Blog_t *blog_p)
{
    ...
    printk("\tdelCt<%px> key[0x%08x:%d] pldCt<%px> key[0x%08x:%d]\n",
            nwe_del_p, nwe_del_p ?
            nwe_del_p->bcm_ext.blog_key[blog_p->nf_dir_del] : 0, blog_p->nf_dir_del,
            nwe_pld_p, nwe_pld_p ?
            nwe_pld_p->bcm_ext.blog_key[blog_p->nf_dir_pld] : 0, blog_p->nf_dir_pld);
...
void blog_dump( Blog_t * blog_p )
{
    ...
    printk( "BLOG <%px> owner<%px> flags<0x%08x> tos_mode<%u:%u>\n"
            ...
            "\thash<0x%08x> prot<%u> wl<0x%08x> wlinfo<0x%4x>\n"
