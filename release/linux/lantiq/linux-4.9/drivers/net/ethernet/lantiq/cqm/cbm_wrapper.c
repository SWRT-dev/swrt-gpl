...
{
	if (g_cbm_ops && g_cbm_ops->cbm_enable_backpressure)
...
{
	if (g_cbm_ops && g_cbm_ops->cbm_get_mtu_size)
...
int cbm_setup_desc(struct cbm_desc *desc, u32 data_ptr, u32 data_len,
		   struct sk_buff *skb)
{
	if (g_cbm_ops && g_cbm_ops->cbm_setup_desc)
...
{
	if (g_cbm_ops->cqm_buffer_free_by_policy)
		return g_cbm_ops->cqm_buffer_free_by_policy(free_info);
	else
		return CBM_FAILURE;
}
		  ...
		  struct cbm_dp_en_data *data, u32 flags, u32 alloc_flags)
{
	if (g_cbm_ops && g_cbm_ops->cbm_dp_enable)
		return g_cbm_ops->cbm_dp_enable(owner, dp_port, data, flags,
						alloc_flags);
	else
		return CBM_FAILURE;
}
