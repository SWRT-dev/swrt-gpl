...
static int enable_hgu_trunking(void)
{
	...
	if((ret=gsw_api_kioctl(gswipr, GSW_PORT_CFG_GET, &portcfg)) < GSW_statusOk) {
	...
	if((ret=gsw_api_kioctl(gswipr, GSW_PORT_CFG_SET, &portcfg)) < GSW_statusOk) {
	...
	if((ret=gsw_api_kioctl(gswipl, GSW_REGISTER_GET, &regcfg)) < GSW_statusOk) {
	...
	if((ret=gsw_api_kioctl(gswipl, GSW_REGISTER_SET, &regcfg)) < GSW_statusOk) {
	...
	if((ret=gsw_api_kioctl(gswipl, GSW_PCE_RULE_WRITE, &pcecfg)) < GSW_statusOk) {
