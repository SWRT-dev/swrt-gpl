		...
		char *param)
{
	...
	if (iface->state != HAPD_IFACE_ENABLED) {
		wpa_printf(MSG_ERROR, "Reconfig cannot be done before hostapd is enabled");
		return -1;
	}
        ...
        const struct hostapd_freq_params *freq_params)
{
  ...
  if(freq_params->center_freq1) {
    if (is_6ghz_freq(freq_params->center_freq1))
      vht_seg0_idx = 1 + (freq_params->center_freq1 - 5945) / 5;
    else
  ...
  if(freq_params->center_freq2) {
    if (is_6ghz_freq(freq_params->center_freq2))
      vht_seg1_idx = 1 + (freq_params->center_freq2 - 5945) / 5;
    else
...
static int update_rnr_in_beacon(struct hostapd_data *hapd, struct update_rnr rnr, int radio_index, int vap_index)
{
	int ret = 0;
	...
	*/
	...
	os_memcpy(hapd->iconf->coloc_6g_ap_info[radio_index].bss_info[vap_index].bssid, rnr.bssid, ETH_ALEN);
	hapd->iconf->coloc_6g_ap_info[radio_index].bss_info[vap_index].BssParams = 0; /* clear */
	hapd->iconf->coloc_6g_ap_info[radio_index].bss_info[vap_index].BssParams |= RNR_TBTT_INFO_BSS_PARAM_COLOC_ESS; /* TODO (6GHz) Dynamic */
	...
	if( !os_memcmp( hapd->conf->ssid.ssid, rnr.ssid, hapd->conf->ssid.ssid_len ) )	
	{ 
	...
	os_memcpy(hapd->iconf->coloc_6g_ap_info[radio_index].bss_info[vap_index].Ssid, rnr.ssid, os_strlen((const char *)rnr.ssid));
	/*
	...
	WPA_PUT_LE32(hapd->iconf->coloc_6g_ap_info[radio_index].bss_info[vap_index].ShortSsid,
			 crc32(rnr.ssid, os_strlen((const char *)rnr.ssid)) );
                ...
                char *cmd)
{
	...
	/* note: We don't support hidden ssid? */
	PARSE_TOKEN(tmp,cmd," ",&ptr);
	...
	os_memcpy(rnr.ssid, tmp, os_strlen(tmp));
...
static int hostapd_ctrl_iface_get_multibss_enable(struct hostapd_data *hapd, char *reply)
{
	if(hapd->iconf->multibss_enable)
		os_snprintf(reply, 2, "1");
	else
		os_snprintf(reply, 2, "0");
	return 1;
...
{
	os_snprintf(reply, 3, "%d", hapd->conf->unsolicited_frame_duration);
	return 3;
}
...
{
	int val = atoi(cmd);
	if ( val <= MIN_UNSOLICITED_FRAME_DURATION || val > MAX_UNSOLICITED_FRAME_DURATION )
		return -1;
	else
		hapd->conf->unsolicited_frame_duration = val;
...
{
	os_snprintf(reply, 2, "%d", hapd->conf->unsolicited_frame_support);
	return 1;
}
...
{
	int val = atoi(cmd);
	if ( val < DISABLE_UNSOLICITED_FRAME_SUPPORT || val > FILS_DISCOVERY_ENABLE )
	{
		wpa_printf(MSG_ERROR, "CTRL: UPDATE_RNR: Bad unsolicited_frame_support %s"
				" choose 0:Disable, 1:20TU_Probe_response, 2:FILS Discovery", cmd);
		return -1;
	}
	else {
		hapd->conf->unsolicited_frame_support = val;
		if( hostapd_drv_set_unsolicited_frame(hapd, val, hapd->conf->unsolicited_frame_duration) < 0 )
			return -1;
		...
		char *cmd)
{
	...
	if (link_msmt_report.rx_ant_id <= 0) {
		wpa_printf(MSG_ERROR,
	...
	if (link_msmt_report.tx_ant_id <= 0) {
		wpa_printf(MSG_ERROR,
					      ...
					      socklen_t fromlen)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		wpa_printf(MSG_MUTE, "%s; *** Received from FAPI: 'STA_STEER' (buf= '%s') ***\n", __FUNCTION__, buf);
		if (hostapd_ctrl_iface_sta_steer(hapd, buf + 10))
			reply_len = -1;
	} else if (os_strncmp(buf, "STA_ALLOW", 9) == 0) {
		wpa_printf(MSG_MUTE, "%s; *** Received from FAPI: 'STA_ALLOW' (buf= '%s') ***\n", __FUNCTION__, buf);
		...
		reply_len = hostapd_ctrl_iface_get_multibss_enable(hapd,reply);
	} else if (os_strncmp(buf, "IS_TRANSMITTED_BSSID", 20) == 0) {
		reply_len = hostapd_ctrl_iface_is_transmitted_bssid(hapd,reply);
	} else if (os_strncmp(buf, "GET_UNSOLICITED_FRAME_SUPPORT", 29) == 0) {
		reply_len = hostapd_ctrl_iface_get_unsolicited_frame_support(hapd,reply);
		...
		reply_len = hostapd_ctrl_iface_get_unsolicited_frame_duration(hapd,reply);
