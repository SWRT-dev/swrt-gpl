	     ...
	     char *argv[])
{
	if (argc < 14) {
		printf("Invalid report_beacon command: needs at least 14"
			...
			" rcpi, rsni, bssid, ant_id, tsf + some optianl arguments\n");
		return -1;
	}
...
static int hostapd_cli_cmd_update_rnr(struct wpa_ctrl *ctrl,int argc, char *argv[])
{
	...
	return hostapd_cli_cmd(ctrl, "UPDATE_RNR", 6, argc, argv);
}
...
static int hostapd_cli_cmd_get_multibss_enable(struct wpa_ctrl *ctrl,int argc, char *argv[])
{
	...
	res = os_snprintf(cmd, sizeof(cmd), "GET_MULTIBSS_ENABLE");

	if (os_snprintf_error(sizeof(cmd), res)) {
                printf("Too long get_multibss_enable command.\n");
                return -1;
        }
...
static int hostapd_cli_cmd_is_transmitted_bssid(struct wpa_ctrl *ctrl,int argc, char *argv[])
{
	...
	res = os_snprintf(cmd, sizeof(cmd), "IS_TRANSMITTED_BSSID");

	if (os_snprintf_error(sizeof(cmd), res)) {
                printf("Too long is_tranmitted_bssid command.\n");
                return -1;
        }
...
static int hostapd_cli_cmd_get_unsolicited_frame_duration(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	res = os_snprintf(cmd, sizeof(cmd), "GET_UNSOLICITED_FRAME_DURATION");

	if (os_snprintf_error(sizeof(cmd), res)) {
                printf("Too long get_unsolicited_frame_duration command.\n");
                return -1;
        }
...
static int hostapd_cli_cmd_set_unsolicited_frame_duration(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	int total, i;
        ...
        res = os_snprintf(cmd, sizeof(cmd), "SET_UNSOLICITED_FRAME_DURATION");
...
static int hostapd_cli_cmd_get_unsolicited_frame_support(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	res = os_snprintf(cmd, sizeof(cmd), "GET_UNSOLICITED_FRAME_SUPPORT");

	if (os_snprintf_error(sizeof(cmd), res)) {
		printf("Too long get_unsolicited_frame_support command.\n");
		return -1;
	}
...
static int hostapd_cli_cmd_set_unsolicited_frame_support(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	res = os_snprintf(cmd, sizeof(cmd), "SET_UNSOLICITED_FRAME_SUPPORT");
        if (os_snprintf_error(sizeof(cmd) - 1, res))
                goto err;
