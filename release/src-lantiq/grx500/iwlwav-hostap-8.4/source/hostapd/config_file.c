...
static int hostapd_parse_unsolicited_frame_support(struct hostapd_bss_config *bss, char *val)
{
	if ( val )
	{
		bss->unsolicited_frame_support = atoi(val);
		if ( bss->unsolicited_frame_support < DISABLE_UNSOLICITED_FRAME_SUPPORT || 
			bss->unsolicited_frame_support > FILS_DISCOVERY_ENABLE )
		{
...
static int hostapd_parse_unsolicited_frame_duration(struct hostapd_bss_config *bss, char *val)
{
	if ( val )
	{
		bss->unsolicited_frame_duration = atoi(val);
		if ( bss->unsolicited_frame_duration <= MIN_UNSOLICITED_FRAME_DURATION 
			|| bss->unsolicited_frame_duration > MAX_UNSOLICITED_FRAME_DURATION )
		{
		...
		struct hostapd_bss_config *bss, char *val)
{
	...
	/* expected order: radio_index BSSID SSID multibss_enable is_transmitted_bssid
		eg: 0 11:22:33:44:55:66 test_6g_vap 1 1 */
	u8 bssid[ETH_ALEN] = {'\0'};
	u8 empty_bssid[ETH_ALEN] = {'\0'};
	u8 ssid[SSID_MAX_LEN] = {'\0'};
	int j = 0;
	Boolean found = FALSE; // flag will be used to find whether bssid entry found
	Boolean multibss_enable = FALSE, is_transmitted_bssid = FALSE, unsolicited_frame_support = FALSE;

	PARSE_TOKEN(tmp, val, " ", &ptr);
	param_val = atoi(tmp);
	if(( param_val < 0 ) || ( param_val >= MAX_SUPPORTED_6GHZ_RADIO ))
	{
	...
	PARSE_TOKEN(tmp, val, " ", &ptr);
	if( os_strlen(tmp) >= SSID_MAX_LEN  ) {
		wpa_printf(MSG_ERROR,
		   "CTRL: UPDATE_RNR: Bad SSID %s", tmp);
		return -1;
	}
	os_memcpy(ssid, tmp, os_strlen(tmp));
	...
	if((found == FALSE) )
	{
		wpa_printf(MSG_ERROR,"No availble space to hold new rnr report");
		return -1;
	}
	os_memcpy(conf->coloc_6g_ap_info[radio_index].bss_info[j].Ssid, ssid, os_strlen((const char *)ssid));
	conf->coloc_6g_ap_info[radio_index].bss_info[j].NeighApTbttOffset = TBTT_OFFSET_UNKNOWN; /* TODO (6GHz) Dynamic, 255=unknown */
	os_memcpy(conf->coloc_6g_ap_info[radio_index].bss_info[j].bssid, bssid, ETH_ALEN);
	/*
	OCT Recommended 0
	Same SSID  x
	Multiple BSSID x
	Transmitted BSSID x
	Member of ESS with 2.4/5GHz Co-located AP(#20024) 1
	20 TU Probe Response Active x
	Co-Located  AP 1
	Reserved 0
	0 1 x 1 x x x 0 
	*/
	conf->coloc_6g_ap_info[radio_index].bss_info[j].BssParams |= RNR_TBTT_INFO_BSS_PARAM_COLOC_ESS; /* TODO (6GHz) Dynamic */
	 ...
	 */
	WPA_PUT_LE32(conf->coloc_6g_ap_info[radio_index].bss_info[j].ShortSsid, crc32(ssid, os_strlen((const char *)ssid)) );
	if( multibss_enable )
	{ 
	...
	if( !os_memcmp( bss->ssid.ssid, ssid, bss->ssid.ssid_len ) )	
	{ 
...
static int hostapd_parse_colocated_6g_radio_info(struct hostapd_config *conf, char *val)
{
	...
	param_val = atoi(tmp);
	if( ( param_val < 0 ) ||
			 ( param_val >= MAX_SUPPORTED_6GHZ_RADIO ) ) {
			wpa_printf(MSG_ERROR, "CONFIG: UPDATE_RNR: Bad radio_index %s", tmp);
			return -1;
	}
	radio_index = param_val;
	...
	conf->coloc_6g_ap_info[radio_index].ChanNum = param_val;
			       ...
			       const char *buf, char *pos, int line)
{
    ...
    if (conf->ap_max_num_sta < 0 || conf->ap_max_num_sta > MAX_STA_COUNT) {
      ...
      return 1;
		...
		if (len > AP_MAX_VALID_CHAR) {
			wpa_printf(MSG_ERROR, "Line %d: invalid erp_domain len %d",
				   line, len);
			return 1;
		}
		...
		if (bss->max_num_sta < 0 ||
			...
			return 1;
               ...
               bss->num_vrt_bkh_netdevs = atoi(pos);
               if (bss->num_vrt_bkh_netdevs < 0 ||
                   bss->num_vrt_bkh_netdevs > 8) {
                       wpa_printf(MSG_ERROR, "Line %d: Invalid num_vrt_bkh_netdevs=%d; allowed range 0..%d",
                                  line, bss->num_vrt_bkh_netdevs, 8);
                       return 1;
               }
    ...
    if (bss->num_res_sta < 0) {
      ...
      return 1;
		...
		int val = atoi(pos);
		if(!(IS_VALID_RANGE(val,81,84) || IS_VALID_RANGE(val,115,135))) {
			wpa_printf(MSG_ERROR, "Line %d: invalid op_class %d",
                                line, val);
                        return 1;
		}
		conf->op_class = val;
    ...
    */
    ...
    for (i = 0; i < ACS_BW_THRESH_NUM; i++) {
      if ((conf->acs_bw_threshold[i] > 100) ||
		...
		int vlan;
		vlan = atoi(pos);
		if (!IS_VALID_RANGE(val, DYNAMIC_VLAN_DISABLED, DYNAMIC_VLAN_REQUIRED)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid dynamic_vlan value %d",
				line, val);
			return 1;
		}
		bss->ssid.dynamic_vlan = val;
		...
		else if (os_strcmp(pos, "ext_hybrid") == 0) val = MESH_MODE_EXT_HYBRID;
		...
		conf->scan_passive_total_per_channel = val;
		...
		conf->channel_transition_delay_factor = val;
		...
		conf->scan_activity_threshold = val;
		...
		conf->allow_scan_during_cac = val;
		...
		if ((conf->obss_beacon_rssi_threshold < -100)
			|| (conf->obss_beacon_rssi_threshold > 0)) {
	...
	/* HE debug */
		...
		int val = atoi(pos);
		if ((val < EXT_CAP_MAX_NUM_MSDU_IN_AMSDU_MIN) || (val > EXT_CAP_MAX_NUM_MSDU_IN_AMSDU_MAX)) {
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, FRIENDLY_NAME_MAX_LEN)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid friendly_name len %d",
				   line, len);
			return 1;
		}
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, AP_MAX_VALID_CHAR)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid manufacturer_url len %d",
				   line, len);
			return 1;
		}
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, MODEL_DESCRIPTION_MAX_LEN)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid model_description len %d",
				   line, len);
			return 1;
		}
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, AP_MAX_VALID_CHAR)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid model_url len %d",
				   line, len);
			return 1;
		}
		...
		if (len != UNIVERSAL_PRODUCT_CODE_MAX_DIGIT) {
			wpa_printf(MSG_ERROR, "Line %d: invalid Universal Product Code (upc) len %d",
				line, len);
			return 1;
		}
		os_free(bss->upc);
		...
		*bss->network_auth_type = auth_type;
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, AP_MAX_VALID_CHAR)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid osu_server_uri len %d",
				   line, len);
			return 1;
		}
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, AP_MAX_VALID_CHAR)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid osu_nai len %d",
				   line, len);
			return 1;
		}
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, AP_MAX_VALID_CHAR)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid osu_nai2 len %d",
				   line, len);
			return 1;
		}
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, AP_MAX_VALID_CHAR)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid osu_method_list len %d",
				   line, len);
			return 1;
		}
		...
		int len = os_strlen(pos);
		if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, AP_MAX_VALID_CHAR)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid osu_icon len %d",
				   line, len);
			return 1;
		}
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
		...
		conf->sConfigMRCoex = atoi(pos);

	} else if (os_strcmp(buf, "sConfigMRCoexActiveTime") == 0) {
		conf->sConfigMRCoexActiveTime = atoi(pos);

	} else if (os_strcmp(buf, "sConfigMRCoexInactiveTime") == 0) {
		conf->sConfigMRCoexInactiveTime = atoi(pos);
		...
		if ( hostapd_parse_colocated_6g_vap_info(conf, bss, pos) )
			return 1;
                ...
                if ( hostapd_parse_unsolicited_frame_support(bss, pos) )
                        return 1;
	 } else if (os_strcmp(buf, "unsolicited_frame_duration") == 0) {
                if ( hostapd_parse_unsolicited_frame_duration(bss, pos) )
                        return 1;
	} else {
		wpa_printf(MSG_ERROR,
