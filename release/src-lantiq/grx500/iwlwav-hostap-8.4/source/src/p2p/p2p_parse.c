			       ...
			       struct p2p_message *msg)
{
	...
	case P2P_ATTR_CAPABILITY:
		if (len != 2) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Capability "
	...
	case P2P_ATTR_DEVICE_ID:
		if (len != ETH_ALEN) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Device ID "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->device_id = data;
	...
	case P2P_ATTR_GROUP_OWNER_INTENT:
		if (len != 1) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid GO Intent "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->go_intent = data;
	...
	case P2P_ATTR_STATUS:
		if (len != 1) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Status "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->status = data;
	...
	case P2P_ATTR_LISTEN_CHANNEL:
		...
		if (len != 5) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Listen Channel "
				   "attribute (length %d)", len);
			return -1;
		}
	...
	case P2P_ATTR_OPERATING_CHANNEL:
		...
		if (len != 5) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Operating "
	...
	case P2P_ATTR_CONFIGURATION_TIMEOUT:
		if (len != 2) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Configuration "
	...
	case P2P_ATTR_INTENDED_INTERFACE_ADDR:
		if (len != ETH_ALEN) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Intended P2P "
	...
	case P2P_ATTR_GROUP_BSSID:
		if (len != ETH_ALEN) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid P2P Group BSSID "
				   "attribute (length %d)", len);
			return -1;
		}
	...
	case P2P_ATTR_INVITATION_FLAGS:
		if (len != 1) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Invitation "
	...
	case P2P_ATTR_MANAGEABILITY:
		if (len != 1) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Manageability "
				   "attribute (length %d)", len);
			return -1;
		}
	...
	case P2P_ATTR_EXT_LISTEN_TIMING:
		if (len != 4) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Extended Listen "
	...
	case P2P_ATTR_MINOR_REASON_CODE:
		if (len != 1) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid Minor Reason "
	...
	case P2P_ATTR_OOB_GO_NEG_CHANNEL:
		if (len != 6) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid OOB GO Neg "
				   "Channel attribute (length %d)", len);
			return -1;
		}
	...
	case P2P_ATTR_CONNECTION_CAPABILITY:
		if (len != 1) {
			wpa_printf(MSG_DEBUG,
				   "P2P: Invalid Connection Capability (length %u)",
				   len);
			return -1;
		}
		msg->conn_cap = data;
	...
	case P2P_ATTR_ADVERTISEMENT_ID:
		if (len != 10) {
			wpa_printf(MSG_DEBUG,
				   "P2P: Invalid Advertisement ID (length %u)",
				   len);
			return -1;
		}
		msg->adv_id = data;
	...
	case P2P_ATTR_SESSION_ID:
		if (len != sizeof(u32) + ETH_ALEN) {
			wpa_printf(MSG_DEBUG,
				   "P2P: Invalid Session ID Info (length %u)",
				   len);
			return -1;
		}
		msg->session_id = data;
