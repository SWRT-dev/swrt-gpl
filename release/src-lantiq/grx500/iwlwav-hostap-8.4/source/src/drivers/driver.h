...
struct update_rnr
{
	...
	Boolean multibss_enable;
	Boolean is_transmitted_bssid;
	Boolean unsolicited_frame_support;
};
  ...
  int (*set_antenna)(void *priv, u32 tx_ant, u32 rx_ant);
	...
	*/
	...
	int (*set_chan_dfs_state)(void *priv,enum nl80211_dfs_state dfs_state,
								int freq, int dfs_time);
	/**
	 * set_unsolicited_frame - start transmitting unsolicited frame
	 * @priv: Private driver interface data
	 * @enable: Flag indicating enable/disable unsolicited frame tx
