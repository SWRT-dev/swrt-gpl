			...
			if (channel->flag & HOSTAPD_CHAN_RADAR) {
				if (((channel->flag & HOSTAPD_CHAN_DFS_MASK) == HOSTAPD_CHAN_DFS_AVAILABLE) &&
					...
					hostapd_set_chan_dfs_state(iface,
								NL80211_DFS_AVAILABLE, channel->freq, 0);
				}
				if ((channel->flag & HOSTAPD_CHAN_DFS_MASK) == HOSTAPD_CHAN_DFS_UNAVAILABLE) {
					hostapd_set_chan_dfs_state(iface,
								NL80211_DFS_UNAVAILABLE, channel->freq, channel->nop_time_passed_min);
					update = 1;
				}
			}
		}
			     ...
			     int bandwidth, int secondary_offset)
{
	...
	if (switch_to_full) {
		...
		if (NL80211_SB_DFS_BW_80 == csa_settings.sb_dfs_bw)
			iface->conf->vht_oper_chwidth = vht_oper_chwidth = CHANWIDTH_80MHZ;
		else
			iface->conf->vht_oper_chwidth = vht_oper_chwidth = CHANWIDTH_USE_HT;
