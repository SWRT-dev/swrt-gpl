...
static void hostapd_post_up_vendor_vap_cmd(struct hostapd_data *hapd, struct hostapd_bss_config *conf)
{
	...
	if (hapd->conf->unsolicited_frame_support) {
		if (hostapd_drv_set_unsolicited_frame(hapd, hapd->conf->unsolicited_frame_support,
						      hapd->conf->unsolicited_frame_duration) < 0 )
			wpa_printf(MSG_WARNING, "Unsolicited frame config failed");
		else
...
#define HE_OPER_VHT_OPER_INFO_SIZE 3
	...
	if (!he_oper_bss_color) {
		...
		} else if ((hapd->iface->conf->hw_mode == HOSTAPD_MODE_IEEE80211G) || (hapd->iface->conf->hw_mode == HOSTAPD_MODE_IEEE80211B)) {
			he_oper_bss_color = HE_OPERATION_BSS_COLOR_2_4GHZ;
		}
	}

	os_memset(he_operation_hw, 0, sizeof(he_operation_hw));
	if(hostapd_drv_get_he_operation(hapd, (void*)&he_operation_hw) < 0) {
		...
		he_operation_hw[HE_OPERATION_CAP3_IDX] = 0;
    if(he_oper_bss_color_disable_flag) {
      ...
      he_operation_hw[HE_OPERATION_CAP3_IDX] &= (~HE_OPERATION_BSS_COLOR_DISABLED);
		...
		he_operation_hw[HE_OPERATION_CAP3_IDX] |= (he_oper_bss_color & HE_OPERATION_BSS_COLOR);
...
static void wpa_msg_ap_state(struct hostapd_data *hapd, const char* event_str)
{
	if ((hapd->conf->mesh_mode == MESH_MODE_BACKHAUL_AP ||
		...
		size_t size = (os_strlen(event_str) +
		...
		written += os_snprintf(msg, size, "%s", event_str);
		...
		for (i = 0; i < hapd->num_bkh_vrt_ifaces; i++) {
			...
			if (!has_bkhs)
				written += os_snprintf(msg + written,
						       size - written,
						       "bAP_ifaces=");
			has_bkhs = 1;
...
int hostapd_activate_backhaul_ifaces(struct hostapd_data *hapd)
{
	...
	if (num_vrt_bkh_ifaces > UINT8_MAX) {
		wpa_printf(MSG_ERROR, "%s: num_vrt_bkh_ifaces is bigger than UINT8_MAX",  __func__);
		return -1;
	}

	if (strnlen(hapd->conf->iface, IFNAMSIZ) > TRUNCATE_BKH_VRT_IFACE) {
	...
	for (i = 0; i < num_vrt_bkh_ifaces; ++i) {
		...
		ret = os_snprintf(bkh_vrt_iface->ifname,
			    sizeof(bkh_vrt_iface->ifname),
			    "b%hhu_%.*s", i, TRUNCATE_BKH_VRT_IFACE, hapd->conf->iface);
		if (os_snprintf_error(sizeof(bkh_vrt_iface->ifname), ret)) {
			wpa_printf(MSG_ERROR, "%s: os_snprintf error %d", __func__, ret);
						 ...
						 int err)
{
	...
	if (iface->conf->sConfigMRCoex != -1){
		int sConfigMRCoex[3] = {iface->conf->sConfigMRCoex, -1, -1};
	...
	if(iface->conf->sCoCPower){
		/* if we are in manual mode , check that we do not set zeros*/
		if ((iface->conf->sCoCPower[0] == 0) && (iface->conf->sCoCPower[1] || iface->conf->sCoCPower[2])){
