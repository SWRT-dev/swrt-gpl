...
static void create_rnr_vap(u8 **pos, int radio, struct hostapd_data *hapd, Boolean is_same_ssid)
{
	u8 empty_bssid[ETH_ALEN] = {'\0'};
	size_t j;
...
static void create_rnr(u8 **pos, int radio, struct hostapd_data *hapd, int count, Boolean is_same_ssid)
{
	...
	*pos++ = hapd->conf->ssid.ssid_len;
	...
	if (hapd->iconf->ieee80211ax && !hapd->conf->disable_11ax) {
		...
		if(get_he_cap(hapd->iface->conf->he_oper.he_oper_params[HE_OPERATION_CAP2_IDX],
			...
			pos = hostapd_eid_he_6ghz_band_capabilities(hapd,pos);
	}
	}
	...
	if (short_ssid_list) {
		...
		while (end - pos >= 4) {
			...
			{ 
				...
				{ 
					if (!os_memcmp( pos,
					c[i].bss_info[j].ShortSsid,
					sizeof(c[i].bss_info[j].ShortSsid) ) )
						return EXACT_SSID_MATCH;
				}
			}
			pos += 4;
	...
	*pos++ = WLAN_EID_SSID;
    ...
    if (hapd->iconf->ieee80211ax && !hapd->conf->disable_11ax) {
  		...
  		if(get_he_cap(hapd->iconf->he_capab.he_mac_capab_info[HE_MACCAP_CAP4_IDX],
			...
			if(get_he_cap(hapd->iface->conf->he_oper.he_oper_params[HE_OPERATION_CAP2_IDX],
                                                        HE_OPERATION_CAP2_6GHZ_OPERATION_INFO_PRESENT))
				tailpos = hostapd_eid_he_6ghz_band_capabilities(hapd,tailpos);
    }
#endif /* CONFIG_IEEE80211AX */
