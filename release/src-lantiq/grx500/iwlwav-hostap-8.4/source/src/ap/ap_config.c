...
void hostapd_config_defaults_bss(struct hostapd_bss_config *bss)
{
	...
	bss->unsolicited_frame_support = 0;
	bss->unsolicited_frame_duration = 20;
...
#undef ecw2cw
...
#ifdef CONFIG_ACS
  {
	...
	int sCoCPower[] = {0, 0, 0};/* this is only for manual mode (zeros will not be sets), auto mode is handled by driver*/
	int sRTSmode[] = {0, 0};
	...
	if (sCoCPower[0] || sCoCPower[1] || sCoCPower[2])
			wpa_printf(MSG_WARNING, "set sCoCPower init value is set by hostapd: sCoCPower %d %d %d", sCoCPower[0] ,sCoCPower[1] ,sCoCPower[2]);
