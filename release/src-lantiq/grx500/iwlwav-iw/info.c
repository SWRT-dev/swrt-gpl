...
static int print_phy_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb_msg[NL80211_ATTR_WIPHY_BANDS]) {
		nla_for_each_nested(nl_band, tb_msg[NL80211_ATTR_WIPHY_BANDS], rem_band) {
			...
			if (tb_band[NL80211_BAND_ATTR_HT_TX_BF_CAPA]) {
				print_ht_tx_bf(nla_get_u32(tb_band[NL80211_BAND_ATTR_HT_TX_BF_CAPA]));
			}
		       ...
		       enum id_input id)
{
	...
	err = handle_cmd(state, (int)CIB_NONE, 2, feat_args);
