...
static const _regd_country2domain_t country_regdcode_table[] =
{
    ...
    {"CB", REGD_CODE_DOC      }, /* Canada High Power */
    {"CC", REGD_CODE_MKK      },
    {"CD", REGD_CODE_MKK      },
    {"CF", REGD_CODE_ETSI     },
    {"CG", REGD_CODE_ETSI     },
...
psdb_parse_regd_table (hw_psdb_t *psdb, psdb_any_table_t *regd_table)
{
    ...
    for (i = 0; i < nof_entries; i++) {
        ...
        if (error) {
            ELOG_DDD("Illegal chan num %d (%d MHz) idx %d", chan_num, chan_freq, idx);
            goto ERROR;
        }
...
_mtlk_psdb_parse_cca_th_table (hw_psdb_t *psdb, psdb_any_table_t *table)
{
    ...
    res = _mtlk_psdb_check_table_sizes("CcaTh configuration",
              table, MTLK_CCA_TH_PARAMS_LEN, (MTLK_HW_BAND_MAX-1));
    if (MTLK_ERR_OK != res) {
      return res;
    }
    ...
    for (i = 0; i < MTLK_CCA_TH_PARAMS_LEN; i++) {
      ...
      cdb_table->cca_th_params[MTLK_HW_BAND_6_GHZ][i] = (int8)table->data.cca_th_entries[i].value_6G;

      ILOG2_DDDDDDD("CCA_TH[%d]: 0x%08X -> %d,  0x%08X -> %d,  0x%08X -> %d", i,
          ...
          table->data.cca_th_entries[i].value_6G, cdb_table->cca_th_params[MTLK_HW_BAND_6_GHZ][i]);
    }
...
/* 6GHz channel limits (up to 6.4GHz) */
#define WAVE_CHAN_6G_LOWER      1
#define WAVE_CHAN_6G_UPPER    109  /* 117 in the next phase */
...
static struct cdb_cfg_radio_params {
  ...
  [WAVE_CDB_CFG_6G ]     = { MTLK_HW_BAND_6_GHZ  , WAVE_CHAN_6G_LOWER,   WAVE_CHAN_6G_UPPER   }, /* 6GHz support */
  ...
  if (wave_cdb_cfg_is_not_set(radio_cfg->cdb_config)) {
    ...
    if ((!cdb_table->band_support_2G) && (!cdb_table->band_support_5G) && (!cdb_table->band_support_6G)) {
      ...
      if (cdb_table->band_support_6G) {
        radio_cfg = &cdb_table->radio_cfg[idx++];
        radio_cfg->cdb_config = WAVE_CDB_CFG_6G;
      }
    }
...
wave_psdb_get_hw_radio_band_cfg (hw_psdb_t *psdb)
{
  ...
  if (ant_num_radio_0 == 2 && ant_num_radio_1 == 2)
