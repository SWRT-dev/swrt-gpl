...
_wave_scan_support_find_6G_chan_bonding (unsigned chan) {
  ...
  for (i = 0; i < ARRAY_SIZE(_chan_6G_bonding_table); i++) {
...
set_chan:
  ...
  if (ss->flags & SDF_BACKGROUND)
  {
    ...
    if (++ss->chan_in_chunk > ss->NumChansInChunk)
    {
      ...
      mtlk_hw_reset_channel_rssi(mtlk_vap_get_hw(core->vap_handle), channum2cssidx(ieee80211_frequency_to_channel(ss->orig_chandef.chan.center_freq)));
  ...
  mtlk_hw_reset_channel_rssi(mtlk_vap_get_hw(core->vap_handle), channum2cssidx(ieee80211_frequency_to_channel(req->channels[i].center_freq)));
...

  {
    ...
    if (req->type == MTLK_SCAN_STA_ROC) {
      timeout_val = req->duration;
...
int _scan_do_scan(mtlk_core_t *core, struct mtlk_scan_request *request)
{
  ...
  if (request->type == MTLK_SCAN_AP || request->type == MTLK_SCAN_SCHED_AP || request->type == MTLK_SCAN_STA_ROC)
  {
  ...
  {
    struct cfg80211_chan_def orig_chandef;
    ...
    orig_chandef.sb_dfs_bw = orig_mtlk_chandef->sb_dfs.sb_dfs_bw;
    ...
    if ((mtlk_core_is_band_supported(core, UMI_PHY_TYPE_BAND_5_2_GHZ) == MTLK_ERR_OK)
      ...
      ss->ChanChunkInterval = ss->iwpriv_scan_params_bg.chan_chunk_interval_busy;
    else
