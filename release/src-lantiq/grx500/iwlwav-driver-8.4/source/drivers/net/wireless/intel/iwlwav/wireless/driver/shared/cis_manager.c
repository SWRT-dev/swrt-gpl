...
_parse_tpc_g6 (cis_v6_tx_common_t *tpc, int data_size,
               mtlk_eeprom_cis_data_t *parsed_cis, unsigned *tpc_ant_mask, uint8 revision)
{
  ...
  while (data_size >= tpc_struct_size) {
    ...
    /* TODO:6GHz: Need to review if the revision check is necessary */
    if ((revision == HW_VERS_600_GPB624_5G) && (chan_6g)) {
    ...
    for(idx = 0; idx < EEPROM_GEN6_MAX_ANT; idx++) {
      ...
      mtlk_dump(2, &data->tpc_chan_data, sizeof(data->tpc_chan_data), "");
...
static int
_mtlk_cis_area_parse_g6 (mtlk_eeprom_cis_data_t *parsed_cis, void *cis, void *cis_end, uint8 revision)
{
  ...
  while(tpc) {
    ...
    if (MTLK_ERR_OK != _parse_tpc_g6(tpc, data_size, parsed_cis, &tpc_ant_mask, revision)) {
      goto ERROR;
    }
                     ...
                     mtlk_eeprom_cis_data_t *parsed_cis)
{
  ...
  parsed_cis->band6g_supported = FALSE;
    ...
    res = _mtlk_cis_area_parse_g6(parsed_cis, raw_cis, cis_end, card_id->revision);
