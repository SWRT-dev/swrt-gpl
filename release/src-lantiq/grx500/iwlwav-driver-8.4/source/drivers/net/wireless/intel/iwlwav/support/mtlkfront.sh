...
abs_builddir="$(dirname $FILE_NAME)"
...
if logger_preprocessing_enabled && \
  ...
  for file in $abs_top/logmacros.h $abs_top/logmacros.c $abs_top/loggroups.h; do
    ...
    if test -e $file; then
	    if test ! -e $TIMESTAMP_FILE -o $file -nt $TIMESTAMP_FILE; then
	      ...
	      perl $abs_tools_srcdir/rtlogger/logprep/logprep.pl gcc \
	                                                         ...
	                                                         $abs_top/loggroups.h $abs_top/logmacros.h \
	                                                         $abs_top/logmacros.c $abs_builddir/$logger_origin_name.scd $logger_binary_name \
	                                                         < $file || { echo Logprep error while processing file $file && exit 1; }
	      #Do not rebuild all sources because of loggger database changes
	      touch -m -t 198001010000.00 $abs_top/loggroups.h $abs_top/logmacros.c $abs_top/logmacros.h
	      #Make sure logmacro_database.o will rebuild because it could be that new macros were added
	      find $abs_top -name logmacro_database.c | xargs touch 2>/dev/null
