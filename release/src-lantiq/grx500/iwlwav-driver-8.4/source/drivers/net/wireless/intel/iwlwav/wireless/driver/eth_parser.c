...
void mtlk_eth_parser (void *data, int length, const char *if_name, const char *f)
{
  ...
  /* IPv6 */
    ...
    struct in6_addr ip_src_addr,ip_dst_addr;
    ...
    ip_src_addr.in6_u.u6_addr32[0] = ntohl(iph->saddr.in6_u.u6_addr32[0]);
    ip_src_addr.in6_u.u6_addr32[1] = ntohl(iph->saddr.in6_u.u6_addr32[1]);
    ip_src_addr.in6_u.u6_addr32[2] = ntohl(iph->saddr.in6_u.u6_addr32[2]);
    ip_src_addr.in6_u.u6_addr32[3] = ntohl(iph->saddr.in6_u.u6_addr32[3]);

    ip_dst_addr.in6_u.u6_addr32[0] = ntohl(iph->daddr.in6_u.u6_addr32[0]);
    ip_dst_addr.in6_u.u6_addr32[1] = ntohl(iph->daddr.in6_u.u6_addr32[1]);
    ip_dst_addr.in6_u.u6_addr32[2] = ntohl(iph->daddr.in6_u.u6_addr32[2]);
    ip_dst_addr.in6_u.u6_addr32[3] = ntohl(iph->daddr.in6_u.u6_addr32[3]);

    ILOG2_SSYYKK("%s: %s SRC:%Y DST:%Y IPv6 src_ip %K dst_ip %K", if_name, f, ether_header->h_source,
                  ether_header->h_dest, ip_src_addr.s6_addr, ip_dst_addr.s6_addr);
  }
  /* All Others */
  else {
