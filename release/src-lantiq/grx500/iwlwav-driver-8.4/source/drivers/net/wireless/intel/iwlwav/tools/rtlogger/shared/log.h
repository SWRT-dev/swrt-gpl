...
*/
#define IWLWAV_RTLOG_SILENT_DLEVEL  (-3)
#define IWLWAV_RTLOG_ERROR_DLEVEL   (-2)
#define IWLWAV_RTLOG_WARNING_DLEVEL (-1)
...
extern mtlk_log_buf_entry_t *logReserveBuffer(uint32 len);
extern void logFillLogEvent(uint32 oid, uint32 gid, uint32 fid, uint32 lid, uint32 len, uint8 **buff_p);
extern void logReleaseBuffer(mtlk_log_buf_entry_t *p, uint32 len);
