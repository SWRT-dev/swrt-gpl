...
/* FIXME: extern prototypes should be moved from here */
...
void __MTLK_IFUNC mtlk_ndev_send_log_packet(void *data, size_t data_len); /* FIXME: prototype */
...
rtlog_netdev_init(void)
{
    ...
    mtlk_osal_emergency_print("Allocated netdev %s addr %px", dev_name(&rtlog_netdev->dev), rtlog_netdev);
