...
mtlk_coc_update_antenna_cfg (mtlk_coc_t *coc_obj, uint8 and_mask, BOOL *update_hw)
{
  ...
  if (!cur_ant_count || cur_ant_count >= COC_POWER_STATE_LAST) {
...
_mtlk_coc_auto_mode_process (mtlk_coc_t *coc_obj, mtlk_coc_crit_t *coc_crit)
{
  ...
  switch (current_state) {
    ...
    case COC_POWER_1x1_STATE:
      nof_ants_tp = 1;
     if ((max_antennas >= 2) && (max_tp >= coc_obj->auto_antenna_cfg.low_limit_2x2)) {
        nof_ants_tp = 2;
      }
      break;
    case COC_POWER_2x2_STATE:
    ...
    case COC_POWER_4x4_STATE:
      nof_ants_tp = 4;
      if (max_tp < coc_obj->auto_antenna_cfg.high_limit_3x3) {
        nof_ants_tp = 3;
      }
      break;
    default:
  ...
  /* 2. RSSI processing */
  rssi = coc_crit->rssi;
  if (rssi < coc_obj->auto_antenna_cfg.low_rssi_4x4) {
    nof_ants_rssi = 4;
  } else if (rssi < coc_obj->auto_antenna_cfg.low_rssi_3x3) {
    nof_ants_rssi = 3;
  } else if (rssi < coc_obj->auto_antenna_cfg.low_rssi_2x2) {
    nof_ants_rssi = 2;
  } else {
    nof_ants_rssi = 1;
  }
...
#if 0 /* TODO: align with FW */
  _mtlk_coc_switch_mode(coc_obj, 1, 1);
#endif
...
mtlk_coc_set_auto_params (mtlk_coc_t *coc_obj, const mtlk_coc_auto_cfg_t *in_auto_params)
{
  ...
  else if ((auto_params->low_rssi_2x2 <= auto_params->low_rssi_3x3) ||
           (auto_params->low_rssi_3x3 <= auto_params->low_rssi_4x4)) {
    ELOG_DDDD("CID-%04x: Wrong CoC auto configuration: RSSI values (%d > %d > %d)",
              oid, auto_params->low_rssi_2x2, auto_params->low_rssi_3x3, auto_params->low_rssi_4x4);
  }
  else {
    mtlk_osal_lock_acquire(&coc_obj->lock);
