...
CHAN6G(101,6445),
CHAN6G(105,6465),
CHAN6G(109,6485)/*, 6.4 GHz band end
CHAN6G(113,6505),
CHAN6G(117,6525),
CHAN6G(121,6545),
CHAN6G(125,6565),
...
struct wv_vif_priv * __MTLK_IFUNC
...
_wv_ieee80211_op_config (struct ieee80211_hw *hw, u32 changed)
{
 ...
 char *band6g = "6Ghz";
  ...
  switch ((enum nl80211_band)channel->band) {
                                     ...
                                     struct ieee80211_channel_switch *ch_switch)
{
  ...
  ILOG0_SSDDDDD("%s: Ch switch settings: band=%s, freq=%d, "
  ...
  cpd.radar_required = _wv_ieee80211_is_radar_chan(channel);
    ...
    struct ieee80211_channel_switch *ch_switch)
{
  ...
  ILOG0_SSDDD("%s: Sta vif got CSA from AP. CSA settings: band=%s, freq=%d, "
...
static mtlk_error_t
_wv_request_sid_if_needed (struct ieee80211_vif *vif, struct ieee80211_sta *sta)
{
...
wv_ieee80211_peer_ap_address (struct ieee80211_vif *vif)
{
  struct wv_vif_priv *wv_iface_inf = (struct wv_vif_priv*)vif->drv_priv;
  if (!wv_iface_inf) {
    ELOG_S("%s: Error getting peer_ap_address", ieee80211_vif_to_name(vif));
    return NULL;
  }
...
static void _wv_ieee80211_op_remove_interface (struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
  ...
  if (mtlk_vap_is_sta(_vap_to_be_removed)) {
    ...
    if (wv_iface_inf->is_set_sid) {
      res = _wv_release_sid(wv_iface_inf->df_user, wv_iface_inf->vif_sid);
      if (MTLK_ERR_OK != res) {
    ...
    enum ieee80211_sta_state new_state)
{
  ...
  if (old_state == IEEE80211_STA_NOTEXIST && new_state == IEEE80211_STA_NONE) {
     ...
     */
    if (mtlk_vap_is_sta(vap_handle)) {
      ret = _wv_request_sid_if_needed(vif, sta);
      if (MTLK_ERR_OK != ret) {
...
static void _wv_ieee80211_set_default_key (struct ieee80211_hw *hw, struct ieee80211_vif *vif, int idx)
{
  ...
  if (idx >= WAVE_WEP_NUM_DEFAULT_KEYS) {
    ...
    struct ieee80211_vif *vif)
{
  ...
  ILOG2_SY("%s: Peer ap MAC = %Y", ieee80211_vif_to_name(vif), wv_iface_inf->peer_ap_addr);
...
static void
_set_rx_antenna_pattern (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
...
static void
_set_ldpc_cap (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
...
static void
_set_ampdu_factor (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
...
static void
_set_ampdu_density (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
...
static void
_set_rx_stbc (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
...
static void
_set_vht_support (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
...
static void
_set_vht_160mhz_short_gi (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
...
static void
_set_vht_cap_160mhz (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
  BOOL vht_160_mhz = hw_get_vht_160mhz_support(hw_handle);
...
static void
_set_he_support (mtlk_handle_t hw_handle, struct ieee80211_supported_band *cfg80211_band, char *band_str)
{
  ...
  char*         band_str)   /* To give context in log messages */
{
  ...
  *supported_band = mtlk_osal_mem_alloc(size, MTLK_MEM_TAG_MAC80211);
  ...
  _set_ldpc_cap            (hw_handle, *supported_band, band_str);
  _set_ampdu_factor        (hw_handle, *supported_band, band_str);
  _set_ampdu_density       (hw_handle, *supported_band, band_str);
  _set_rx_stbc             (hw_handle, *supported_band, band_str);
  _set_rx_antenna_pattern  (hw_handle, *supported_band, band_str);
  _set_vht_support         (hw_handle, *supported_band, band_str);
  _set_vht_cap_160mhz      (hw_handle, *supported_band, band_str);
  _set_vht_160mhz_short_gi (hw_handle, *supported_band, band_str);
  _set_he_support          (hw_handle, *supported_band, band_str);
    ...
    unsigned radio_id, struct wiphy *wiphy, u32 num_sts)
{
  ...
  if (mtlk_is_band_supported(hw_handle, radio_id, nlband2mtlkband(NL80211_BAND_6GHZ))) {
    int ret_code = _wv_mac80211_init_band(hw_handle, &wiphy->bands[NL80211_BAND_6GHZ], &_supported_band_6ghz,
                       _6ghz_channels, ARRAY_SIZE(_6ghz_channels), num_sts, "6 GHz");
    if (ret_code != MTLK_ERR_OK)
      return ret_code;
...
void __MTLK_IFUNC
wv_ieee80211_radar_detected (struct ieee80211_hw *hw,
                             struct cfg80211_chan_def *chandef, uint8 rbm)
{
    ...
    enum ieee80211_roc_type type)
{
  ...
  ILOG1_SSDD("roc_type=%s, band=%s, freq=%d, duration=%i",
      type == IEEE80211_ROC_TYPE_NORMAL ? "ROC_TYPE_NORMAL" : "ROC_TYPE_MGMT_TX",
      (enum nl80211_band)chan->band == NL80211_BAND_2GHZ ? "2Ghz" : "5Ghz",
  ...
  sr->type = MTLK_SCAN_STA_ROC;
  sr->requester_vap_index = wv_iface_inf->vap_index;
  sr->wiphy = hw->wiphy;
  sr->n_channels = 1; /* only one chnannel */
  sr->duration = WAVE_RADIO_PDB_GET_INT(radio, PARAM_DB_RADIO_CTS_TO_SELF_TO);
...
static int _wv_ieee80211_cancel_remain_on_channel(struct ieee80211_hw *hw)
{
  int res;
  mtlk_clpb_t *clpb = NULL;
