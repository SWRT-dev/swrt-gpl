...
#define CHAN_IDX_ILLEGAL_6G (NUM_TOTAL_CHAN_FREQS_6G)
 /*-------------------------------------------------*/
...
/* Conversion from channel number to our channel-scan-support index for 6GHz band only*/
/* chan num = (2*index)+1 */
...
unsigned channum6g2chanidx(int num)
{
  ...
  if ((num < 0) || (num > WAVE_LAST_6G_CHAN)) {
    return CHAN_IDX_ILLEGAL_6G;
  }
  idx = (num - 1) >> 1;

  return (NUM_TOTAL_CHAN_FREQS_6G < idx) ? CHAN_IDX_ILLEGAL_6G : idx;
}
