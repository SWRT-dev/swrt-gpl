...
mtlk_vap_connect_backhaul_sta(mtlk_vap_handle_t vap_handle, sta_entry *sta)
{
  ...
  for (_vap_index = 0; _vap_index < obj->max_vaps_count; _vap_index++) {
    ...
    if (NULL != mtlk_df_user_get_backhaul_sta(df_user))
      continue;
...
void __MTLK_IFUNC
mtlk_vap_diconnect_backhaul_sta(mtlk_vap_handle_t vap_handle, sta_entry *sta)
{
  mtlk_vap_info_internal_t *vap_info;
  mtlk_df_user_t *df_user;
  ...
  if (mtlk_df_user_get_backhaul_sta(df_user) != sta) {
  ...
  sta->secondary_vap_id = vap_info->id;
  mtlk_sta_decref(sta); /* De-reference held as backhaul station */
}
