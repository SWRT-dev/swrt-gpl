...
/* IPv4*/
static int                  parse_igmp  (mtlk_core_t *nic, struct igmphdr *igmp_header, const IEEE_ADDR *sta_addr)    __MTLK_INT_HANDLER_SECTION;
static void                 _mtlk_mc_change_to_exclude_ip4  (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr);
static void                 _mtlk_mc_change_to_include_ip4  (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr, int src_num);
static void                 _mtlk_mc_block_old_sources_ip4  (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr, int src_num);
static void                 _mtlk_mc_allow_new_sources_ip4  (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr, int src_num);
static void                 _mtlk_mc_add_ip4_sta            (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr);
static void                 _mtlk_mc_del_ip4_sta            (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr);

static mcast_ip4_grp_t *    _mtlk_mc_add_sta_to_group_unlocked_ip4   (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr, mcast_mode_t mode);
static mcast_ip4_grp_t *    _mtlk_mc_del_sta_from_group_unlocked_ip4 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr);
...
/* IPv6 */
static int                  parse_icmp6 (mtlk_core_t *nic, struct icmp6hdr *icmp6_header, uint32 length, const IEEE_ADDR *sta_addr)  __MTLK_INT_HANDLER_SECTION;
static void                 _mtlk_mc_change_to_include_ip6  (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, int src_num);
static void                 _mtlk_mc_block_old_sources_ip6  (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, int src_num);
static void                 _mtlk_mc_allow_new_sources_ip6  (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, int src_num);
static void                 _mtlk_mc_add_ip6_sta            (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, BOOL allow_link_local);
static void                 _mtlk_mc_del_ip6_sta            (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr);

static mcast_ip6_grp_t *    _mtlk_mc_add_sta_to_group_unlocked_ip6   (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, mcast_mode_t mode);
static mcast_ip6_grp_t *    _mtlk_mc_del_sta_from_group_unlocked_ip6 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr);
...
__mtlk_mc_find_mcsta (mcast_sta_t *root, const IEEE_ADDR *sta_addr)
{
  mcast_sta_t *item;
  MTLK_LIST_FIND_ITEM(item, root, !(ieee_addr_compare(&item->sta_addr, sta_addr)));
  return item;
}

static __INLINE int
__mtlk_mc_del_mcsta (mcast_sta_t **root, const IEEE_ADDR *sta_addr)
{
  mcast_sta_t *item, *last_item;
  MTLK_LIST_DEL_ITEM(item, last_item, *root, !(ieee_addr_compare(&item->sta_addr, sta_addr)), {});
}

static int
__mtlk_mc_del_mcsta_by_mac (mcast_sta_t **root, const unsigned char *mac)
{
  mcast_sta_t *item, *last_item;
  MTLK_LIST_DEL_ITEM(item, last_item, *root,
      (0==mtlk_osal_compare_eth_addresses(mac, item->sta_addr.au8Addr)), {});
}

static __INLINE mcast_sta_t*
__mtlk_mc_add_mcsta (mcast_sta_t **root, const IEEE_ADDR *sta_addr)
{
...
static BOOL
_mtlk_mc_add_new_mcsta(mcast_sta_t **root, const IEEE_ADDR *sta_addr)
{
  if (!__mtlk_mc_find_mcsta(*root, sta_addr)) {
    __mtlk_mc_add_mcsta(root, sta_addr);
    return TRUE;
  }
  return FALSE;
}
...
mtlk_mc_parse (mtlk_core_t* core, mtlk_nbuf_t *nbuf, const IEEE_ADDR *sta_addr)
{
  ...
  ILOG3_DDD("CID-%04x: ETH proto_type:%04x VLAN len:%d", mtlk_vap_get_oid(core->vap_handle), type, hdr_offset);
  ...
  case ETH_P_IP:
    {
      ...
      if (nbuf_len < hdr_offset + sizeof(*ip_header)) {
        ILOG3_D("CID-%04x: Frame length is wrong", mtlk_vap_get_oid(core->vap_handle));
        return MTLK_ERR_NOT_IN_USE;
      }
      ...
      if (ip_header->protocol == IPPROTO_IGMP) {
        res = parse_igmp(core, igmp_header, sta_addr);
      }
      break;
    }
  case ETH_P_IPV6:
    {
      ...
      if (
        ...
        res = parse_icmp6(core, icmp6_header, nbuf_len - ((wave_addr)icmp6_header - (wave_addr)nbuf->data), sta_addr);
      break;
    }
  default:
    break;
...
void __MTLK_IFUNC
mtlk_mc_add_sta (mtlk_core_t *nic, IEEE_ADDR *sta_addr)
{
  ...
  _mtlk_mc_add_ip6_sta(nic, sta_addr, &ip_addr, TRUE);
}
  ...
  for (i = 0; i < MCAST_HASH_SIZE; i++) {
    ...
    for (grp = nic->mcast.mcast_ip4_hash[i]; grp != NULL;) {
      ...
      for (src = grp->ip4_src_list; src != NULL;) {
        ...
        __mtlk_mc_del_mcsta_by_mac(&src->info.mcsta, mac);
      ...
      __mtlk_mc_del_mcsta_by_mac(&grp->info.mcsta, mac);
...
static mcast_ip4_grp_t *
_mtlk_mc_add_sta_to_group_unlocked_ip4 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr, mcast_mode_t mode)
{
  mcast_ip4_grp_t *grp;
  mtlk_mc_addr_t grp_mc_addr;
  ...
  if (_mtlk_mc_add_new_mcsta(&grp->info.mcsta, sta_addr)) {
...
static mcast_ip4_grp_t *
_mtlk_mc_del_sta_from_group_unlocked_ip4 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr)
{
  mcast_ip4_grp_t *grp;
  mtlk_mc_addr_t grp_mc_addr;
  mcast_ctx_t *mcast = &nic->mcast;
  ...
  if (!__mtlk_mc_del_mcsta(&grp->info.mcsta, sta_addr))
    goto end;
...
static void
_mtlk_mc_change_to_exclude_ip4 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr)
{
  ...
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  grp = _mtlk_mc_add_sta_to_group_unlocked_ip4(nic, sta_addr, grp_addr, MCAST_NO_MODE);
  if (grp) {
...
static void
_mtlk_mc_change_to_include_ip4 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr, int src_num)
{
  /* Simplified implementation.
  ...
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  if (src_num == 0)
    grp = _mtlk_mc_del_sta_from_group_unlocked_ip4(nic, sta_addr, grp_addr);
  else
    grp = _mtlk_mc_add_sta_to_group_unlocked_ip4(nic, sta_addr, grp_addr, MCAST_NO_MODE);
...
static void
_mtlk_mc_allow_new_sources_ip4 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr, int src_num)
{
  /* Simplified implementation.
  ...
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  _mtlk_mc_add_sta_to_group_unlocked_ip4(nic, sta_addr, grp_addr, MCAST_INCLUDE_MODE);
  mtlk_osal_lock_release(&nic->mcast.lock);
}


static void
_mtlk_mc_block_old_sources_ip4 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr, int src_num)
{
  /* Simplified implementation.
  ...
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  _mtlk_mc_add_sta_to_group_unlocked_ip4(nic, sta_addr, grp_addr, MCAST_EXCLUDE_MODE);
  mtlk_osal_lock_release(&nic->mcast.lock);
}

static void
_mtlk_mc_add_ip4_sta (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr)
{
  MTLK_ASSERT(FALSE == mtlk_core_mcast_module_is_available(nic));
  ...
  /* Add STA to group */
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  _mtlk_mc_add_sta_to_group_unlocked_ip4(nic, sta_addr, grp_addr, MCAST_NO_MODE);
  mtlk_osal_lock_release(&nic->mcast.lock);
}

static void
_mtlk_mc_del_ip4_sta (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in_addr *grp_addr)
{
  MTLK_ASSERT(FALSE == mtlk_core_mcast_module_is_available(nic));

  mtlk_osal_lock_acquire(&nic->mcast.lock);
  _mtlk_mc_del_sta_from_group_unlocked_ip4(nic, sta_addr, grp_addr);
  mtlk_osal_lock_release(&nic->mcast.lock);
}
...
static int
parse_igmp (mtlk_core_t *nic, struct igmphdr *igmp_header, const IEEE_ADDR *sta_addr)
{
  ...
  case IGMPV2_HOST_MEMBERSHIP_REPORT:
    ...
    _mtlk_mc_add_ip4_sta(nic, sta_addr, &grp_addr);
    break;
  case IGMP_HOST_LEAVE_MESSAGE:
    ILOG1_DY("Leave group report: IPv4 %B, sta %Y", htonl(grp_addr.s_addr), sta_addr->au8Addr);
    _mtlk_mc_del_ip4_sta(nic, sta_addr, &grp_addr);
    break;
  case IGMPV3_HOST_MEMBERSHIP_REPORT:
    ...
    for (i = 0; i < grp_num; i++) {
      ...
      case IGMPV3_CHANGE_TO_INCLUDE:
         ...
         * no sources included */
        _mtlk_mc_change_to_include_ip4(nic, sta_addr, &grp_addr, src_num);
        break;
      ...
      case IGMPV3_CHANGE_TO_EXCLUDE:
         ...
         * how much sources are excluded */
        _mtlk_mc_change_to_exclude_ip4(nic, sta_addr, &grp_addr);
        break;
      case IGMPV3_ALLOW_NEW_SOURCES:
        ILOG1_D(" --- Allow new sources, %d source(s)", src_num);
        _mtlk_mc_allow_new_sources_ip4(nic, sta_addr, &grp_addr, src_num);
        break;
      case IGMPV3_BLOCK_OLD_SOURCES:
        ILOG1_D(" --- Block old sources, %d source(s)", src_num);
        _mtlk_mc_block_old_sources_ip4(nic, sta_addr, &grp_addr, src_num);
        break;
...
static mcast_ip6_grp_t *
_mtlk_mc_add_sta_to_group_unlocked_ip6 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, mcast_mode_t mode)
{
  mcast_ip6_grp_t *grp;
  mtlk_mc_addr_t grp_mc_addr;
  mcast_ctx_t *mcast = &nic->mcast;
...
static mcast_ip6_grp_t *
_mtlk_mc_del_sta_from_group_unlocked_ip6 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr)
{
  mcast_ip6_grp_t *grp;
  mtlk_mc_addr_t grp_mc_addr;
  mcast_ctx_t *mcast = &nic->mcast;
...
static void
_mtlk_mc_change_to_exclude_ip6 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr)
{
  /* Simplified implementation.
   * Station is added to the multicast group no matter
   * how much sources are excluded */
  ...
  /* Add STA to group and change mode to EXCLUDE */
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  grp = _mtlk_mc_add_sta_to_group_unlocked_ip6(nic, sta_addr, grp_addr, MCAST_NO_MODE);
  if (grp) {
    /* mode is required for processing IGMPV3_ALLOW_NEW_SOURCES and IGMPV3_BLOCK_OLD_SOURCES */
    grp->info.mode = MCAST_EXCLUDE_MODE;
  }
...
static void
_mtlk_mc_change_to_include_ip6 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, int src_num)
{
  /* Simplified implementation.
   * Station is removed from the multicast group only if no sources included.
   * Otherwise station is added to the multicast group */
  ...
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  if (src_num == 0)
    grp = _mtlk_mc_del_sta_from_group_unlocked_ip6(nic, sta_addr, grp_addr);
  else
    grp = _mtlk_mc_add_sta_to_group_unlocked_ip6(nic, sta_addr, grp_addr, MCAST_NO_MODE);
...
static void
_mtlk_mc_allow_new_sources_ip6 (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, int src_num)
{
  /* Simplified implementation.
   * if mode == INCLUDE: Station added to the multicast group no matter
   * how much sources are in IGMPv3 message.
  ...
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  _mtlk_mc_add_sta_to_group_unlocked_ip6(nic, sta_addr, grp_addr, MCAST_INCLUDE_MODE);
  mtlk_osal_lock_release(&nic->mcast.lock);
}


static void
_mtlk_mc_block_old_sources_ip6(mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, int src_num)
{
  /* Simplified implementation.
   * if mode == EXCLUDE: Station added to the multicast group no matter
   * how much sources are in IGMPv3 message.
  ...
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  _mtlk_mc_add_sta_to_group_unlocked_ip6(nic, sta_addr, grp_addr, MCAST_EXCLUDE_MODE);
  mtlk_osal_lock_release(&nic->mcast.lock);
}

static void
_mtlk_mc_add_ip6_sta (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr, BOOL allow_link_local)
{
  MTLK_ASSERT(FALSE == mtlk_core_mcast_module_is_available(nic));

  /* Check link local multicast space FF02::/16 and source station ID */
  if ((!allow_link_local && ((grp_addr->in6_u.u6_addr32[0] & 0xFF0F0000) == 0xFF020000)))
    return;

  /* Add STA to group */
  mtlk_osal_lock_acquire(&nic->mcast.lock);
  _mtlk_mc_add_sta_to_group_unlocked_ip6(nic, sta_addr, grp_addr, MCAST_NO_MODE);
  mtlk_osal_lock_release(&nic->mcast.lock);
}

static void
_mtlk_mc_del_ip6_sta (mtlk_core_t *nic, const IEEE_ADDR *sta_addr, const struct in6_addr *grp_addr)
{
  MTLK_ASSERT(FALSE == mtlk_core_mcast_module_is_available(nic));

  mtlk_osal_lock_acquire(&nic->mcast.lock);
  _mtlk_mc_del_sta_from_group_unlocked_ip6(nic, sta_addr, grp_addr);
  mtlk_osal_lock_release(&nic->mcast.lock);
}
...
static int
parse_icmp6 (mtlk_core_t *nic, struct icmp6hdr *icmp6_header, uint32 length, const IEEE_ADDR *sta_addr)
{
  unsigned grp_num, src_num;
  unsigned i;
  ...
  case ICMPV6_MGM_REPORT:
    ...
    _mtlk_mc_add_ip6_sta(nic, sta_addr, &grp_addr, FALSE);
    break;
  case ICMPV6_MGM_REDUCTION:
    ILOG1_KY("MLD membership done: IPv6 %K, sta %Y", grp_addr.s6_addr, sta_addr->au8Addr);
    _mtlk_mc_del_ip6_sta(nic, sta_addr, &grp_addr);
    break;
  case ICMPV6_MLD2_REPORT:
    ...
    for (i = 0; i < grp_num; i++) {
      ...
      case MLD2_CHANGE_TO_INCLUDE:
        ...
        /* Station removed from the multicast list only if
         * no sources included */
        _mtlk_mc_change_to_include_ip6(nic, sta_addr, &grp_addr, src_num);
        break;
      ...
      case MLD2_CHANGE_TO_EXCLUDE:
        ...
        /* Station added to the multicast llist no matter
         * how much sources are excluded */
        _mtlk_mc_change_to_exclude_ip6(nic, sta_addr, &grp_addr);
        break;
      case MLD2_ALLOW_NEW_SOURCES:
        ILOG1_D(" --- Allow new sources, %d source(s)", src_num);
        _mtlk_mc_allow_new_sources_ip6(nic, sta_addr, &grp_addr, src_num);
        break;
      case MLD2_BLOCK_OLD_SOURCES:
        ILOG1_D(" --- Block old sources, %d source(s)", src_num);
        _mtlk_mc_block_old_sources_ip6(nic, sta_addr, &grp_addr, src_num);
        break;
...
mtlk_mc_update_group_id_sta (mtlk_core_t *nic, uint32 grp_id, mc_action_t action, mtlk_mc_addr_t *mc_addr, sta_entry *sta)
{
  ...
  case MTLK_MC_STA_JOIN_GROUP:
    ...
    if (mc_addr->type == MTLK_IPv4) {
      ...
      mcsta = __mtlk_mc_find_mcsta(group->mcsta, mtlk_sta_get_addr(sta));
      if (!mcsta) {
...
_mtlk_mc_update_sta_status (mtlk_core_t *nic, mtlk_core_ui_mc_update_sta_db_t *req, mcast_group_info_t *group_info)
{
  ...
  for (i = 0; i < req->macs_num; i++, mac_list++) {
    ...
    mcsta = __mtlk_mc_find_mcsta(mcsta_root, mac_list);
    if (MTLK_MC_STADB_DEL == req->action) {
      ...
      if (mcsta)
        ...
        mcsta = __mtlk_mc_add_mcsta(&group_info->mcsta, mac_list);
        if (mcsta)
  ...
  while (mcsta) {
    ...
    switch (status) {
      ...
      case MCAST_STA_STATUS_DELETE:
        /* Leave group in FW */
        mtlk_core_mcast_group_id_action_serialized(nic, MTLK_MC_STA_LEAVE_GROUP, group_info->grp_id, (uint8 *) mcsta->sta_addr.au8Addr, &req->mc_addr);
...
mtlk_mc_get_grid (mtlk_core_t *core, mtlk_nbuf_t *nbuf)
{
  ...
  switch (type) {
    ...
    case ETH_P_IPV6:
    {
      ...
      mtlk_osal_lock_acquire(&core->mcast.lock);
      if ((group = _mtlk_mc_find_ip6_group(core, &src_addr, &grp_addr)) != NULL) {
        /* Group was found */
        group_id = group->grp_id;
      }
      mtlk_osal_lock_release(&core->mcast.lock);
