...
typedef union
{
	...
	struct
	{
		...
		uint32 crRegPhyRx0EqReserved : 3; //Rx Equalizer Setting : , Sets the fixed value of the Rx equalizer. To enable tuning at the board level for Rx eye compliance, connect this signal to a register. The recommended setting is 3’b010. The optimal EQ setting will vary depending on users’ system environment., reset value: 0x2, access type: RW
		uint32 crRegPhyTx0TermOffsetReserved : 5; //Transmitter Termination Offset : , Enables adjustment of the transmitter termination value from the default value of 50  Ω ., reset value: 0x0, access type: RW
...
typedef union
{
	...
	struct
	{
		uint32 efuseAddr : 10; //BIT_ADDRESS[4:0] :	Bit address use only for programming operations. The value is ignored for reads. , WORD_ADDRESS[9:5] : WORD address of the fuse array use for read and program operation., reset value: 0x0, access type: RW
		uint32 reserved0 : 22;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		...
		uint32 efuseTsuCsStrobe : 16; //TIMER_TSU_CS_STROBE , Setup time beween CSB activation to STROBE activation. , , reset value: 0x1, access type: RW
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 efuseTrd : 16; //TIMER_TRD  , Strobe width during read. Minimum size is 120ns. , , reset value: 0xC, access type: RW
...
typedef union
{
	...
	struct
	{
		uint32 chipId : 32; //WAVE600 ChipID  : , Manufacturer ID = 0x009 , Part number = 0xE7D3 , version ID = 0x0 , , reset value: 0xE7D3013, access type: RO
	} bitFields;
} RegSocRegsChipId_u;
...
typedef union
{
	...
	struct
	{
		uint32 efuseInitVector : 32; //First WORD of EFUSE, reset value: 0x0, access type: RO
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 pvtEna : 1; //Temperature Sensor Enable : ,  0 - disable ,  1 - enable, reset value: 0x0, access type: RW
		uint32 pvtVsample : 1; //Temperature Sensor Vsample select :, reset value: 0x0, access type: RW
		uint32 pvtPsample : 2; //Temperature Sensor Tsample select :, reset value: 0x0, access type: RW
		uint32 pvtPsampleP : 1; //Temperature Sensor Psample_P select :, reset value: 0x0, access type: RW
		uint32 pvtPsampleN : 1; //Temperature Sensor Psample_N select :, reset value: 0x0, access type: RW
		uint32 reserved0 : 2;
		uint32 pvtTrim : 5; //Temperature Sensor Trim :, reset value: 0x0, access type: RW
		uint32 reserved1 : 19;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 pvtData : 10; //Temperature Sensor Data Out , reset value: 0x0, access type: RO
		uint32 reserved0 : 2;
		uint32 pvtDataValid : 1; //Temperature Sensor Data Valid : ,  0 - Not valid ,  1 - Data valid, reset value: 0x0, access type: RO
		uint32 reserved1 : 19;
	} bitFields;
