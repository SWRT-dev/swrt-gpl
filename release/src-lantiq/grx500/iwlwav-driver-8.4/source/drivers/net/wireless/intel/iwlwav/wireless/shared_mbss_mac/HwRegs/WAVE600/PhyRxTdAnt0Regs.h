...
typedef union
{
	...
	struct
	{
		uint32 olaSamplePerWindow : 8; //ola number of samples per window, reset value: 0x0, access type: RW
		uint32 reserved0 : 24;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		...
		uint32 reserved3 : 3;
		uint32 olaRssiMask : 1; //ola rssi mask : 1bit for each antenna, reset value: 0x0, access type: RW
		uint32 reserved4 : 3;
	} bitFields;
...
typedef union
{
	...
	struct
	{
		uint32 olaState : 3; //ola_state: , 000 - IDLE , 001 - FIRST WINDOW , 011 - REGULAR WINDOW , 100 - FIRST LAST WINDOW , 010 - LAST WINDOW , 101 - End OF WINDOWS, reset value: 0x0, access type: RO
		uint32 reserved0 : 1;
