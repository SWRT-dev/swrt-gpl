...
((_chipid_is_gen6_a0(id)) ? MTLK_GET_STATS_ADDRESS_A0(stats, parameter) : \
((_chipid_is_gen6_b0(id)) ? MTLK_GET_STATS_ADDRESS_B0(stats, parameter) : \
MTLK_GET_STATS_ADDRESS_D2(stats, parameter)))
...
{
  unsigned idx = CHAN_IDX_ILLEGAL, tmp_idx; 
  if (band6g_supported) {
    tmp_idx = channum6g2chanidx(channel_num);
    /* Avoid index 0 and map the 20MHz channels to indices 1,2,3,.. similar to other bands */
    idx = (tmp_idx != CHAN_IDX_ILLEGAL_6G ? (tmp_idx >> 1) + 1 : CHAN_IDX_ILLEGAL);
  } else {
...
struct _mtlk_hw_t
{
  ...
  uint32 ch_utilization_busy_tx_32[NUM_TOTAL_CHANS + 1];
                      ...
                      unsigned long    size)
{
  ...
  if ((CHI_CPU_NUM_TX == cpu) || (CHI_CPU_NUM_IF == cpu) || (CHI_CPU_NUM_RX == cpu)) {
      ...
      do {
          print_position = text;
          /* find_print_position() uses unsafe strlen, here its safe, as print_position is not NULL
           * and is NUL-terminated from init, and mtlk_snprintf() always is NUL-terminated */
...
_mtlk_mmb_cause_mac_assert (mtlk_hw_t *hw, uint32 mips_no)
{
  ...
  WLOG_PPD("mmb_pas=%p | mmb_base_phy=%p | pas_offset=%08X", hw->mmb_pas, hw->mmb_base_phy, pas_offset);
...
wave_mmb_build_pdpool_return_list (mtlk_hw_t *hw, uint32 FirstFreePdOffset, wave_hw_pdpool_return_list_t *pdpool_return_list)
{
  ...
  pdPoolStart = 0;
  CARD_SELECTOR_START(hw->ccr->hw_type)
    IF_CARD_PCIEG6 ( pdPoolStart = (uint8 *)(hw->mmb_base + pool_offset) );
  CARD_SELECTOR_END(); /* other types not supported yet */
  ...
  /* Adjust number of PDs */
  pdPoolNumOfPds = MIN(pdPoolNumOfPds, hw->dp_dev.dp_config.tx_bufs);
  TRACE_PARAM_INT(pdPoolNumOfPds);

  ret_dma_list = mtlk_osal_mem_alloc(sizeof(void*) * pdPoolNumOfPds, MTLK_MEM_TAG_HW);
  if (!ret_dma_list) {
    res = MTLK_ERR_NO_MEM;
    goto Fail;
  }
                                      ...
                                      mtlk_hw_data_ind_mirror_t *data_ind)
{
  ...
  CARD_SELECTOR_START(hw->ccr->hw_type)
    IF_CARD_PCIEG6 (
        /* The new algorithm is used on Gen6:
                     ...
                     uint32 hw_chip_rev, int fast_path)
{
      ...
      (HW_TYPE_WRX_600   == hw_type_info))   /* Wave600 ASIC */
  {
    ...
    switch (cpu) {
      ...
      case CHI_CPU_NUM_LM:
        ...
        return;
      case CHI_CPU_NUM_IF: fmt = FW_HOSTIF_NAME_FMT;
  ...
  switch (cpu) {
    case CHI_CPU_NUM_UM:
      ...
      mtlk_snprintf(fw_info->fname, sizeof(fw_info->fname), FW_UPPER_NAME_FMT, FW_AP_NAME_PREFFIX, chip_name);
      break;

    case CHI_CPU_NUM_LM:
      /* There is no separate file for ap and sta, FW can function as both. Use ap prefix */
      mtlk_snprintf(fw_info->fname, sizeof(fw_info->fname), FW_LOWER_NAME_FMT, FW_AP_NAME_PREFFIX, chip_name);
      break;

    case CHI_CPU_NUM_TX:
      mtlk_snprintf(fw_info->fname, sizeof(fw_info->fname), FW_TRANS_NAME_FMT, chip_name);
      break;

    case CHI_CPU_NUM_IF:
    {
      const char* suffix =  _wave_hw_get_hostif_dc_mode(hw, FASTPATH_DISABLED);
      mtlk_snprintf(fw_info->fname, sizeof(fw_info->fname), FW_HOSTIF_NAME_FMT, chip_name, suffix);
      break;
    }

    case CHI_CPU_NUM_RX:
      mtlk_snprintf(fw_info->fname, sizeof(fw_info->fname), FW_RXHANDLER_NAME_FMT, chip_name);
      break;
...
{
    ILOG0_DSDDDDDDD("Line %u: CHI: %s: PdPoolBaseAddress=0x%x u32DataPdPoolOffsetFromBaseAddress=0x%x PdSizeInBytes=0x%x PayloadAddressOffsetInPd=0x%x NextPdPointerOffsetInPd=0x%x u32NextPdPointerFieldMask=%u"
...
_mtlk_get_rx_ring_size(mtlk_hw_t *hw)
{
    ...
    if (param_ring_size > 0)
    {
      ...
      CARD_SELECTOR_START(hw->ccr->hw_type) /* initialize ring_size */
        IF_CARD_PCIEG6 (ring_size = pool_size);
      CARD_SELECTOR_END(); /* other types should not get here */
    }
...
_mtlk_mmb_cleanup_reset_mac(mtlk_hw_t *hw)
{
  ...
  ILOG0_D("TELEMETRY: CID-%02x: FW CPU reset is done", hw->card_idx);
}
...
_mtlk_hw_parse_statistics (mtlk_hw_t *hw)
{
  ...
  if (_chipid_is_gen6_a0 (chip_id)) {
      ...
      phy_stats_size = sizeof(wholePhyRxStatusDb_t_wave600d2);
      phy_stats = (void*)MTLK_GET_STATS_ADDRESS_D2(hw_stats->stats_copy, phyStatistics);
  }
...
_mtlk_mmb_data_path_init (mtlk_hw_t* hw /*, BOOL is_recovery*/)
{
  ...
  if (mtlk_mmb_dcdp_path_available(hw) /* && !skip_datapath_init*/)
  {
      ...
      if (dp_dev->dp_resources.buflist) {
        ...
        if (dp_dev->dp_resources.num_bufpools > dp_dev->dp_resources.num_bufs_req) {
          ELOG_DD("Number of returned buffers (%u) is greater than requested size (%u)",
              dp_dev->dp_resources.num_bufpools, dp_dev->dp_resources.num_bufs_req);
          MTLK_ASSERT(FALSE);
...
#ifdef MTLK_DEBUG
          if (idx < 4 || idx > (dp_dev->dp_resources.num_bufpools-32)) {
...
_mtlk_hw_get_prop (mtlk_hw_t *hw, mtlk_hw_prop_e prop_id, void *buffer, uint32 size)
{
  ...
  case MTLK_HW_EEPROM_NAME:
    ...
    else
    {
      ...
      mtlk_snprintf(fname, size, "cal_wlan%d.bin", card_idx_to_master_vap_idx (hw->card_idx));
                          ...
                          BOOL power_mgmt_on, uint32 tid)
{
  ...
  */
  ...
  /* we allow switching channels in AP mode for DPP Public Action Frames */
  ...
  if (PROCESS_MANAGEMENT == extra_processing)
  {
    ...
    if (MAN_TYPE_PROBE_RES == subtype) {
      ...
      if((ie_bss) && (ie_bss->length == 5)) {
        ...
        idx = channum2cssidx(channum);
        ch_util = hw->chan_statistics[idx].ch_utilization_total;
...
mtlk_hw_send_hdk_prepare_ant_config (mtlk_hw_t *hw)
{
    ...
    ILOG0_V("Sending UMI_HDK_PREPARE_ANT_CONFIG_CHANGE_REQ message");
    res = mtlk_txmm_msg_send_blocked(&man_msg, MTLK_MM_BLOCKED_SEND_TIMEOUT);
    if (res != MTLK_ERR_OK) {
...
BOOL __MTLK_IFUNC wave_hw_zwdfs_antenna_is_active(mtlk_hw_t *hw)
{
  ...
  for (radio_idx = 0; radio_idx < radio_descr->num_radios; radio_idx++) {
    ...
    if (wave_radio_get_zwdfs_ant_config(radio)) {
      return TRUE;
    }
  }
  return FALSE;
...
static void __MTLK_IFUNC
wave_hw_cdb_antenna_cfg_req_update_ant_mask (wave_hw_radio_band_cfg_t band_cfg,
                                             uint8 *ant_mask)
{

  /* TODO: These masks are based on inputs from FW. A better and clean way
     would be to derive these masks based on the band configuration
     from the PSD parsed data
   */
  switch (band_cfg) {
...
int __MTLK_IFUNC wave_hw_cdb_antenna_cfg_req (mtlk_hw_t *hw, wave_hw_radio_band_cfg_t band_cfg)
{
  wave_radio_descr_t *radio_descr;
  wave_radio_t *radio;
  unsigned radio_idx;
  ...
  /* Invalid band configuration inputs */
  if (band_cfg != WAVE_HW_RADIO_BAND_CFG_DB_3x1 &&
      band_cfg != WAVE_HW_RADIO_BAND_CFG_SCAN) {
    ELOG_S("Invalid band configuration: %s", band_cfg_to_str[band_cfg]);
    return MTLK_ERR_VALUE;
  }

  /* Reject duplicate requests to enable/disable ZWDFS antenna */
  if ((band_cfg == WAVE_HW_RADIO_BAND_CFG_DB_3x1 && wave_hw_zwdfs_antenna_is_active(hw)) ||
      (band_cfg == WAVE_HW_RADIO_BAND_CFG_SCAN && !wave_hw_zwdfs_antenna_is_active(hw))) {
    ELOG_S("ZWDFS antenna is already %s", (band_cfg == WAVE_HW_RADIO_BAND_CFG_DB_3x1) ? "enabled" : "disabled");
    return MTLK_ERR_VALUE;
  }

  wave_hw_cdb_antenna_cfg_req_update_ant_mask(band_cfg, &ant_mask[0]);
  ...
  for (radio_idx = 0; radio_idx < radio_descr->num_radios; radio_idx++) {
    ...
    ILOG0_D("Update antenna config for radio_id %u", radio_idx);
    wave_radio_antenna_cfg_update(radio, ant_mask[radio_idx]);
  }

  /* Prepare and send the HDK Antennas Configuration change to FW */
  for (radio_idx = 0; radio_idx < radio_descr->num_radios; radio_idx++) {
    ILOG0_D("Processing radio_idx %u", radio_idx);
    radio = wave_radio_descr_wave_radio_get(radio_descr, radio_idx);
    MTLK_ASSERT(radio != NULL);

    res = wave_radio_cdb_antenna_cfg_req(radio, wave_radio_band_get(radio));

    ILOG0_DS("HDK antenna config change for radio_idx %u %s",
             radio_idx, (MTLK_ERR_OK == res) ? "succeeded" : "failed");
  ...
  for (radio_idx = 0; radio_idx < radio_descr->num_radios; radio_idx++) {
    radio = wave_radio_descr_wave_radio_get(radio_descr, radio_idx);
    MTLK_ASSERT(radio != NULL);
    ILOG0_D("Update COC antenna config for radio_idx %u", radio_idx);
...
static int __MTLK_IFUNC wave_hw_get_noise_floor (mtlk_hw_t *hw, unsigned radio_idx)
{
...
void get_channel_data_hw_save_chan_stats_info(mtlk_core_t *core, struct intel_vendor_channel_data *ch_data)
{
  ...
  ch_data->noise_floor = wave_hw_get_noise_floor(hw, radio_idx);
}
...
int scan_get_aocs_info (mtlk_core_t *core, struct intel_vendor_channel_data *ch_data, struct channel_survey_support *csys)
{
  ...
  ch_data->filled_mask = CHDATA_DYNBW | CHDATA_NOISE_FLOOR | CHDATA_SNR |
...
mtlk_hw_save_chan_statistics_info (mtlk_hw_t *hw, struct mtlk_chan_def *ccd, struct intel_vendor_channel_data *ch_data, BOOL ch_radar_noise, uint8 radio_id)
{
  ...
  if (MTLK_HW_BAND_6_GHZ == band)
    idx = compute_stat_index(true, ch_data->channel);
  else
    idx = channum2cssidx(ch_data->channel);
  ...
  __stats_accumulate_64bit(&hw->chan_statistics[idx].ch_utilization_busy_tx, &hw->ch_utilization_busy_tx_32[idx],
      (MTLK_GET_STATS(hw_stats->stats_copy, currentChannelStats.channelUtilizationStats[radio_id].chUtilizationBusyTx, chip_id)));
  ...
  hw->chan_statistics[idx].ch_noise = ch_data->noise_floor;
  }
}
...
wave_hw_bss_tx_map_init(mtlk_hw_t *hw)
{
  ...
  mtlk_osal_lock_init(&hw->bss_tx_hd_map_lock);
...
wave_hw_bss_tx_map_check(mtlk_hw_t *hw)
{
  ...
  for (i = 0; i < ARRAY_SIZE(hw->bss_tx_hd_map); i++) {
    mtlk_osal_lock_acquire(&hw->bss_tx_hd_map_lock);
...
static BOOL
_wave_hw_is_bss_tx_sta_cfmed (mtlk_hw_t *hw, uint16 sid, BOOL failed, int *from_idx)
{
  int i;
  bss_tx_hd_map_t hd_map_entry;
  BOOL unconfirmed = FALSE;

  if (*from_idx >= ARRAY_SIZE(hw->bss_tx_hd_map)) {
    ELOG_DD("card %d: wrong from_idx %d", hw->card_idx, *from_idx);
    goto finish;
  }

  for (i = *from_idx; i < ARRAY_SIZE(hw->bss_tx_hd_map); i++) {
    mtlk_osal_lock_acquire(&hw->bss_tx_hd_map_lock);
    hd_map_entry = hw->bss_tx_hd_map[i];
    mtlk_osal_lock_release(&hw->bss_tx_hd_map_lock);
...
wave_hw_is_bss_tx_sta_cfmed (mtlk_hw_t *hw, uint16 sid)
{
  ...
  if (_wave_hw_is_bss_tx_sta_cfmed(hw, sid, FALSE, &start_idx)) {
    ILOG1_DD("card idx %d: 1st check: all is confirmed for sid %d", hw->card_idx, sid);
    return TRUE;
  }
  ...
  if (_wave_hw_is_bss_tx_sta_cfmed(hw, sid, TRUE, &start_idx)) {
    ILOG1_DD("card idx %d: 2nd check: all is confirmed for sid %d", hw->card_idx, sid);
    return TRUE;
  }

  ILOG0_D("card idx %d: failed", hw->card_idx);
  return FALSE;
}
