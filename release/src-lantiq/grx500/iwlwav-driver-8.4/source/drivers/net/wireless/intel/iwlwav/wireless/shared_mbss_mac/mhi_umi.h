...
#define UM_MAN_MU_OFDMA_BF_CONFIG_REQ 		UMI_MSG_MAN_REQ(UMI_MAN_MU_OFDMA_BF_CONFIG_REQ)
#define MC_MAN_MU_OFDMA_BF_CONFIG_CFM		UMI_MSG_MAN_CFM(UMI_MAN_MU_OFDMA_BF_CONFIG_REQ)
...
typedef struct _UMI_UNSOLICITED_SET
{
	...
	uint16 filsTimestampOffset;
	uint16 txIntervalMsecs;
	uint8  isEnabled;
	uint8  frameType;
	uint8  Status;
	uint8  Reserved[3];
...
#define KEY_CIPHER_SUITE_MIN  0x0
#define KEY_CIPHER_SUITE_MAX  0x7

#define KEY_IDX_MIN  0x0
#define KEY_IDX_MAX  0x3
...
typedef struct _RXDAT_IND_MSG_DESC
{
    uint32 u32HostPayloadAddr;
...
/* Drop probability calculation: the following struct is used to hold the constants for the normalizing of 'P' 
if (PIE->drop_prob_ < 4) {
	p /= 2048;
*/
...
typedef struct _UMI_PIE_CONTROL
{
	...
	uint32			minPd;										/* A constant value below which the packet is enqueued. 																*/
	...
	/* word 34 */
	uint16			tUpdate;									/* A constant value in 4[uSec] quantizing which indicates the periodicity of the Drop Probability Calculation.			*/
	uint8 			userMode;									/* TRUE: PIE parameters have been modified by the user.																	*/
	uint8			getSetOperation;
}UMI_PIE_CONTROL;
