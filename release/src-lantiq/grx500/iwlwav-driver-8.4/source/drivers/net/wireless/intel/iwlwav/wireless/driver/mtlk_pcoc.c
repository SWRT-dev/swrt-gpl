...
{
#if 0 /* temporary workaround due to issue in cpufreq */
  return (PMCU_Power_State_Get() == LTQ_CPUFREQ_PS_D0);
#else
...
{
#if 0 /* temporary workaround due to issue in cpufreq */
  return (PMCU_Power_State_Get() == LTQ_CPUFREQ_PS_D3);
#else
...
int wv_PMCU_Create(void)
{
  ...
  /* init variables */
  mtlk_osal_atomic_set(&pPMCU->freq_to_switch, PMCU_SWITCH_TO_NONE);
#if 0 /* Temporary workaround until fixed in CPU freq */
  pPMCU->power_state = PMCU_Power_State_Get();
#else
  {
