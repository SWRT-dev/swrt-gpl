...
/* Unsolicited Probe Response generation Disabled by default */
#define ENABLE_UNSOLICIT_PROBE_RESP 0
#define UNSOLICIT_PROBE_RESP_INTERVAL (5)
...
#define MAX_AMSDU_NUM     31 /* AMSDU_NUM range */
...
struct nic_slow_ctx {
  ...
  mtlk_osal_timer_t unsolicited_probe_resp_timer;
#endif
};
...
bool extended_6g_ssid_match(mtlk_hw_band_e band, mtlk_pdb_t *param_db_core, ie_t *essid, 
                            u8 *ssid, u8 ssid_len, u8 *buf, u8 buf_size, IEEE_ADDR *bssid);
#if ENABLE_UNSOLICIT_PROBE_RESP
int unsolicited_probe_async_func(mtlk_handle_t core_object, const void *payload, uint32 size);
#endif
