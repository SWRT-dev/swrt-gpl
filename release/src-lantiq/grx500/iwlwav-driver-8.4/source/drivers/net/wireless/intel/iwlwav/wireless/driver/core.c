                                     ...
                                     sta_entry *sta)
{
  ...
  if (mtlk_vap_is_ap(nic->vap_handle) &&
    ...
    mtlk_vap_diconnect_backhaul_sta(nic->vap_handle, sta);
  }
}
...
{
  return !mtlk_core_scan_is_running(nic) || is_scan_fast_rcvry;
}
...
unicast:
    ...
    if (sta) {
      ...
      if (bridge_mode == BR_MODE_WDS || (mtlk_stadb_get_four_addr_sta_cnt(stadb) &&
                                         mesh_mode != MESH_MODE_EXT_HYBRID)) {
...
_mtlk_core_handle_tx_data (mtlk_core_t *nic, mtlk_core_handle_tx_data_t *data, uint32 nbuf_flags)
{
  ...
  else
  {
    ...
    if (sta) {
      ...
      if (MESH_MODE_EXT_HYBRID == mesh_mode && mtlk_sta_is_4addr(sta))
        data->vap_id = sta->secondary_vap_id;
      else if (sta->peer_ap || mtlk_sta_is_4addr(sta)) {
        wds = 1;
...
static uint32 _core_management_frames_rate_select (mtlk_core_t *core)
{
  ...
  return _core_management_frames_rate_get(mcore);
}
...
_mtlk_core_fill_set_bss_request(mtlk_core_t *core, UMI_SET_BSS *req, struct mtlk_bss_parameters *params)
{
    ...
    /* 802.11AX HE support */
    {
      ...
      req->u8HE_Bss_Color = he_operation[HE_OPERATION_CAP3_IDX];

      ILOG1_D("UMI_SET_BSS->u8HE_Bss_Color: %d", req->u8HE_Bss_Color);
    }
}
...
int __MTLK_IFUNC mtlk_core_set_bss(mtlk_core_t *core, mtlk_core_t *context_core, UMI_SET_BSS *fw_params)
{
    ...
    *req = *fw_params;
    ...
    ILOG2_D("UMI_SET_BSS->u8HE_Bss_Color:%d", req->u8HE_Bss_Color);
...
mtlk_core_on_bss_dpp_acked(mtlk_core_t *nic, uint32 subtype, uint8 action_code ,uint8 action_field, BOOL is_dpp){
  ...
  {
    mtlk_osal_timer_set(&ss->scan_timer, 0);
  }
}
#endif
...
_mtlk_core_receive_dynamic_mc_rate (mtlk_core_t *core, uint32 *dynamic_mc_rate)
{
  ...
  dmr_cfg = (UMI_DMR_CONFIG *)(man_entry->payload);
  dmr_cfg->getSetOperation = API_GET_OPERATION;

  res = mtlk_txmm_msg_send_blocked(&man_msg, MTLK_MM_BLOCKED_SEND_TIMEOUT);

  if (MTLK_ERR_OK == res && UMI_OK == dmr_cfg->Status) {
    *dynamic_mc_rate = MAC_TO_HOST32(dmr_cfg->dmrMode);
  }
  else {
    ELOG_DDD("CID-%04x: Receiving UM_MAN_DMR_CONFIG_REQ failed, res=%d status=%hhu",
            mtlk_vap_get_oid(vap_handle), res, dmr_cfg->Status);
    if (UMI_OK != dmr_cfg->Status)
      res = MTLK_ERR_MAC;
  }
...
_wave_core_pie_cfg_receive (mtlk_core_t *core, wave_pie_cfg_t *pie_cfg_params)
{
  ...
  if (MTLK_ERR_OK == res) {
    ...
    pie_cfg_params->min_pd        = MAC_TO_HOST32(pie_cfg->minPd);
...
{
#if 0 /* suppot of UMI_HE_MU_OPERATION_CONFIG removed from the FW */
  mtlk_txmm_msg_t                 man_msg;
  mtlk_txmm_data_t               *man_entry = NULL;
  ...
  if (MTLK_ERR_OK != res || UMI_OK != mu_operation_config->Status) {
    ELOG_DDD("CID-%04x: Set UMI_HE_MU_OPERATION_CONFIG failed, res=%d status=%hhu",
...
_mtlk_core_send_dynamic_mc_rate (mtlk_core_t *core, uint32 dynamic_mc_rate)
{
  ...
  res = mtlk_txmm_msg_send_blocked(&man_msg, MTLK_MM_BLOCKED_SEND_TIMEOUT);
  if (MTLK_ERR_OK != res || UMI_OK != dmr_cfg->Status) {
    ELOG_DDD("CID-%04x: Sending UM_MAN_DMR_CONFIG_REQ failed, res=%d status=%hhu",
          mtlk_vap_get_oid(vap_handle), res, dmr_cfg->Status);
    if (UMI_OK != dmr_cfg->Status)
      res =  MTLK_ERR_MAC;
  }
...
_mtlk_core_store_and_send_dynamic_mc_rate (mtlk_core_t *core, uint32 dynamic_mc_rate)
{
  ...
  if ((res = _mtlk_core_send_dynamic_mc_rate(core, dynamic_mc_rate)) == MTLK_ERR_OK)
...
_wave_core_pie_cfg_send(mtlk_core_t *core, wave_pie_cfg_t *pie_cfg_params)
{
  ...
  pie_cfg->minPd       = HOST_TO_MAC32(pie_cfg_params->min_pd);
...
static int
_mtlk_core_set_interfdet_do_params (mtlk_core_t *core, wave_radio_t *radio, const int8 detection_threshold_20mhz,
        const int8 notification_threshold_20mhz, const int8 detection_threshold_40mhz,
        const int8 notification_threshold_40mhz)
{
  BOOL interf_det_enabled, is_spectrum_40;
  int res = MTLK_ERR_UNKNOWN;
  int8 notification_threshold;

  MTLK_ASSERT(core != NULL);
  MTLK_ASSERT(mtlk_vap_is_master_ap(core->vap_handle));

  interf_det_enabled = FALSE;
  interf_det_enabled |= (0 != detection_threshold_20mhz);
  interf_det_enabled |= (0 != notification_threshold_20mhz);

  if (!core->is_stopped) {
    is_spectrum_40 = (CW_40 == _mtlk_core_get_spectrum_mode(core));

    if (is_spectrum_40)
      notification_threshold = notification_threshold_40mhz;
    else
      notification_threshold = notification_threshold_20mhz;
      
    if (MTLK_ERR_OK == (res = _mtlk_core_set_fw_interfdet_req(core, notification_threshold))) {
      ...
      WAVE_RADIO_PDB_SET_INT(radio, PARAM_DB_RADIO_INTERFDET_20MHZ_DETECTION_THRESHOLD, detection_threshold_20mhz);
      WAVE_RADIO_PDB_SET_INT(radio, PARAM_DB_RADIO_INTERFDET_20MHZ_NOTIFICATION_THRESHOLD, notification_threshold_20mhz);
      WAVE_RADIO_PDB_SET_INT(radio, PARAM_DB_RADIO_INTERFDET_40MHZ_DETECTION_THRESHOLD, detection_threshold_40mhz);
      WAVE_RADIO_PDB_SET_INT(radio, PARAM_DB_RADIO_INTERFDET_40MHZ_NOTIFICATION_THRESHOLD, notification_threshold_40mhz);
    }
    else {
...
_mtlk_core_set_interfdet_cfg (mtlk_handle_t hcore, const void* data, uint32 data_size)
{
  ...
  MTLK_CLPB_TRY(interfdet_cfg, interfdet_cfg_size)
    ...
    res = _mtlk_core_set_interfdet_do_params(core, radio, interfdet_cfg->req_thresh.detection_threshold_20mhz,
            interfdet_cfg->req_thresh.notification_threshold_20mhz, interfdet_cfg->req_thresh.detection_threshold_40mhz,
            interfdet_cfg->req_thresh.notification_threshold_40mhz);
    MTLK_CFG_END_CHEK_ITEM_AND_CALL()
  MTLK_CLPB_FINALLY(res)
    return mtlk_clpb_push_res(clpb, res);
  MTLK_CLPB_END
...
_mtlk_core_get_iw_generic (mtlk_handle_t hcore, const void* data, uint32 data_size)
{
  ...
  MTLK_CLPB_TRY(req_df_cfg, req_df_cfg_size)
    ...
    res = mtlk_txmm_msg_send_blocked(&man_msg, MTLK_MM_BLOCKED_SEND_TIMEOUT);
    if (MTLK_ERR_OK != res || UMI_OK != pdata->retStatus) {
      ELOG_DDD("CID-%04x: Can't send generic request to MAC, timed-out res=%d status=%hhu",
              mtlk_vap_get_oid(core->vap_handle), res, pdata->retStatus);
      if (UMI_OK != pdata->retStatus)
        res = MTLK_ERR_MAC;
      MTLK_CLPB_EXIT(res);
    }
...
static uint32
unsolicited_probe_handler (mtlk_osal_timer_t *timer, mtlk_handle_t data)
{
  mtlk_error_t err;
  struct nic *nic = (struct nic *)data;
  wave_radio_t *radio;
  ...
  if (band == MTLK_HW_BAND_6_GHZ) {
    err = _mtlk_core_execute_async(nic, MTLK_ABILITY_NONE, HANDLE_T(nic),unsolicited_probe_async_func,
                                   NULL, 0, _MTLK_CORE_PRIORITY_EMERGENCY, NULL);
    if (err != MTLK_ERR_OK)
      ELOG_DD("CID-%04x: Can't schedule unsolicited probe resp task (err=%d)", mtlk_vap_get_oid(nic->vap_handle), err);
  }

  return UNSOLICIT_PROBE_RESP_INTERVAL;
}
#endif
...
void mtlk_core_finalize_and_send_probe_resp(mtlk_core_t *core, mtlk_mngmnt_frame_t *frame)
{
   ...
   * In the case of 6GHz clients, the destination address is always set to Broadcast address                                           */
  if (band == MTLK_HW_BAND_6_GHZ)
...
#endif
  wv_ieee80211_mngmn_frame_rx(nic, data, size, freq, phy_info->sig_dbm, phy_info->snr_db, phy_info->noise_estimation,
                              subtype, phy_info->rate_idx, phy_info->phy_mode, pmf_flags, nss);
}

ie_t *mtlk_core_find_eid(u8 *buf, unsigned len, u8 elem_id, u8 ext_elem_id)
{
    ...
    for (i = 0; i < max_vaps; i++) {
      ...
      if (mtlk_vap_is_ap(vap_handle)) {
        ...
        if (size > sizeof(frame_head_t)) {
          ...
          essid = mtlk_core_find_eid(loc_ptr, loc_size, IE_SSID, 0);
        }
        ...
        if ((essid && essid->length == our_ssid_len && !memcmp((char *) essid + sizeof(ie_t), our_ssid, our_ssid_len))
            || (band != MTLK_HW_BAND_6_GHZ && (!essid || essid->length == 0) && !hidden)
            || (!hidden && extended_6g_ssid_match(band, param_db_core,
                                                  essid, our_ssid, our_ssid_len,
                                                  loc_ptr, loc_size, bssid)))
        {
          ...
          if (cur_core->slow_ctx->probe_resp_templ) {
             ...
             if ((softblockcheck == DRV_SOFTBLOCK_ACCEPT) || (softblockcheck == DRV_SOFTBLOCK_ALLOW)) {
                ...
                if(MTLK_CORE_PDB_GET_INT(cur_core, PARAM_DB_CORE_MBSSID_VAP) != WAVE_RADIO_OPERATION_MODE_MBSSID_NON_TRANSMIT_VAP)
...
int _handle_radar_event(mtlk_handle_t core_object, const void *payload, uint32 size)
{
  ...
  struct wiphy *wiphy = wv_mac80211_wiphy_get(mac80211);
  ...
  if (sizeof(UMI_RADAR_DETECTION) == size) {
    ...
    emulating = FALSE;
    /* Sub Band DFS PHY layer development has not started yet.
  ...
  if (ss->dfs_debug_params.debug_chan) {
    ...
    ss->dfs_debug_params.cac_started = cac_started;
  }
  ...
  mtlk_df_user_handle_radar_event(df_user, wv_ieee80211_hw_get(mac80211), &ccd, cac_started, rbm);
    ...
    if ((core->slow_ctx->group_cipher == IW_ENCODE_ALG_TKIP)
     ...
     || (core->slow_ctx->group_cipher == IW_ENCODE_ALG_AES_CMAC)) {
      if (0 != MTLK_BFIELD_GET(info->flags, STA_FLAGS_IS_8021X_FILTER_OPEN)) {
        ELOG_DY("CID-%04x: STA:%Y flag STA_FLAGS_IS_8021X_FILTER_OPEN must be 0", mtlk_vap_get_oid(core->vap_handle), mac80211_sta->addr);
        MTLK_BFIELD_SET(info->flags, STA_FLAGS_IS_8021X_FILTER_OPEN, 0);
      }
    } else if (core->slow_ctx->group_cipher == IW_ENCODE_ALG_NONE) {
      MTLK_BFIELD_SET(info->flags, STA_FLAGS_IS_8021X_FILTER_OPEN, 1);
    }
  }
    ...
    if (core->slow_ctx->peerAPs_key_idx) {
      ...
      if (MTLK_ERR_OK != _mtlk_core_set_wep_key_blocked(core, &sid, core->slow_ctx->peerAPs_key_idx - 1)) {
        ...
        if (connected_to_backhaul_ap) {
          mtlk_vap_diconnect_backhaul_sta(core->vap_handle, sta);
        }
...
#if ENABLE_UNSOLICIT_PROBE_RESP
  MTLK_INIT_STEPS_LIST_ENTRY(core_slow_ctx, UNSOLICITED_PROBE_TIMER_INIT)
#endif
...
#if ENABLE_UNSOLICIT_PROBE_RESP
    MTLK_CLEANUP_STEP(core_slow_ctx, UNSOLICITED_PROBE_TIMER_INIT, MTLK_OBJ_PTR(slow_ctx),
                      mtlk_osal_timer_cleanup, (&slow_ctx->unsolicited_probe_resp_timer));
#endif
...
#if ENABLE_UNSOLICIT_PROBE_RESP
     MTLK_INIT_STEP(core_slow_ctx, UNSOLICITED_PROBE_TIMER_INIT, MTLK_OBJ_PTR(slow_ctx),
                    mtlk_osal_timer_init, (&slow_ctx->unsolicited_probe_resp_timer,
                                           unsolicited_probe_handler,
                                           HANDLE_T(nic)));
#endif
...
#if ENABLE_UNSOLICIT_PROBE_RESP
  MTLK_START_STEPS_LIST_ENTRY(core_slow_ctx, UNSOLICITED_PROBE_TIMER_START)
#endif
...
#if ENABLE_UNSOLICIT_PROBE_RESP
    MTLK_STOP_STEP(core_slow_ctx, UNSOLICITED_PROBE_TIMER_START, MTLK_OBJ_PTR(slow_ctx),
                   mtlk_osal_timer_cancel_sync, (&slow_ctx->unsolicited_probe_resp_timer))
#endif
...
_mtlk_core_set_ap_beacon_info (mtlk_handle_t hcore, const void* data, uint32 data_size)
{
  ...
  MTLK_CLPB_TRY(beacon_info_params, params_size)
    ...
    if (MTLK_ERR_OK != mtlk_vap_manager_get_vap_handle(mtlk_vap_get_manager(master_core->vap_handle),
                                                       beacon_info_params->vap_index, &vap_handle))
      MTLK_CLPB_EXIT(MTLK_ERR_PARAMS);

    /* Ap mode only, Station mode has set_beacon_interval */
    if (!mtlk_vap_is_ap(vap_handle))
      MTLK_CLPB_EXIT(MTLK_ERR_PARAMS);
...
#define _WAVE_CORE_REQ_MAP_END(_id_)                                                    \
  ...
  _WAVE_CORE_REQ_MAP_START(id)
    ...
    _MTLK_CORE_HANDLE_REQ_SERIALIZABLE(WAVE_RADIO_REQ_MBSS_VAP_DELETE_SEC_VAPS, _mtlk_core_cfg_delete_sec_vaps);
    ...
    _MTLK_CORE_HANDLE_REQ_SERIALIZABLE(WAVE_HW_REQ_GET_ZWDFS_ANT,               wave_core_get_zwdfs_antenna);
    _MTLK_CORE_HANDLE_REQ_SERIALIZABLE(WAVE_HW_REQ_SET_ZWDFS_ANT,               wave_core_set_zwdfs_antenna);
...
_mtlk_core_set_enc_ext_cfg (mtlk_handle_t hcore, const void* data, uint32 data_size)
{
  ...
  { /*key seq block*/
    uint8 * core_seq = core->slow_ctx->seq;
    /*now that the key is initialized in the fw,
     * if the seq is not initialized in slow ctx we will initialize it */
    if ((core_seq[0] | core_seq[1] | core_seq[2] | core_seq[3] | core_seq[4] | core_seq[5]) == 0){
      core_seq[0] = 1;
      ILOG2_D("CID-%04x: key seq initialized", mtlk_vap_get_oid(core->vap_handle));
    }
  }
...
#if ENABLE_UNSOLICIT_PROBE_RESP
     mtlk_osal_timer_cancel_sync(&core->slow_ctx->unsolicited_probe_resp_timer);
#endif
...
mtlk_core_cfg_recovery_set_coex (mtlk_core_t *core)
{
  ...
  return mtlk_core_send_coex_config_req(core, coex_mode, active_time, inactive_time);
}
...
static int _mtlk_core_recover_interfdet_threshold (mtlk_core_t *core)
{
  ...
  return _mtlk_core_set_fw_interfdet_req(core, thr);
}
...
int mtlk_core_dp_get_subif_cb (mtlk_vap_handle_t vap_handle, char *mac_addr, uint32_t *subif, uint8 vap_id) {
  uint16    sta_id = 0;
  sta_entry   *sta = NULL;
  mtlk_core_t *nic = mtlk_vap_get_core(vap_handle);
...
_mtlk_core_get_unconnected_station (mtlk_handle_t hcore, const void* data, uint32 data_size)
{
  ...
  if (mtlk_vap_manager_is_sta_connected(core, &sta_core, addr)) {
    ...
    /* add station */
    {
      ...
      MTLK_BFIELD_SET(psUmiStaAdd->u8Flags, STA_ADD_FLAGS_IS_8021X_FILTER_OPEN, 1);
...
int fill_channel_data (mtlk_core_t *core, struct intel_vendor_channel_data *ch_data)
{
  ...
  ch_data->freq = channel_to_frequency(freq2lowchannum(cd->center_freq1, cd->width), wave_radio_band_get(radio));
