...
TRACE_EVENT(drv_handle_radar_debug,
	...
	TP_printk(
		LOCAL_PR_FMT " driver radar debug for " CHANDEF_PR_FMT,
		LOCAL_PR_ARG, CHANDEF_PR_ARG
	)
);
...
TRACE_EVENT(drv_notify_cac_started,
	TP_PROTO(struct ieee80211_local *local,
		 struct ieee80211_sub_if_data *sdata,
		 u32 cac_time_ms),
	...
	TP_printk(
		LOCAL_PR_FMT VIF_PR_FMT " cac started time:%d",
