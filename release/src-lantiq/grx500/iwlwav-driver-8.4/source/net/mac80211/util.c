...
int ieee80211_reconfig(struct ieee80211_local *local)
{
	...
	list_for_each_entry(sdata, &local->interfaces, list) {
		...
		case NL80211_IFTYPE_AP:
			changed |= BSS_CHANGED_SSID | BSS_CHANGED_P2P_PS | BSS_CHANGED_AP_ISOLATE;
				     ...
				     unsigned int mpdu_offset)
{
	...
	/* TODO: Check for the 6GHz band support */
...
void ieee80211_dfs_cac_restart(struct ieee80211_hw *hw){
	...
	list_for_each_entry(sdata, &hw_to_local(hw)->interfaces, list) {
		...
		if (sdata->wdev.cac_started) {
			...
			drv_notify_cac_started(sdata->local, sdata,
...
void ieee80211_dfs_radar_detected_work(struct work_struct *work)
{
	...
	if (num_chanctx > 1)
		...
		drv_handle_radar_debug(local, &chandef);
		rtnl_unlock();
	}
}
