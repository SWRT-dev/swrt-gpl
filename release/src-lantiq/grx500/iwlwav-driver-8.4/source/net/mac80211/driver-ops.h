...
{
	trace_drv_handle_radar_debug(local, chandef);
	if (local->ops->handle_radar_debug)
		local->ops->handle_radar_debug(&local->hw, chandef);
	trace_drv_return_void(local);
}
...
static inline void drv_notify_cac_started(struct ieee80211_local *local,
					  struct ieee80211_sub_if_data *sdata,
					  u32 cac_time_ms)
{
	trace_drv_notify_cac_started(local, sdata, cac_time_ms);
	if (local->ops->notify_cac_started)
		local->ops->notify_cac_started(&local->hw, &sdata->vif,
					       cac_time_ms);
	trace_drv_return_void(local);
}
					   ...
					   bool cac_canceled)
{
	...
	if (local->ops->notify_cac_finished)
		local->ops->notify_cac_finished(&local->hw, &sdata->vif,
						cac_canceled);
	trace_drv_return_void(local);
}
				    ...
				    struct cfg80211_qos_map *qos_map)
{
	...
	if (local->ops->set_qos_map)
		ret = local->ops->set_qos_map(&local->hw, &sdata->vif, qos_map);
