			  ...
			  struct cfg80211_chan_def *chandef)
{
	...
	if (info->attrs[NL80211_ATTR_SB_DFS_BW])
		chandef->sb_dfs_bw = nla_get_u8(info->attrs[NL80211_ATTR_SB_DFS_BW]);
					 ...
					 struct genl_info *info)
{
	...
	if (wiphy->cac_time_debug >= 0 && wiphy->cac_time_debug < 1000000)
					  ...
					  struct genl_info *info)
{
	...
	if ((dfs_state == NL80211_DFS_AVAILABLE) && (dfs_region != NL80211_DFS_ETSI)) {
		GENL_SET_ERR_MSG(info,
	...
	/* Not allowed */
	if (chandef.chan->dfs_state == NL80211_DFS_UNAVAILABLE && dfs_state == NL80211_DFS_AVAILABLE) {
		GENL_SET_ERR_MSG(info,
