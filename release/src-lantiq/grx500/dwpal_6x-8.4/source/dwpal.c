...
DWPAL_Ret dwpal_nl80211_cmd_send(void *context, struct nl_msg *msg, int *cmd_res /*OUT*/, DWPAL_nl80211Callback nlCallback, void *cb_arg)
{
	...
	*cmd_res = 1;
	while (*cmd_res > 0)
	{
		/* TODO: use select() to not get stuck here forever */
	...
	while ( (lineMsg != NULL) && (ret == DWPAL_SUCCESS) )
	{
		...
		while ( (fieldsToParse[i].parsingType != DWPAL_NUM_OF_PARSING_TYPES) && (ret == DWPAL_SUCCESS) )
		{
			...
			else
			{
				...
				field = (void *)(fieldsToParse[i].field + (lineIdx * sizeOfStruct));
...
DWPAL_Ret dwpal_hostap_cmd_send(void *context, const char *cmdHeader, FieldsToCmdParse *fieldsToCmdParse, char *reply /*OUT*/, size_t *replyLen /*IN/OUT*/)
{
	...
	status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s", cmdHeader);
	if (status <= 0)
		console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
			...
			if (fieldsToCmdParse[i].field != NULL)
			{
				switch (fieldsToCmdParse[i].parsingType)
				{
					case DWPAL_STR_PARAM:
						...
						{
							status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s %s", cmd, (char *)fieldsToCmdParse[i].field);
							if (status <= 0)
								console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
						}
						else
						{
							status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s %s%s", cmd, fieldsToCmdParse[i].preParamString, (char *)fieldsToCmdParse[i].field);
							if (status <= 0)
								console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
						}
						break;
					...
					case DWPAL_INT_PARAM:
						...
						if (fieldsToCmdParse[i].preParamString == NULL)
						{
							status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s %d", cmd, *((int *)fieldsToCmdParse[i].field));
							if (status <= 0)
								console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
						}
						else
						{
							status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s %s%d", cmd, fieldsToCmdParse[i].preParamString, *((int *)fieldsToCmdParse[i].field));
							if (status <= 0)
								console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
						}
						break;
					...
					case DWPAL_UNSIGNED_INT_PARAM:
						...
						if (fieldsToCmdParse[i].preParamString == NULL)
						{
							status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s %u", cmd, *((unsigned int *)fieldsToCmdParse[i].field));
							if (status <= 0)
								console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
						}
						else
						{
							status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s %s%u", cmd, fieldsToCmdParse[i].preParamString, *((unsigned int *)fieldsToCmdParse[i].field));
							if (status <= 0)
								console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
						}
						break;
