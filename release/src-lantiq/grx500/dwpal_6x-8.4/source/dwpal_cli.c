...
static bool isCliRunning = true, isContinueListening = true, isWaitForAllEvents = false;
...
static bool resultsPrint(FieldsToParse fieldsToParse[], size_t totalSizeOfArg, size_t sizeOfStruct)
{
	...
	for (k=0; k < totalSizeOfArg; k++)
	{
		...
		if (totalSizeOfArg > 1)
		{
			status = sprintf_s(indexToPrint, sizeof(indexToPrint), "[%d] ", k);
			if (status <= 0)
				console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
		...
		while (fieldsToParse[i].parsingType != DWPAL_NUM_OF_PARSING_TYPES)
		{
			...
			field = (void *)(fieldsToParse[i].field + k * sizeOfStruct);
...
static DWPAL_Ret dwpal_req_beacon_handle(void *localContext, char *VAPName, char *fields[], bool isDwpalExtenderMode)
{
	...
	for (i=0; i < 9; i++)
	{
		status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s %s", cmd, fields[i]);
		if (status <= 0)
			console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
	}
				...
				*/
				...
				else
				{
					...
					else
					{
						...
						{
							status = sprintf_s(cmd, DWPAL_TO_HOSTAPD_MSG_LENGTH, "%s %s", cmd, field);
							if (status <= 0)
								console_printf("%s; sprintf_s failed!\n", __FUNCTION__);
						}
