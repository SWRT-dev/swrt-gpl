...
#ifdef CONFIG_DPP2
static int wps_process_dpp_bootstrap_uri(struct wps_data *wps,
					 struct wps_parse_attr *attr)
{

	if (attr->dpp_bootstrap_uri == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: DPP URI is empty");
		wps->dpp_bootstrap_uri_len = 0;
		return 0;
	}
	wpa_printf(MSG_DEBUG, "WPS: DPP Bootstrap URI %s len %d",
		   attr->dpp_bootstrap_uri,
		   attr->dpp_bootstrap_uri_len);
	if (attr->dpp_bootstrap_uri_len <= sizeof(wps->dpp_bootstrap_uri)) {
		os_memcpy(wps->dpp_bootstrap_uri, attr->dpp_bootstrap_uri,
			  attr->dpp_bootstrap_uri_len);
		wps->dpp_bootstrap_uri_len = attr->dpp_bootstrap_uri_len;
		wps->wps->event_cb(wps->wps->cb_ctx, WPS_EV_RX_DPP_URI,
				(union wps_event_data *)attr->dpp_bootstrap_uri);
	} else {
		wpa_printf(MSG_DEBUG, "WPS: DPP URI len is more than Max URI len");
		return -1;
	}
...
#ifdef CONFIG_DPP2
	    wps_process_dpp_bootstrap_uri(wps, &eattr) ||
#endif /* CONFIG_DPP2 */
