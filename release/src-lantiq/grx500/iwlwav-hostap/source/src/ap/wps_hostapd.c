				 ...
				 union wps_event_data *data)
{
	...
	case WPS_EV_RX_DPP_URI:
		if ( hostapd_dpp_qr_code(hapd, data->rx_dpp_uri.dpp_bootstrap_uri) > 0 )
			hostapd_dpp_enable_cc(hapd, "1");
		else
			wpa_printf(MSG_ERROR, "failed to generate qr_code from received bootstrap_uri in wps");
		wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_RX_DPP_URI " %s", data->rx_dpp_uri.dpp_bootstrap_uri);
		break;
