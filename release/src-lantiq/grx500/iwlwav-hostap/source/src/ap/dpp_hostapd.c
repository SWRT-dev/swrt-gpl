...
int hostapd_dpp_enable_cc(struct hostapd_data *hapd, const char *enable)
{
	unsigned int val = atoi(enable);
	if( val )
		hapd->conf->dpp_configurator_connectivity = 1;
	else
		hapd->conf->dpp_configurator_connectivity = 0;

	if( ieee802_11_update_beacon(hapd) ) {
		wpa_printf(MSG_ERROR, "CTRL: SET_BEACON failed");
		return -1;
	}
	wpa_printf(MSG_DEBUG, "configure cc with %u", val);
	return 0;
}
#endif /* CONFIG_DPP2 */
