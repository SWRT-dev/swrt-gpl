				     ...
				     union wps_event_data *data)
{
	...
	case WPS_EV_AP_PIN_SUCCESS:
		break;
#ifdef CONFIG_DPP2
	case WPS_EV_RX_DPP_URI:
		break;
	case WPS_EV_TX_DPP_URI:
	{
		#define MAX_CHIRP_ITER 100
		#define MAX_CMD_LEN 32
		char chirp_cmd[MAX_CMD_LEN] = {'\0'};
		unsigned int max_id = dpp_next_id(wpa_s->dpp) - 1; /*get max_id, valid id is 1 less then max_id */
		while(max_id) {
			sprintf_s(chirp_cmd, MAX_CMD_LEN, " own=%u iter=%d ", max_id--, MAX_CHIRP_ITER); /*TODO: get own id*/
	 		wpas_dpp_chirp(wpa_s, chirp_cmd);
	 	}
	 	wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_TX_DPP_URI " %s", data->rx_dpp_uri.dpp_bootstrap_uri);
	}
		break;
#endif
	}
