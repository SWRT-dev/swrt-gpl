...
#ifdef CONFIG_DPP2
	if (DPP_VERSION > 1 && wpa_s->key_mgmt == WPA_KEY_MGMT_DPP && wpa_s->current_ssid &&
	    wpa_s->current_ssid->dpp_netaccesskey && wpa_s->current_ssid->dpp_pfs != 2 &&
		...
		wpa_s->dpp_pfs = dpp_pfs_init(wpa_s->current_ssid->dpp_netaccesskey,
					      wpa_s->current_ssid->dpp_netaccesskey_len);
		if (!wpa_s->dpp_pfs) {
