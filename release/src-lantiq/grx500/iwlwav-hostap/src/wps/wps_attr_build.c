...
#ifdef CONFIG_DPP2
int wps_build_bstrap_uri(struct wps_data *wps, struct wpabuf *msg)
{
	wpa_printf(MSG_DEBUG, "WPS:  * DPP Bootstrap URI");

	if (wps->wps->dpp_bootstrap_uri_len == 0)
		return 0;
	wpabuf_put_be16(msg, ATTR_DPP_BOOTSTRAP_URI);
	wpabuf_put_be16(msg, wps->wps->dpp_bootstrap_uri_len);
	wpabuf_put_data(msg, wps->wps->dpp_bootstrap_uri, wps->wps->dpp_bootstrap_uri_len);
	wps->wps->event_cb(wps->wps->cb_ctx, WPS_EV_TX_DPP_URI,
		  (union wps_event_data *)wps->wps->dpp_bootstrap_uri);
