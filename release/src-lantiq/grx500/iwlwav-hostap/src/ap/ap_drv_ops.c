			  ...
			  int dfs_time, int dfs_debug)
{
	...
	if (!hapd->driver || !hapd->driver->set_chan_dfs_state)
		return 0;

	if (!iface->conf->ieee80211h) {
		wpa_printf(MSG_ERROR, "Can't set Channel available, DFS functionality "
			   "is not enabled");
		return -1;
	}

	res = hapd->driver->set_chan_dfs_state(hapd->drv_priv, dfs_state, freq, dfs_time, dfs_debug);
