...
void hostapd_ltq_update_coc(struct hostapd_iface *iface, struct intel_vendor_event_coc *bw_params, size_t data_len)
{
	...
	wpa_printf(MSG_INFO,"BW = %d, IE = %d, iface->fallback_coc_freq = %d, spatial streams = %d fb_sec_chan = %d\n"
			, bw_params->coc_BW, bw_params->coc_omn_IE, iface->fallback_coc_freq, iface->require_omn, iface->fb_coc_sec_chan);
