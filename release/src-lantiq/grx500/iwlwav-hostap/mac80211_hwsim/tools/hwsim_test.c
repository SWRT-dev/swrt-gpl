	       ...
	       u_int8_t tos)
{
	...
	err = memcpy_s(eth->ether_dhost, sizeof(eth->ether_dhost), dst, ETH_ALEN);
	if (EOK != err) {
		wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
		return;	
	}
	err = memcpy_s(eth->ether_shost, sizeof(eth->ether_shost), src, ETH_ALEN);
	if (EOK != err) {
		wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
		return;
	}
...
int main(int argc, char *argv[])
{
	...
	len = strnlen_s(s_ifname, RSIZE_MAX_STR);
	if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, RSIZE_MAX_STR - 1)) {
		goto fail;
	}
	strncpy_s(ifr.ifr_name, sizeof(ifr.ifr_name), s_ifname, len);
	...
	err = memcpy_s(addr1, sizeof(addr1), ifr.ifr_hwaddr.sa_data, ETH_ALEN);
	if (EOK != err) {
		wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
		goto fail;
	}
	...
	memset(&ifr, 0, sizeof(ifr));
	len = strnlen_s(d_ifname, RSIZE_MAX_STR);
	if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, RSIZE_MAX_STR - 1)) {
		goto fail;
	}
	strncpy_s(ifr.ifr_name, sizeof(ifr.ifr_name), d_ifname, len);
	...
	err = memcpy_s(addr2, sizeof(addr2), ifr.ifr_hwaddr.sa_data, ETH_ALEN);
	if (EOK != err) {
		wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
		goto fail;
	}
