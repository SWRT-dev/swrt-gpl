					     ...
					     struct wpa_global *global)
{
	...
	dbus_message_iter_get_basic(&iter, &ifname);
        len = strnlen_s(ifname, IFNAMSIZ);
	if (!IS_VALID_RANGE(len, AP_MIN_VALID_CHAR, IFNAMSIZ - 1)) {
		wpa_printf(MSG_ERROR, "wpas_dbus_global_add_interface - wrong ifname len");
		goto error;
	}
					   ...
					   struct wpa_supplicant *wpa_s)
{
	...
	dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
		 ...
		 */
		ret = sprintf_s(path, WPAS_DBUS_OBJECT_PATH_MAX,
	...
	while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
		...
		if (entry.type == DBUS_TYPE_ARRAY &&
			...
			if (should_quote_opt(entry.key)) {
				size = strnlen_s(entry.str_value, RSIZE_MAX_STR)
				/* Zero-length option check */
				if ((size == 0) || (size == RSIZE_MAX_STR -1))
					goto error;
				size += 3;  /* For quotes and terminator */
				value = os_zalloc(size);
				if (value == NULL)
					goto error;
				ret = sprintf_s(value, size, "\"%s\"",
						  entry.str_value);
				if (ret <= 0) {
					wpa_printf(MSG_ERROR, "%s: sprintf_s error %d", __func__, ret);
					goto error;
					...
					struct wpa_supplicant *wpa_s)
{
	...
	while (wpa_dbus_dict_has_dict_entry(&iter_dict)) {
		...
		if ((entry.array_len <= 0) || (entry.array_len > 65536) ||
		    !strnlen_s(entry.key, entry.array_len)) {
			reply = wpas_dbus_new_invalid_opts_error(
		...
		err = memcpy_s(blob->data, entry.array_len, (u8 *) entry.bytearray_value,
				entry.array_len);
		if (EOK != err) {
				    ...
				    struct wpa_supplicant *wpa_s)
{
	int flush_age = 0;
	const char *sig = dbus_message_get_signature(message);
	rsize_t len = strnlen_s(sig, MAX_SIG_LEN);
