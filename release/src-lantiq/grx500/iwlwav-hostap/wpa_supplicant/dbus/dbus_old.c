				       ...
				       char **bssid)
{
	const unsigned int dev_path_prefix_len =
		strnlen_s(WPAS_DBUS_PATH_INTERFACES "/", sizeof(WPAS_DBUS_PATH_INTERFACES "/"));
	char *obj_path_only;
	char *next_sep;
	...
	if (next_sep != NULL) {
		...
		if (network && net_part) {
			...
			const char *net_name = net_part +
				strnlen_s(WPAS_DBUS_NETWORKS_PART "/", sizeof(WPAS_DBUS_NETWORKS_PART "/"));
			*network = NULL;
			if (strnlen_s(net_name, DBUS_NET_NAME))
				*network = os_strdup(net_name);
		} else if (bssid && bssid_part) {
			/* Deal with a request for a scanned BSSID */
			const char *bssid_name = bssid_part +
				strnlen_s(WPAS_DBUS_BSSIDS_PART "/", sizeof(WPAS_DBUS_BSSIDS_PART "/"));
			if (strnlen_s(bssid_name, DBUS_BSSID_NAME))
...
int wpas_dbus_register_iface(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->dbus_path == NULL)
		return -1;
	ret = sprintf_s(wpa_s->dbus_path, WPAS_DBUS_OBJECT_PATH_MAX,
