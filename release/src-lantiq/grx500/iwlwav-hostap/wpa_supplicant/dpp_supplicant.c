...
#if defined(CONFIG_DPP2) && defined(CONFIG_WPS)
void wpas_generate_prefchan_list(struct wpa_supplicant *wpa_s,
				 char *pref_chan,
				 size_t pref_chan_size)
{
	u8 op_class, channel;
	int res;
	...
	if (is_chan6)
	{
		ieee80211_freq_to_channel_ext(2437, 0, 0, &op_class, &channel);
		res = os_snprintf(pref_chan, pref_chan_size, "%u/%u", op_class, channel);
		if (!os_snprintf_error(pref_chan_size, res)) {
			pref_chan += res;
			pref_chan_size -= res;
		}
	}
	...
	if (mode) {
		bool is_chan44 = false, is_chan149 = false;
		...
		if (is_chan149) {
			ieee80211_freq_to_channel_ext(5745, 0, 0, &op_class, &channel);
			res = os_snprintf(pref_chan, pref_chan_size, "%u/%u", op_class, channel);
			if (!os_snprintf_error(pref_chan_size, res)) {
				pref_chan += res;
				pref_chan_size -= res;
			...
			ieee80211_freq_to_channel_ext(5220, 0, 0, &op_class, &channel);
			res = os_snprintf(pref_chan, pref_chan_size, "%u/%u", op_class, channel);
			if (!os_snprintf_error(pref_chan_size, res)) {
				pref_chan += res;
				pref_chan_size -= res;
	...
	if (mode) {
		for (c = 0; c < mode->num_channels; c++) {
			...
			if ((chan->flag & (HOSTAPD_CHAN_DISABLED |
			     ...
			     chan->freq != 60480)
				continue;
			ieee80211_freq_to_channel_ext(60480, 0, 0, &op_class, &channel);
			res = os_snprintf(pref_chan, pref_chan_size, "%u/%u", op_class, channel);
			if (!os_snprintf_error(pref_chan_size, res)) {
				pref_chan += res;
				pref_chan_size -= res;
...
int wpas_dpp_init(struct wpa_supplicant *wpa_s)
{
	...
	u8 adv_proto_id[7];
#if defined(CONFIG_DPP2) && defined(CONFIG_WPS)
#define PREF_CHAN_LEN 128
#define BOOTSTRAP_CMD_LEN 256
	int bstrap_id;
	const char *uri;
	struct wps_context *wps = wpa_s->wps;
	char pref_chan[PREF_CHAN_LEN];
	char cmd[BOOTSTRAP_CMD_LEN];
#endif /* CONFIG_DPP2 CONFIG_WPS2 */
...
#if defined(CONFIG_DPP2) && defined(CONFIG_WPS)
	wps->dpp_bootstrap_uri_len = 0;
	if (wpa_s->dpp && wpa_s->conf->dpp_wps_bootstrapping_config) {
		wpas_generate_prefchan_list(wpa_s, pref_chan, PREF_CHAN_LEN);
		os_snprintf(cmd, sizeof(cmd), "type=qrcode mac="COMPACT_MACSTR" "
				"chan=%s",
				MAC2STR(wpa_s->own_addr),
				pref_chan);
		bstrap_id = dpp_bootstrap_gen(wpa_s->dpp, cmd);
		if (bstrap_id >= 0) {
			uri = dpp_bootstrap_get_uri(wpa_s->dpp, bstrap_id);
			if (uri) {
				wps->dpp_bootstrap_uri_len = os_snprintf(wps->dpp_bootstrap_uri,
									 DPP_BOOTSTRAP_URI_MAX_LEN, "%s", uri);
				wpa_printf(MSG_DEBUG, "DPP: URI used in WPS M7 %s", wps->dpp_bootstrap_uri);
			}
		}
	}
#endif /* CONFIG_DPP2 CONFIG_WPS*/
