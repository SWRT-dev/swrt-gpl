...
void get_stainfo(int bssidx, int vifidx)
{
	...
	if (vifidx > 0) {
		snprintf(line_buf, sizeof(line_buf), "wl%d.%d_ifname", bssidx, vifidx);
...
int rast_stamon_get_rssi(int bssidx, struct ether_addr *addr)
{
	...
	__get_wlifname(bssidx, 0, athfix);
	ch = shared_get_channel(athfix);
	channf = QCA_DEFAULT_NOISE_FLOOR;
...
void rast_retrieve_static_maclist(int bssidx, int vifidx)
{
	...
	if(bssinfo[bssidx].static_maclist[vifidx] == NULL) {
		...
		bssinfo[bssidx].static_maclist[vifidx] = (struct maclist *)malloc(msize);
		if(bssinfo[bssidx].static_maclist[vifidx] == NULL) {
			return;
		}
	}
...
cprintf("#static_maclist[%d](%s)(%s)\n", static_maclist->count, ether_ntoa(&static_maclist->ea[static_maclist->count]), p);
				static_maclist->count++;
			}
		}
		else
			cprintf("## invalid line (%s)\n", line);
	}
	pclose(fp);
}
