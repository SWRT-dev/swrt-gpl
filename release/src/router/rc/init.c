...
//#if 0
		if(!nvram_match("ed_thresh_dft", "1")) {
			...
			if(!strncmp(nvram_safe_get("territory_code"), "EU", 2)) {
				nvram_set("sb/0/eu_edthresh2g", "-61");
				nvram_set("sb/1/eu_edthresh5g", "-61");
			} else {
				nvram_set("sb/0/ed_thresh2g", "-50");
				nvram_set("sb/1/ed_thresh5g", "-50");
			}	
		}
//#endif
#ifdef RTCONFIG_AMAS
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
	...
	case MODEL_GTAX11000_PRO:
		...
		nvram_set("lan_ifname", "br0");
		if (is_router_mode()) {
			nvram_set("lan_ifnames", "eth1 eth2 eth3 eth4 eth5 eth6 eth7 eth8 eth9");
			nvram_set("wan_ifname", "eth0");
			nvram_set("wan_ifnames", "eth0");
		} else {
			nvram_set("lan_ifnames", "eth0 eth1 eth2 eth3 eth4 eth5 eth6 eth7 eth8 eth9");
			nvram_set("wan_ifnames", "");
			nvram_set("wan_ifname", "");
		}
...
#endif
		{
			nvram_set("sta_phy_ifnames", "eth7 eth8 eth9");
		}
		...
		nvram_set("eth_ifnames", "eth0");
		nvram_set("sta_ifnames", "eth7 eth8 eth9");
...
//#endif
		...
		nvram_set_int("led_group3_red_gpio", 40);	// sw_led_14-pin_40
		nvram_set_int("led_group3_green_gpio", 42);	// sw_led_16-pin_42
		nvram_set_int("led_group3_blue_gpio", 41);	// sw_led_15-pin_41
...
/* [MUST]: Need to Clarify ... */
	...
	else if ((model == MODEL_RTAX53U) || (model == MODEL_RTAX54))
	{
	...
	if ((model == MODEL_RTACRH18)||(model == MODEL_RT4GAC86U)||(model == MODEL_RT4GAX56)||(model == MODEL_RTAX53U)||(model == MODEL_RTAX54))
	{
...
int init_main(int argc, char *argv[])
{
	...
	sigset_t sigset;
	siginfo_t info;
	int rc_check = 0, dev_check = 0, boot_check = 0; //Power on/off test
