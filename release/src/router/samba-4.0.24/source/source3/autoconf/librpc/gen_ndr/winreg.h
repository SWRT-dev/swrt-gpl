...
struct winreg_StringBuf {
	...
	uint16_t size;
	const char *name;/* [length_is(length/2),size_is(size/2),charset(UTF16),unique] */
};

struct winreg_ValNameBuf {
	uint16_t length;/* [value(strlen_m_term(name)*2)] */
	uint16_t size;
	const char *name;/* [unique,charset(UTF16),size_is(size/2),length_is(length/2)] */
};
...
struct winreg_EnumValue {
	struct {
		...
		uint8_t *value;/* [length_is(length?*length:0),size_is(size?*size:0),unique,range(0,0x4000000)] */
