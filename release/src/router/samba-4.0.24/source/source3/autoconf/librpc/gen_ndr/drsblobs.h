...
struct package_PrimaryKerberosString {
	...
	const char * string;/* [relative,subcontext(0),flag(LIBNDR_FLAG_STR_NOTERM|LIBNDR_FLAG_REMAINING),subcontext_size(size)] */
};
...
struct package_PrimaryKerberosKey3 {
	...
	DATA_BLOB *value;/* [relative,subcontext(0),flag(LIBNDR_FLAG_REMAINING),subcontext_size(value_len)] */
};
...
struct package_PrimaryKerberosKey4 {
	...
	uint32_t keytype;
	uint32_t value_len;/* [value((value?value->length:0))] */
	DATA_BLOB *value;/* [subcontext(0),relative,subcontext_size(value_len),flag(LIBNDR_FLAG_REMAINING)] */
};
