...
//	retval += websWrite(wp, "BSSID: %s\t", wl_ether_etoa(&bi->BSSID));
	retval += websWrite(wp, "BSSID: %02X:%02X:%02X:%02X:%02X:%02X\t", bi->BSSID.octet[0], bi->BSSID.octet[1], bi->BSSID.octet[2],
...
wl_status(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	if ((repeater_mode() || psr_mode()) &&
		...
		nvram_pf_match(prefix2, "bss_enabled", "1"))
		strlcpy(ifname, nvram_pf_safe_get(prefix2, "ifname"), sizeof(ifname));
	else
...
wl_sta_info(char *ifname, struct ether_addr *ea)
{
	...
	memcpy(buf + safe_strlen(buf) + 1, (void *)ea, ETHER_ADDR_LEN);
...
ej_wl_status(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
...
#ifdef RTCONFIG_WIFI6E
			(nvram_pf_match(prefix, "nband", "4") ? "6 GHz" :
			(nvram_pf_match(prefix, "nband", "1") ? "5 GHz" : "2.4 GHz"))
#else
	...
	if (nvram_pf_match(prefix, "mode", "wds")){
	...
	if (nvram_pf_match(prefix, "mode", "ap"))
	{
		if (nvram_pf_match(prefix, "lazywds", "1") || nvram_pf_invmatch(prefix, "wds", ""))
			ret += websWrite(wp, "Mode	: Hybrid\n");
		else
			ret += websWrite(wp, "Mode	: AP Only\n");
	}
	else if (nvram_pf_match(prefix, "mode", "wds"))
	{
		ret += websWrite(wp, "Mode	: WDS Only\n");
		return ret;
	}
	else if (nvram_pf_match(prefix, "mode", "sta"))
	{
...
#endif
	else if (nvram_pf_match(prefix, "mode", "wet"))
	{
...
//		return ret;
	...
	for (; first <= last; first++) {
		...
		memcpy((char *)(buf + safe_strlen(buf) + 1), (char*)&chanspec_arg, sizeof(chanspec_arg));
...
wds_list:
#endif
	if ((nvram_pf_match(prefix, "mode", "ap") || nvram_pf_match(prefix, "mode", "wds")) && !nvram_pf_match(prefix, "wds", "")) {
		ret += websWrite(wp, "\n");
		ret += websWrite(wp, "Bridge List                             \n");
		ret += websWrite(wp, "----------------------------------------\n");
		ret += websWrite(wp, "%-4s%-18s%-7s\n", "idx", "MAC", "Status");

		strlcpy(macs, nvram_pf_safe_get(prefix, "ifname"), sizeof(macs));
		i = 1;
		foreach(mac, macs, next) {
			...
			if (safe_atoi(nvram_pf_safe_get(prefix, "wds_timeout")) &&
...
ej_wl_status_2g(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for (ii = 0; ii < DEV_NUMIFS; ii++) {
		...
		if (temp && safe_strlen(temp) > 0)
		{
			retval += ej_wl_status(eid, wp, argc, argv, safe_atoi(temp));
			retval += websWrite(wp, "\n");
		}
	}
...
static int ej_wl_channel_list(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	snprintf(prefix, sizeof(prefix), "wl%d_", unit);
	strlcpy(ifname, nvram_pf_safe_get(prefix, "ifname"), sizeof(ifname));
	strlcpy(chlist, nvram_pf_safe_get(prefix, "chlist"), sizeof(chlist));
	...
	for (i = 0; i < dtoh32(list->count) && i < IW_MAX_FREQUENCIES; i++) {
		...
		if (safe_strlen(tmp2))
			nvram_pf_set(prefix, "chlist", tmp2);
	}
ERROR:
...
ej_wl_chanspecs(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (safe_strlen(wf_chspec_ntoa(c, chanbuf)) == 0) {
		...
		if (strlen(tmp2))
			nvram_pf_set(prefix, "chansps", tmp2);
	}
...
ej_wl_channel_list_2g(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for(unit = 0; unit < num_of_wl_if(); unit++) {
		snprintf(prefix, sizeof(prefix), "wl%d_", unit);
		if(WL_NBAND_2G == nvram_pf_get_int(prefix, "nband"))
...
ej_wl_channel_list_5g(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for(unit = 0; unit < num_of_wl_if(); unit++) {
		snprintf(prefix, sizeof(prefix), "wl%d_", unit);
		if(WL_NBAND_5G == nvram_pf_get_int(prefix, "nband"))
			return ej_wl_channel_list(eid, wp, argc, argv, unit);
	}
...
ej_wl_channel_list_5g_2(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for(unit = 0; unit < num_of_wl_if(); unit++) {
		snprintf(prefix, sizeof(prefix), "wl%d_", unit);
		if( WL_NBAND_5G == nvram_pf_get_int(prefix, "nband") ||
		    WL_NBAND_6G == nvram_pf_get_int(prefix, "nband")) {
			if(!count) {
				count++;
				continue;
			}
...
ej_wl_channel_list_6g(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for(unit = 0; unit < num_of_wl_if(); unit++) {
		snprintf(prefix, sizeof(prefix), "wl%d_", unit);
		if(WL_NBAND_6G == nvram_pf_get_int(prefix, "nband"))
			return ej_wl_channel_list(eid, wp, argc, argv, unit);
	}
...
int
ej_wl_chanspecs_6g_2(int eid, webs_t wp, int argc, char_t **argv)
{
	char prefix[16], tmp[32];
	int unit, count = 0;
	for(unit = 0; unit < num_of_wl_if(); unit++) {
...
static int ej_wl_rssi(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	snprintf(prefix, sizeof(prefix), "wl%d_", unit);
	strlcpy(ifname, nvram_pf_safe_get(prefix, "ifname"), sizeof(ifname));
	mode = nvram_pf_safe_get(prefix, "mode");
	sta = !strcmp(mode, "sta");
	wet = !strcmp(mode, "wet");
...
static int ej_wl_rate(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	if (nvram_pf_match(prefix, "mode", "wet")) {
...
ej_wl_rate_6g_2(int eid, webs_t wp, int argc, char_t **argv)
{
	int i = 0;
	for(i=0; i<(sizeof(wl_band_list)/sizeof(wl_band_list[0])) && wl_band_list[i][0] != '\0'; i++){
		if(strncmp("6g2", wl_band_list[i], 3) == 0)
			break;
	}
	return ej_wl_rate(eid, wp, argc, argv, i);
}
...
int
ej_wl_chipnum_6g_2(int eid, webs_t wp, int argc, char **argv)
{
	char prefix[16], tmp[32];
	int unit, count = 0;
	for(unit = 0; unit < num_of_wl_if(); unit++) {
...
int wl_wps_info(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	if(hook_get_json == 1){ //return json format
		struct json_object *wps_info_obj = json_object_new_object();

			//0. WSC Status
		if (nvram_pf_match(prefix, "wps_mode", "enabled"))
		{
			json_object_object_add(wps_info_obj, "WSC_Status", json_object_new_string(getWscStatusStr()));
		}
		else
			json_object_object_add(wps_info_obj, "WSC_Status", json_object_new_string("Not used"));

		//1. WPSConfigured
		if (!wps_is_oob())
			json_object_object_add(wps_info_obj, "WPSConfigured", json_object_new_string("Yes"));
		else
			json_object_object_add(wps_info_obj, "WPSConfigured", json_object_new_string("No"));

		//2. WPSSSID
		{
			memset(tmpstr, 0, sizeof(tmpstr));
			char_to_ascii(tmpstr, nvram_pf_safe_get(prefix, "ssid"));
			json_object_object_add(wps_info_obj, "WPSSSID", json_object_new_string(tmpstr));
		}

		{
			memset(tmpstr, 0, sizeof(tmpstr));
			getWPSAuthMode(unit, tmpstr, sizeof(tmpstr));
			json_object_object_add(wps_info_obj, "WPSAuthMode", json_object_new_string(tmpstr));
		}
		...
		//4. EncrypType
		{
			...
			json_object_object_add(wps_info_obj, "EncrypType", json_object_new_string(tmpstr));
		}
		...
		//5. DefaultKeyIdx
		{
			...
			json_object_object_add(wps_info_obj, "DefaultKeyIdx", json_object_new_string(tmpstr));
		}

		//6. WPAKey
#if 0
		//hide for security
		if (!strlen(nvram_pf_safe_get(prefix, "wpa_psk")))
			json_object_object_add(wps_info_obj, "WPAKey", json_object_new_string("None"));
		else
		{
			memset(tmpstr, 0, sizeof(tmpstr));
			char_to_ascii(tmpstr, nvram_pf_safe_get(prefix, "wpa_psk"));
			json_object_object_add(wps_info_obj, "WPAKey", json_object_new_string(tmpstr));
		}
#else
#endif
		...
		//7. AP PIN Code
		{
			...
			json_object_object_add(wps_info_obj, "AP_PIN_Code", json_object_new_string(tmpstr));
		}
		...
		if (!strlen(nvram_pf_safe_get(prefix, "wpa_psk")))
		{
			json_object_object_add(wps_info_obj, "Saved_WPAKey", json_object_new_string("None"));
		}
		else
		{
			memset(tmpstr, 0, sizeof(tmpstr));
			char_to_ascii(tmpstr, nvram_pf_safe_get(prefix, "wpa_psk"));
			json_object_object_add(wps_info_obj, "Saved_WPAKey", json_object_new_string(tmpstr));
		}
#else
#endif
		//9. WPS enable?
		if (!strcmp(nvram_pf_safe_get(prefix, "wps_mode"), "enabled"))
			json_object_object_add(wps_info_obj, "WPS_enable", json_object_new_string("1"));
		else
			json_object_object_add(wps_info_obj, "WPS_enable", json_object_new_string("0"));

		//A. WPS mode
		wps_sta_pin = nvram_safe_get("wps_sta_pin");
		if (strlen(wps_sta_pin) && strcmp(wps_sta_pin, "00000000"))
			json_object_object_add(wps_info_obj, "WPS_mode", json_object_new_string("1"));
		else
			json_object_object_add(wps_info_obj, "WPS_mode", json_object_new_string("2"));

		//B. current auth mode
		json_object_object_add(wps_info_obj, "current_auth_mode", json_object_new_string(nvram_pf_safe_get(prefix, "auth_mode_x")));

		//C. WPS band
		json_object_object_add(wps_info_obj, "WPS_band", json_object_new_string(nvram_safe_get("wps_band_x")));

		retval += websWrite(wp, "%s", json_object_to_json_string(wps_info_obj));

		if(wps_info_obj)
			json_object_put(wps_info_obj);
	}
	else // return XML format
	{
		retval += websWrite(wp, "<wps>\n");
...
ej_wps_info_2g(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for(unit = 0; unit < num_of_wl_if(); unit++) {
		snprintf(prefix, sizeof(prefix), "wl%d_", unit);
		if(WL_NBAND_2G == nvram_pf_get_int(prefix, "nband"))
			return wl_wps_info(eid, wp, argc, argv, unit);
	}
...
ej_SiteSurvey(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	params->scan_type = (nvram_pf_match(prefix, "reg_mode", "h") && !is_psta(unit)) ? WL_SCANFLAGS_PASSIVE : 0;
	params->nprobes = -1;
...
ap_list:
	...
	for (i = 0; i < ap_count; i++)
	{
		...
		if (safe_strlen(apinfos[i].SSID) == 0)
			retval += websWrite(wp, "\"\", ");
		else
		{
...
int ej_wl_auth_list(int eid, webs_t wp, int argc, char_t **argv) {
	int unit = 0;
	char prefix[] = "wlXXXXXXXXXX_", prefix2[] = "wlXXXXXXXXXX_";
	char ifname[IFNAMSIZ] = { 0 };
	struct maclist *auth = NULL;
	int mac_list_size;
	int i, firstRow = 1;
...
wl_get_scan_results_escan(char *ifname, chanspec_t chanspec, int ctl_ch, int ctl_ch_tmp)
{
	...
	params->params.scan_type = (nvram_pf_match(prefix, "reg_mode", "h") && !is_psta(unit)) ? WL_SCANFLAGS_PASSIVE : 0;
#else
	params->params.scan_type = (nvram_pf_match(prefix, "reg_mode", "h")) ? WL_SCANFLAGS_PASSIVE : 0;
#endif
...
wl_get_scan_results(char *ifname, chanspec_t chanspec, int ctl_ch, int ctl_ch_tmp)
{
	...
	params->scan_type = (nvram_pf_match(prefix, "reg_mode", "h") && !is_psta(unit)) ? WL_SCANFLAGS_PASSIVE : 0;
#else
	params->scan_type = (nvram_pf_match(prefix, "reg_mode", "h")) ? WL_SCANFLAGS_PASSIVE : 0;
#endif
	params->nprobes = -1;
	params->active_time = -1;
	params->passive_time = -1;
...
#ifdef __CONFIG_DHDAP__
	if (is_dhd && !nvram_pf_match(prefix, "mode", "wds")) {
...
ej_wl_auth_psta(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if (wl_ioctl(ifname, WLC_GET_SSID, &ssid, sizeof(ssid)))
		goto PSTA_ERR;
	else if (strncmp(nvram_pf_safe_get(prefix, "ssid"), (const char *) ssid.SSID, strlen(nvram_pf_safe_get(prefix, "ssid"))))
		goto PSTA_ERR;
