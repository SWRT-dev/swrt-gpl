 ...
 *
 * reference from
 * arch/arm64/boot/dts/mediatek/mt7981.dts
 */
...
int mt7981_mt7531_reg_read(unsigned int phy, unsigned int reg, unsigned int *value)
{
	struct ifreq ifr;
...
int mt7981_mt7531_reg_write(unsigned int phy, unsigned int reg, unsigned int value)
{
	struct ifreq ifr;
	struct mtk_mii_ioctl_data mii;
...
int mt7981_mt7531_phy_read(unsigned int port_num, unsigned int reg, unsigned int *value)
{
	...
	mt7981_mt7531_reg_write(0x1f, REG_SSC_PHY_IAC, reg_value);
	while (1) {
		mt7981_mt7531_reg_read(0x1f, REG_SSC_PHY_IAC, &reg_value);
...
int mt7981_mt7531_phy_write(unsigned int port_num, unsigned int reg, unsigned int value)
{
	unsigned int reg_value;
	int loop_cnt;
	int op_busy;
 ...
 *     -1:	invalid parameter
 */
int mt7981_mt7531_vlan_set(int vtype, char *upstream_if, int vid, int prio, unsigned int mbr, unsigned int untag)
{
...
static void get_phy_info(unsigned int phy, unsigned int *link, unsigned int *speed, phy_info *info)
{
	...
	if (phy >= 0 && phy <= 4) {
		mt7981_mt7531_reg_read(0x1f, (REG_MAC_PMSR_P0 + 0x100*phy), &value);
		l = value & 0x1;
		s = (value >> 2) & 0x3;
	}
	else {
		mt7981_mt7531_reg_read(phy, 0x1, &value);
		l = (value >> 2) & 0x1;
		if (l) {
			mt7981_mt7531_reg_read(phy, 0x18, &value);
			s = value & 0x7;
		}
	}
 ...
 *  otherwise:	fail
 */
static int get_mt7981_mt7531_vport_info(unsigned int vport, unsigned int *link, unsigned int *speed, phy_info *info)
{
	int phy;
 ...
 */
static void get_mt7981_mt7531_phy_linkStatus(unsigned int mask, unsigned int *linkStatus)
{
	int i;
	...
	for (i = 0; m > 0 && !value; ++i, m >>= 1) {
		...
		get_mt7981_mt7531_vport_info(i, &value, NULL, NULL);
	}
	*linkStatus = value;
}
 ...
 * 	6:	LAN3+LAN4
 */
static void config_mt7981_mt7531_LANWANPartition(int type)
{
...
static void get_mt7981_mt7531_Port_Speed(unsigned int port_mask, unsigned int *speed)
{
	int i;
	unsigned int value = 0, m;
	...
	for (i = 0; m; ++i, m >>= 1) {
		...
		get_mt7981_mt7531_vport_info(i, NULL, &value, NULL);
	}
	*speed = value;
}

static void get_mt7981_mt7531_WAN_Speed(unsigned int *speed)
{
	get_mt7981_mt7531_Port_Speed(get_wan_port_mask(0) | get_wan_port_mask(1), speed);
}

static void link_down_up_mt7981_mt7531_PHY(unsigned int vpmask, int status)
{
	int vport, phy;
	unsigned int value = 0, m;
	...
	for (vport = 0, m = vpmask; m; ++vport, m >>= 1) {
		...
		if (phy >= 0 && phy <= 4)
			mt7981_mt7531_phy_read(phy, 0x0, &value);
		else
			mt7981_mt7531_reg_read(phy, 0x0, &value);
		if (!status)
			value |= 0x0800; /* power down PHY */
		else
			value &= 0xf7ff; /* power up PHY */
		if (phy >= 0 && phy <= 4)
			mt7981_mt7531_phy_write(phy, 0x0, value);
		else
			mt7981_mt7531_reg_write(phy, 0x0, value);
	}
...
static void create_Vlan(int bitmask)
{
	...
	mt7981_mt7531_vlan_set(vtype, upstream_if, vid, prio, conv_mbr, conv_untag);
}

int mt7981_mt7531_ioctl(int val, int val2)
{
	...
	case 3:
		...
		break;
	case 8:
		config_mt7981_mt7531_LANWANPartition(val2);
		break;
	case 13:
		get_mt7981_mt7531_WAN_Speed(&value2);
		printf("WAN speed : %u Mbps\n", value2);
		break;
	case 14: // Link up LAN ports
		link_down_up_mt7981_mt7531_PHY(get_lan_port_mask(), 1);
		break;
	case 15: // Link down LAN ports
		link_down_up_mt7981_mt7531_PHY(get_lan_port_mask(), 0);
		break;
	case 16: // Link up ALL ports
		link_down_up_mt7981_mt7531_PHY(wanlanports_mask, 1);
		break;
	case 17: // Link down ALL ports
		link_down_up_mt7981_mt7531_PHY(wanlanports_mask, 0);
		break;
	case 21:
		break;
	case 22:
	...
	case 114: // link up WAN ports
		for (i = WAN_UNIT_FIRST; i <= max_wan_unit; ++i)
			link_down_up_mt7981_mt7531_PHY(get_wan_port_mask(i), 1);
		break;
	case 115: // link down WAN ports
		for (i = WAN_UNIT_FIRST; i <= max_wan_unit; ++i)
			link_down_up_mt7981_mt7531_PHY(get_wan_port_mask(i), 0);
		break;
...
int config_rtkswitch(int argc, char *argv[])
{
	...
	return mt7981_mt7531_ioctl(val, val2);
}
...
unsigned int rtkswitch_wanPort_phyStatus(int wan_unit)
{
	...
	get_mt7981_mt7531_phy_linkStatus(get_wan_port_mask(wan_unit), &status);
...
unsigned int rtkswitch_lanPorts_phyStatus(void)
{
	...
	get_mt7981_mt7531_phy_linkStatus(get_lan_port_mask(), &status);
...
unsigned int rtkswitch_WanPort_phySpeed(void)
{
	...
	get_mt7981_mt7531_WAN_Speed(&speed);
...
void ATE_port_status(int verbose, phy_info_list *list)
{
	...
	for (i = 0; i < port_mapping.count; i++) {
		...
		pS.link[i] = 0;
		pS.speed[i] = 0;
		get_mt7981_mt7531_vport_info(port_mapping.port[i].phy_port_id, &pS.link[i], &pS.speed[i], list ? &list->phy_info[i] : NULL);
		if (list) {
			...
			if (pS.link[i] == 1 && !list->status_and_speed_only) {
				// TODO not complete
				//get_mt7981_mt7531_port_mib(port_mapping.port[i].phy_port_id, &list->phy_info[i]);
			}
...
#else
	for (i = 0; i < NR_WANLAN_PORT; i++) {
		get_mt7981_mt7531_vport_info(lan_id_to_vport_nr(i), &pS.link[i], &pS.speed[i], NULL);
	}
