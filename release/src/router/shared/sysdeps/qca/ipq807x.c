 ...
 * port.  If bled need to know TX/RX statistics of LAN1~2, WAN1,
...
static const unsigned int stb_to_mask[7] = { 
					0,
	(1U << LAN1_PORT),
	(1U << LAN2_PORT),
	(1U << LAN3_PORT),
...
static unsigned int wanlanports_mask = ((1U << WAN_PORT) | \
...
static int n56u_to_model_port_mapping[] = {
	...
	WAN_PORT,	//0000 0000 0010 WAN
	LAN1_PORT,	//0000 0000 0100 LAN1
	LAN2_PORT,	//0000 0000 1000 LAN2
	LAN3_PORT,	//0000 0001 0000 LAN3
#else
	LAN4_PORT,	//0000 0000 0001 LAN4
 ...
 *     -1:	invalid parameter
 */
int qca8075_vlan_set(int vtype, char *upstream_if, int vid, int prio, unsigned int mbr, unsigned int untag)
{
 ...
 *  otherwise:	fail
 */
static int get_qca8075_vport_info(unsigned int vport, unsigned int *link, unsigned int *speed, phy_info *info)
{
	int phy;
 ...
 *  non-zero:	linkrate.
 */
static int get_qca8075_phy_linkStatus(unsigned int mask, unsigned int *linkStatus)
{
	int i,t,speed=0;
	...
	for (i = 0; m > 0 && !value; ++i, m >>= 1) {
		...
		get_qca8075_vport_info(i, &value, (unsigned int*) &t, NULL);
		value &= 0x1;
	}
	*linkStatus = value;
 ...
 * 	6:	LAN3+LAN4
 */
static void config_qca8075_LANWANPartition(int type)
{
	build_wan_lan_mask(type, 0);
	reset_qca_switch();
	dbg("%s: LAN/WAN1 portmask %08x/%08x Upstream %s (unit %d)\n",
		__func__, lan_mask, nvram_get_int("wanports_mask"),
		get_wan_base_if(), __get_upstream_wan_unit());
}

static void get_qca8075_WAN_Speed(unsigned int *speed)
{
	int i, v = -1, t;
	unsigned int m;
	...
	for (i = 0; m; ++i, m >>= 1) {
		...
		get_qca8075_vport_info(i, NULL, (unsigned int*) &t, NULL);
		t &= 0x3;
		if (t > v)
			v = t;
	}
 ...
 */
static void link_down_up_qca8075_PHY(unsigned int vpmask, int status)
{
	int vport, phy, r;
	unsigned int m;
	char iface[IFNAMSIZ];
 ...
 * @return:	0 for success
     non-zero for fail
 */
static int get_qca8075_port_mib(unsigned int port, phy_info *info)
{
	int fd;
...
static void initialize_Vlan(int stb_bitmask)
{
	...
	dbg("%s: LAN/WAN/ portmask %08x/%08x Upstream %s (unit %d)\n",
		__func__, lan_mask, nvram_get_int("wanports_mask"),
		get_wan_base_if(), __get_upstream_wan_unit());
	reset_qca_switch();
}
...
static void create_Vlan(int bitmask)
{
	...
	qca8075_vlan_set(vtype, upstream_if, vid, prio, mbr_qca, untag_qca);
}

int qca8075_ioctl(int val, int val2)
{
	...
	case 3:
		...
		break;
	case 8:
		config_qca8075_LANWANPartition(val2);
		break;
	case 13:
		get_qca8075_WAN_Speed(&value2);
		printf("WAN speed : %u Mbps\n", value2);
		break;
	case 14: // Link up LAN ports
		link_down_up_qca8075_PHY(get_lan_port_mask(), 1);
		break;
	case 15: // Link down LAN ports
		link_down_up_qca8075_PHY(get_lan_port_mask(), 0);
		break;
	case 16: // Link up ALL ports
		link_down_up_qca8075_PHY(wanlanports_mask, 1);
		break;
	case 17: // Link down ALL ports
		link_down_up_qca8075_PHY(wanlanports_mask, 0);
		break;
	case 27:
		reset_qca_switch();
		break;
	...
	case 114: // link up WAN ports
		for (i = WAN_UNIT_FIRST; i <= max_wan_unit; ++i)
			link_down_up_qca8075_PHY(get_wan_port_mask(i), 1);
		break;
	case 115: // link down WAN ports
		for (i = WAN_UNIT_FIRST; i <= max_wan_unit; ++i)
			link_down_up_qca8075_PHY(get_wan_port_mask(i), 0);
		break;
...
rtkswitch_Port_phyStatus(unsigned int port_mask)
{
	...
	get_qca8075_phy_linkStatus(port_mask, &status);
...
rtkswitch_Port_phyLinkRate(unsigned int port_mask)
{
	...
	speed=get_qca8075_phy_linkStatus(port_mask, &status);
...
rtkswitch_wanPort_phyStatus(int wan_unit)
{
	...
	get_qca8075_phy_linkStatus(get_wan_port_mask(wan_unit), &status);
...
rtkswitch_lanPorts_phyStatus(void)
{
	...
	get_qca8075_phy_linkStatus(get_lan_port_mask(), &status);
...
void ATE_port_status(int verbose, phy_info_list *list)
{
	...
	for (i = 0; i < port_mapping.count; i++) {
		...
		pS.link[i] = 0;
		pS.speed[i] = 0;
		get_qca8075_vport_info(port_mapping.port[i].phy_port_id, &pS.link[i], &pS.speed[i], list ? &list->phy_info[i] : NULL);
		...
		if (list) {
			...
			/*if (pS.link[i] == 1 && !list->status_and_speed_only)
				get_qca8075_port_mib(port_mapping.port[i].phy_port_id, &list->phy_info[i]);*/
	...
	for (i = 0; i < NR_WANLAN_PORT; i++) {
		get_qca8075_vport_info(lan_id_to_vport_nr(i), &pS.link[i], &pS.speed[i], list ? &list->phy_info[i] : NULL);
...
{
	char *ipq807x_p1_8023az[] = { "ssdk_sh", "port", "ieee8023az", "set", "1", "disable", NULL };
...
#if defined(XMAX3600)
		.count = 4,
		.port[0] = { .phy_port_id = WAN_PORT, .ext_port_id = WAN_PORT, .label_name = "W0", .cap = PHY_PORT_CAP_WAN, .max_rate = 1000, .ifname = "eth1", .flag = 0, .seq_no = -1, .ui_display = NULL },
		.port[1] = { .phy_port_id = LAN1_PORT, .ext_port_id = LAN1_PORT, .label_name = "L1", .cap = PHY_PORT_CAP_LAN, .max_rate = 1000, .ifname = "eth2", .flag = 0, .seq_no = -1, .ui_display = NULL },
		.port[2] = { .phy_port_id = LAN2_PORT, .ext_port_id = LAN2_PORT, .label_name = "L2", .cap = PHY_PORT_CAP_LAN, .max_rate = 1000, .ifname = "eth3", .flag = 0, .seq_no = -1, .ui_display = NULL },
		.port[3] = { .phy_port_id = LAN3_PORT, .ext_port_id = LAN3_PORT, .label_name = "L3", .cap = PHY_PORT_CAP_LAN, .max_rate = 1000, .ifname = "eth4", .flag = 0, .seq_no = -1, .ui_display = NULL }
#else
