...
static int parse_args(int argc, char **argv, int cmd, struct ip6_tnl_parm2 *p)
{
	...
	while (argc > 0) {
		...
		} else if (strcmp(*argv, "fmrs") == 0){ /*MAP-E FMRS*/
			/*  fmrs IPV6PREFIX/PREFIX6LEN, IPV4PREFIX/PREFIX4LEN,EALEN/OFFSET */
			NEXT_ARG();
			char ip6buf[48];
			char ip4buf[16];
			unsigned int ip4len, ip6len, ealen, offset = 6;
			if( sscanf(*argv, "%47[^/]/%u,%15[^/]/%u,%u,%u", 
				ip6buf, &ip6len, ip4buf, &ip4len, &ealen, &offset) < 5){
				invarg("\"fmrs\" format is unknown.", *argv);
			}

			struct in6_addr ip6prefix;
			struct in_addr ip4prefix;
			if(inet_pton(AF_INET6, ip6buf, &ip6prefix) != 1 ){
				invarg("ip6prefix in \"fmrs\" format is error.", *argv);
			}
			
			if(inet_pton(AF_INET, ip4buf, &ip4prefix) != 1){
				invarg("ip4prefix in \"fmrs\" format is error.", *argv);
			}
			
			struct __ip6_tnl_fmr *fmrP, *nextFmrP = NULL;
			fmrP = (struct __ip6_tnl_fmr *)malloc(sizeof(struct __ip6_tnl_fmr));
			if(fmrP == NULL){
				fprintf(stderr, "[ip6tunnel] malloc for struct __ip6_tnl_fmr fail!");
				while(fmrsList){
					nextFmrP = fmrsList->next;
					free(fmrsList);
					fmrsList = nextFmrP;
				}
				exit(-1);
			}
			memset(fmrP, 0x00, sizeof(struct __ip6_tnl_fmr));
			memcpy(&fmrP->rule.ip6_prefix, &ip6prefix, sizeof(struct in6_addr));
			memcpy(&fmrP->rule.ip4_prefix, &ip4prefix, sizeof(struct in_addr));
			fmrP->rule.ip6_prefix_len = ip6len;
			fmrP->rule.ip4_prefix_len = ip4len;
			fmrP->rule.ea_len = ealen;
			fmrP->rule.offset = offset;

			if(fmrsList == NULL)
				fmrsList = fmrP;
			else{
				nextFmrP = fmrsList;
				while( nextFmrP->next != NULL )
					nextFmrP = nextFmrP->next;
				nextFmrP->next = fmrP;
			}
...
/*MAP-E FMR*/
static int do_add_fmrs(const char *basedev,  void *parm)
{
	struct ip6_tnl_parm2 *p = (struct ip6_tnl_parm2 *)parm;
	while(fmrsList){
		struct __ip6_tnl_fmr *next = fmrsList->next;
		struct ipv6_tnl_fmr fmr;
	
		memset(&fmr, 0x00, sizeof(fmr));
		memcpy(&fmr.laddr, &p->laddr, sizeof(struct in6_addr));
		memcpy(&fmr.raddr, &p->raddr, sizeof(struct in6_addr));
		memcpy(&fmr.rule, &fmrsList->rule ,sizeof(fmr.rule));
		
		tnl_add_fmrs_ioctl(SIOCSETFMRSRULE, basedev,p->name, &fmr);
		free(fmrsList);
		fmrsList = next;		
	}
	return 0;
...
static int do_add(int cmd, int argc, char **argv)
{
	...
	ret = tnl_add_ioctl(cmd, basedev, p.name, &p);
	do_add_fmrs(basedev, &p);
