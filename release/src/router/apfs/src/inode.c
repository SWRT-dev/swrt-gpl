...
#else
	stat->btime = ns_to_timespec64(timespec_to_ns(&ai->i_crtime));
#endif
...
struct inode *apfs_new_inode(struct inode *dir, umode_t mode, dev_t rdev)
{
	...
	ai->i_crtime = ns_to_timespec(timespec64_to_ns(&now));
...
#endif
{
	...
	if (resizing && iattr->ia_size > APFS_MAX_FILE_SIZE)
		...
		((LINUX_VERSION_CODE >= KERNEL_VERSION(4, 9, 0)) && LINUX_VERSION_CODE < KERNEL_VERSION(5, 12, 0))
	err = setattr_prepare(dentry, iattr);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0) && LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
