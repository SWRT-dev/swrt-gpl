...
function popClientListEditTable(event) {
	...
	code += '</div>';
	code += '<div style="margin-top:10px;">';
	code += '<input id="card_client_ip6addr_field" type="text" value="" class="input_32_table client_input_text_disabled" disabled>';
	code += '</div>';
	code += '<div style="margin-top:10px;">';
...
function create_clientlist_card(title, id, obj_width) {
	let cardCode = "";
	cardCode += "<div class='card client_list'>" +
		"<header class='card-header'>" +
		"<a href='#collapse_" + id + "_list' data-bs-toggle='collapse' aria-controls='collapse_" + id + "_list' aria-expanded='true'><i class='arrow_icon arrow_up'></i>" +
		"<span class='title'>" + title + "</span>" +
		"</a>" +
		"</header>";
	cardCode += "<div id='collapse_" + id + "_list' class='collapse show'>" +
		"<table class='table' width='100%'><thead class='client-list-sm'><tr id='tr_" + id + "_title' class='table-header'>";
	if (!(isSwMode('mb') || isSwMode('ew')))
		cardCode += "<th width=" + obj_width[0] + " onclick='sorter.addBorder(this);sorter.doSorter(5, \"num\", \"" + id + "_list\");' style='cursor:pointer;'><#wan_interface#></th>";
	cardCode += "<th class='IE8HACK' width=" + obj_width[1] + "><#Client_Icon#></th>";

	cardCode += "<th class='client-list-sm text-left' width=" + obj_width[2] + " onclick='sorter.addBorder(this);sorter.doSorter(2, \"str\", \"" + id + "_list\");' style='cursor:pointer;'><#ParentalCtrl_username#></th>";
	cardCode += "<th class='client-list-xl text-left' width=" + obj_width[3] + " onclick='sorter.addBorder(this);sorter.doSorter(3, \"num\", \"" + id + "_list\");' style='cursor:pointer;'><#vpn_client_ip#></th>";
	cardCode += "<th class='client-list-xl' width=" + obj_width[4] + " onclick='sorter.addBorder(this);sorter.doSorter(4, \"str\", \"" + id + "_list\");' style='cursor:pointer;'><#ParentalCtrl_hwaddr#></th>";

	if (isSupport("stainfo") && !(isSwMode('mb') || isSwMode('ew'))) {
		cardCode += "<th width=" + obj_width[5] + " onclick='sorter.addBorder(this);sorter.doSorter(6, \"num\", \"" + id + "_list\");' style='cursor:pointer;' title='The transmission rates of your wireless device'>Tx</th>";/*untranslated*/
		cardCode += "<th width=" + obj_width[6] + " onclick='sorter.addBorder(this);sorter.doSorter(7, \"num\", \"" + id + "_list\");' style='cursor:pointer;' title='The receive rates of your wireless device'>Rx</th>";/*untranslated*/
		cardCode += "<th width=" + obj_width[7] + " onclick='sorter.addBorder(this);sorter.doSorter(8, \"num\", \"" + id + "_list\");' style='cursor:pointer;'><#Access_Time#></th>";
	}
	cardCode += "</tr></thead></table>";
	cardCode += "<div id='clientlist_" + id + "_list_Block'></div>";
	cardCode += "</div></div>";
	return cardCode;
}
...
function create_clientlist_listview() {
	...
	var obj_width_map = [["7%", "7%", "25%", "20%", "15%", "8%", "8%", "10%"]];
	var obj_width = obj_width_map[0];
	var wl_colspan = obj_width.length;

	var code = "";

	var drawSwitchMode = function(mode) {
		var drawSwitchModeHtml = "";
		drawSwitchModeHtml+="<div class='switch_block_main'><div class='switch_block'><div class='segmented_picker_label'><#Statistic_show_type#></div></div>"

		drawSwitchModeHtml += "<div class='segmented_picker'>";

		if(mode == "All") {
			drawSwitchModeHtml += "<div class='segmented_picker_option active'>";
			drawSwitchModeHtml += "<div class='block_filter_name'><#All#></div>";
			drawSwitchModeHtml += "</div>";
			drawSwitchModeHtml += "<div class='segmented_picker_option' style='cursor:pointer' onclick='changeClientListViewMode();'>";
			drawSwitchModeHtml += "<div class='block_filter_name' >" + (isSupport("mtlancfg") ? "<#Network#>" : "<#wan_interface#>") + "</div>";
			drawSwitchModeHtml += "</div>";
		}
		else {
			drawSwitchModeHtml += "<div class='segmented_picker_option' style='cursor:pointer' onclick='changeClientListViewMode();'>";
		...
		drawSwitchModeHtml += "</div></div>";
		return drawSwitchModeHtml;
	};
	var drawSwitchMode_b = function(mode) {
		var drawSwitchModeHtml = "";
		...
		if(mode == "All") {
			...
			drawSwitchModeHtml += "<div class='block_filter_name' onclick='changeClientListViewMode();'>" + (isSupport("mtlancfg") ? "<#Network#>" : "<#wan_interface#>") + "</div>";
			drawSwitchModeHtml += "</div>";
		}
		else {
			...
			drawSwitchModeHtml += "<div class='block_filter_name' style='color:#93A9B1;'>" + (isSupport("mtlancfg") ? "<#Network#>" : "<#wan_interface#>") + "</div>";
			drawSwitchModeHtml += "</div>";
		}
		drawSwitchModeHtml += "</div>";
		return drawSwitchModeHtml;
	...
	switch (clienlistViewMode) {
		case "All" :
			code += create_clientlist_card("<#Clientlist_All_List#>", "all", obj_width);
			break;
		case "ByInterface" :

			code += create_clientlist_card("<#tm_wired#>", "wired", obj_width);
			...
			if(!isSupport("noWiFi")){
				if((isSupport("smart_connect") || isSupport("bandstr")) && smart_connect_x == "1"){
					...
					code += create_clientlist_card(tr_title,"sc",obj_width);
				}
				else{
					for(var i = 0; i < wl_nband_title.length; i += 1) {
						...
						code += create_clientlist_card(tr_title, "wl" + wl_map[wl_nband_title[i]], obj_width);
			...
			if(isSupport("amas")){
				if(isSupport("mtlancfg")){
					$.each(sdn_rl_for_clientlist, function(index, sdn_all_rl){
						...
						code += create_clientlist_card(sdn_all_rl.apg_rl.ssid, "sdn" + sdn_idx, obj_width);

					});
				}
				else{
					for(var i=1; i<isSupport("mssid_count")+1; i++){
						code += create_clientlist_card("<#Guest_Network#> - " + i, "gn" + i, obj_width);
					}
				}
			}
			break;
	...
	if(!top.isIE8)
		code += "<div style='text-align:center;margin:15px 0;'><input  type='button' class='button_gen' onclick='exportClientListLog();' value='<#btn_Export#>'></div>";
	...
	for(var i = 0; i < clientList.length; i += 1) {
		if(clientList[clientList[i]].isOnline) {
			...
			var tempArray = [clientList[clientList[i]].internetState, deviceTypeName, clientName, clientList[clientList[i]].ip, 
							...
							clientList[clientList[i]].ip6, "", clientList[clientList[i]].mlo];
			switch (clienlistViewMode) {
				case "All" :
...
function addEvents(objID){
	document.querySelector(`[data-bs-toggle="collapse"][href="#collapse_${objID}"],[data-bs-toggle="collapse"][data-bs-target="#${objID}"]`).addEventListener("click", function(e){
		e.stopPropagation();
		e.preventDefault();
		if ($(this).attr("aria-expanded") == "true") {
			$(".collapse[id='" + $(this).attr("aria-controls") + "']").slideUp();
			$(this).attr("aria-expanded", "false");
		} else {
			$(".collapse[id='" + $(this).attr("aria-controls") + "']").slideDown();
			$(this).attr("aria-expanded", "true");
		}

		if ($(this).find("i").hasClass("arrow_up")) {
			$(this).find("i").removeClass("arrow_up")
		...
		if (sortArray.length == 0) {
			...
			for (var j = 0; j < clientlist_sort.length; j += 1) {
				...
				if (!(isSwMode('mb') || isSwMode('ew'))) {
					...
					clientInterfaceCode += "<div class='interface_container'><div class='" + radioIcon_css + " radio_" + rssi_t + "'></div>";
					if (clientlist_sort[j].isWL != 0 || (isSupport("mtlancfg") && clientlist_sort[j].sdn_idx > 0)) {
						...
						clientInterfaceCode += `<div class='band_block'><span class='${bandClass}'>${band_text}</span></div>`;
					}
					clientInterfaceCode += "</div>";
					clientListCode += "<td class='client-list-sm' width='" + obj_width[0] + "' align='center'>" + clientInterfaceCode + "</td>";
				}
				...
				if (clientlist_sort[j].macRepeat > 1) {
					clientIconCode += '<div class="clientlist_circle"';
					clientIconCode += 'onmouseover="return overlib(\'' + clientlist_sort[j].macRepeat + ' clients are connecting to <% nvram_get("productid"); %> through this device.\');"';
					clientIconCode += 'onmouseout="nd();"><div>';
					clientIconCode += clientlist_sort[j].macRepeat;
					clientIconCode += '</div></div>';
				}
				...
				if (listView_userIconBase64 != "NoIcon") {
					...
					clientIconCode += "<div style='height:40px;width:40px;cursor:default;' class='clientIcon_no_hover' title='"+ clientlist_sort[j].deviceTypeName +"'><i class='" + icon_type + "'></i>";
				...
				clientListCode += "<td class='IE8HACK client-list-sm' width='" + obj_width[1] + "' align='center'>";
				...
				clientListCode += "<td class='client-list-lg' width='" + obj_width[2] + "'>" +
					"<div class='client_info'>" +
					"<div class='client_info_main'>" +
					"<div class='client_info_text'>" +
					"<div class='client-name'>" + "<div class='clientName' onclick='editClientName(this);'>" + clientNameEnCode + "</div>" +
					"<input type='text' value='" + clientNameEnCode + "' maxlength='32' style='width:95%;margin-left:0px;display:none;' onblur='saveClientName(this, " + clientlist_sort[j].type + ", \"" + clientlist_sort[j].mac + "\");'>" + "</div>" +
					"<div class='client-ip'>";
				clientListCode += (clientList[clientlist_sort[j].mac].isWebServer) ? "<a class='link' href='http://" + clientlist_sort[j].ip + "' target='_blank'>" + clientlist_sort[j].ip + "</a>" : `<div style="display: flex;flex-direction: column; align-items: flex-start;"><div>${clientlist_sort[j].ip}</div><div>${clientlist_sort[j].ip6}</div></div>`;
				...
				clientListCode += "<div class='client-mac'>" + clientlist_sort[j].mac + "</div>" +
					"</div>";
				clientListCode += "<div class='client_info_right'>" +
					clientInterfaceCode +
					clientIconCode +
					"</div>" +
					"</div>" +
					"<div class='client_info_bottom'>" +
					"<div class='client_info_detail_block'><div class='client_info_detail_title'>TX</div>" + txRate + " Mbps</div>" +
					"<div class='client_info_detail_block'><div class='client_info_detail_title'>RX</div>" + rxRate + " Mbps</div>" +
					"<div class='client_info_detail_block'><div class='client_info_detail_title'><#Access_Time#></div>" + accessTime + "</div>" +
					...
					"<div class='client_info_detail_block'><div class='client_info_detail_title'><#Access_Time#></div>" + accessTime + "</div>" +
					"</div>" +
					"<div class='client_info_more_close' onclick='closeClientInfoMore(this)'></div>" +
					"</div>" +
					"</td>";

				//Width >= 960
				clientListCode += "<td class='client-list-xl text-left' style='padding:0px 16px; word-wrap:break-word; word-break:break-all;' width='" + obj_width[2] + "'>";
				clientListCode += "<div class='viewclientlist_clientName_edit clientName' onclick='editClientName(this);'>" + clientNameEnCode + "</div>";
				clientListCode += "<input type='text' value='" + clientNameEnCode + "' class='input_25_table' maxlength='32' style='width:95%;margin-left:0px;display:none;' onblur='saveClientName(this, " + clientlist_sort[j].type + ", \"" + clientlist_sort[j].mac + "\");'>";
				clientListCode += "</td>";
				var ipStyle = ('<% nvram_get("sw_mode"); %>' == "1") ? "line-height:16px;text-align:left;padding-left:10px;" : "line-height:16px;text-align:center;";
				clientListCode += "<td class='client-list-xl text-left' width='" + obj_width[3] + "' style='" + ipStyle + "'><div style='display: flex;flex-direction: row; align-items: center; justify-content: space-between; gap:5px;'>";
				clientListCode += (clientList[clientlist_sort[j].mac].isWebServer) ? "<a class='link' href='http://" + clientlist_sort[j].ip + "' target='_blank'>" + clientlist_sort[j].ip + "</a>" : `<div style="display: flex;flex-direction: column; align-items: flex-start;"><div>${clientlist_sort[j].ip}</div><div>${clientlist_sort[j].ip6}</div></div>`;
				if ('<% nvram_get("sw_mode"); %>' == "1") {
					clientListCode += '<span style="float:right;margin-top:-3px;margin-right:5px;" class="ipMethodTag" onmouseover="return overlib(\''
					clientListCode += ipState[clientList[clientlist_sort[j].mac].ipMethod];
				...
				clientListCode += "<td class='client-list-xl' width='" + obj_width[4] + "'>" + clientlist_sort[j].mac + "</td>";

				if (isSupport("stainfo") && !(isSwMode('mb') || isSwMode('ew'))) {
					clientListCode += "<td class='client-list-sm' width='" + obj_width[5] + "'>" + txRate + "</td>";
					clientListCode += "<td class='client-list-sm' width='" + obj_width[6] + "'>" + rxRate + "</td>";
					clientListCode += "<td class='client-list-sm' width='" + obj_width[7] + "'>" + accessTime + "</td>";
				}
				clientListCode += "</tr>";
			}
		}
	...
	$.ajax({
		...
		success: function(response){
			if(document.getElementById("clientlist_viewlist_content").style.display != "none") {
				...
				updateClientListView_timer = setTimeout("updateClientListView();", 3000000);
			}
			else
				clearTimeout(updateClientListView_timer);
		}
				...
				if(category.list.length > 0){
					...
					$.each(category.list, function( index, value ){
						...
						if(_paramObj.source == "cloud" && src != ""){
							$item_obj.find('i').attr('style','--svg:url(' + src + ')');
						}
