...
if(based_modelid == 'GT-BE98_PRO' && smart_connect_mode[2] === '1'){
	let _value = parseInt(smart_connect_mode, 2) - 4;
	smart_connect_mode = parseInt(_value).toString(2);
}
...
function initial(){
	...
	if(smart_connect_mode[0] == '1'){
		document.querySelector('#smart_connect_check_4').checked = true
	}
...
function applyRule(){
	...
	postObj = {
		...
		'psc6g': (document.getElementById('band2_psc6g_checkbox').checked || document.getElementById('band3_psc6g_checkbox').checked) ? '1' : '0',
	...
	variable = {}
		if(_smart_connect_enable == 1 && smart_connect_mode[0] == '1'){		// 6 GHz-2-2
		...
		if(_smart_connect_enable == 1 && smart_connect_mode[1] == '1'){		// 6 GHz-1
		...
		if(_smart_connect_enable == 1 && smart_connect_mode[3] == '1'){		// 5 GHz-1
		...
		if(_smart_connect_enable == 1 && smart_connect_mode[4] == '1'){		// 2.4 GHz
...
function enableSmartCon(val){
	...
	if(val == 0){
		...
		$("#enable_160_field").hide();
		if ((wl_unit == '0' && val == '2') || based_modelid == "RT-AC3200" || ((based_modelid == "RT-AX92U" || based_modelid == "RT-AX95Q" || based_modelid == "XT8PRO" || based_modelid == "BT12" || based_modelid == "BQ16" || based_modelid == "BM68" || based_modelid == "XT8_V2") && country != "EU" && wl_unit == "1")) {
			$("#dfs_checkbox").hide();
		}
		else {
			$("#dfs_checkbox").show();
		...
		if(val == '1'){
			...
			if (band0_channel == '0') {
				...
				$('#band0_autoChannel').html('Current Control Channel: ' + cur_control_channel[3]);
			}
	...
	if(unit == '0'){
		...
		if(band6g_support){		// due to GT-AXE11000/GT-AXE16000 does not support
			if(document.getElementById('band2_psc6g_checkbox').checked){
				...
				document.getElementById('band3_psc6g_checkbox').checked = true;
			}
			...
			for(var i=channel_5g_2.length-1; i>=0; i--){
				...
				if(!is_EU_sku && _channel < 30){ // remove 1, 5, 9, 13, 17, 21, 25, 29,
					if(band6gBW160_limit) channel_6g.splice(i, 1); 
				}
			}
		...
		if (curBandwidth == '0') {
			$('#band3_extChannel_field').show();
			var _wl_channel = new Array();
				loop_auto: for (var i = 0; i < channel_6g.length; i++) {
			...
			if(amesh_support && httpApi.hasAiMeshNode()){
				channel_6g = [];
				for(j=1; j<mesh_6g.chan_320m.chanspec.length; j++){
					channel_6g.push(mesh_6g.chan_320m.chanlist[j]);
					channel_6g_val.push(mesh_6g.chan_320m.chanspec[j]);
				}
			}
			else{
...
function controlHideSSIDHint() {
	...
	if(dwb_info.mode && (dwb_info.band == wl_unit) && document.form.smart_connect_x.value == "1" && (based_modelid == "RT-AX92U" || based_modelid == "RT-AX95Q" || based_modelid == "XT8PRO" || based_modelid == "BT12" || based_modelid == "BQ16" || based_modelid == "BM68" || based_modelid == "XT8_V2"))
...
function ajax_wl_channel(){
	$.ajax({
		...
		success: function(response){			
			...
			$('#band1_autoChannel').html('Current Control Channel: ' + cur_control_channel[0]);
			$('#band2_autoChannel').html('Current Control Channel: ' + cur_control_channel[1]);
			$('#band3_autoChannel').html('Current Control Channel: ' + cur_control_channel[2]);
...
function auth_method_change(unit, value, flag){
	...
	if(unit == '0'){
		...
		if(value == 'owe'){
			...
			if(smart_connect_mode[1] == '1' && flag != 'init'){
...
function handle_channel(unit, channel){
	if(unit == '0'){
		...
		if(_ch == '0' && cur_control_channel == '0'){
			$('#band0_autoChannel').show();
			$('#band0_autoChannel').html('Current Control Channel: ' + current_control_channel[3]);
		}
		else{
		...
		if(_ch == '0' && cur_control_channel == '0'){
			$('#band1_autoChannel').show();
			$('#band1_autoChannel').html('Current Control Channel: ' + current_control_channel[0]);
		}
		else{
		...
		if(_ch == '0' && cur_control_channel == '0'){
			$('#band2_autoChannel').show();
			$('#band2_autoChannel').html('Current Control Channel: ' + current_control_channel[1]);
		}
		else{
		...
		if(_ch == '0' && cur_control_channel == '0'){
			...
			$('#band3_autoChannel').html('Current Control Channel: ' + current_control_channel[2]);
		}
		else{
...
function channel_6g(bw){
	...
	for(var i=wl_channel_list_6g.length-1; i>=0; i--){
		...
		if(!is_EU_sku && _channel < 30){ // remove 1, 5, 9, 13, 17, 21, 25, 29,
			if(productid != 'GT-AXE16000' && productid != 'GT-BE98' && productid != 'GT-BE98_PRO') wl_channel_list_6g.splice(i, 1); 
		}
	}
	...
	if(bw == '0'){
		...
		loop_auto: for(i=0; i<wl_channel_list_6g.length; i++){
			...
			if(nmode_x != 1){
				for(var j=0;j<wl3.channel_160m.length;j++){
			...
			if(nmode_x != 1){
				for(var j=0;j<wl3.channel_80m.length;j++){
			...
			for(var j=0;j<wl3.channel_40m.length;j++){
			...
			for(var j=0;j<wl3.channel_20m.length;j++){
...
function handle_smart_connect(value, flag){
	if(value == '0'){
		...
		if(smart_connect_mode[0] == '1'){		// 6 GHz-2
		...
		if(smart_connect_mode[1] == '1'){		// 6 GHz-1
			$('#band6_hide_ssid_field').hide();
			$('#band2_ssid_field').hide();
			auth_method_change('01', document.form.band2_auth_mode_x.value);
		}
		else{
			$('#band6_hide_ssid_field').show();
			$('#band2_ssid_field').show();
		...
		if(smart_connect_mode[3] == '1'){		// 5 GHz-1
		...
		if(smart_connect_mode[4] == '1'){		// 2.4 GHz
...
function psc6gSyncHandle(checked, unit){
	document.getElementById('band2_psc6g_checkbox').checked = checked;
	document.getElementById('band3_psc6g_checkbox').checked = checked;
	if(unit === '2'){
		setTimeout(function(){
			separateGenChannel('3', document.form.band3_channel.value, document.form.band3_bw.value);
		}, 100);
	}
	else{
		setTimeout(function(){
			separateGenChannel('2', document.form.band2_channel.value, document.form.band2_bw.value);
		}, 100);
	}
}
</script>
			...
			<table id="band_separate" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
				...
				<tr id="smart_connect_band_field">
					...
					<td>
						...
						<input id="smart_connect_check_1" type="checkbox" onchange="updateSmartConnect('1', this.checked)">5 GHz
						<!-- <input id="smart_connect_check_2" type="checkbox" onchange="updateSmartConnect('2', this.checked)">5 GHz-2 -->
						<input id="smart_connect_check_3" type="checkbox" onchange="updateSmartConnect('3', this.checked)">6 GHz-1
						<input id="smart_connect_check_4" type="checkbox" onchange="updateSmartConnect('4', this.checked)">6 GHz-2
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						...
						<span id="band2_psc6g" style="">
							<input id="band2_psc6g_checkbox" type="checkbox" onclick="psc6gSyncHandle(this.checked, '2');separateGenChannel('2', document.form.band2_channel.value, document.form.band2_bw.value);" <% nvram_match("psc6g", "1" , "checked" ); %>><#Enable_PSC_Hint#><br>
						</span>
				...
				<tr id="band2_auth_method_field">
					...
					<td>
						<select name="band2_auth_mode_x" class="input_option select_auth_mode" onChange="auth_method_change('2', this.value);">
							<option value="owe" <% nvram_match("wl1_auth_mode_x", "owe",   "selected"); %>><#Wireless_Encryption_OWE#></option>
				...
				<tr id="band3_channel_field" style="">
					...
					<td>
						...
						<span id="band3_psc6g" style="">
							<input id="band3_psc6g_checkbox" type="checkbox" onclick="psc6gSyncHandle(this.checked, '3');separateGenChannel('3', document.form.band3_channel.value, document.form.band3_bw.value);" <% nvram_match("psc6g", "1" , "checked" ); %>><#Enable_PSC_Hint#><br>
						</span>
