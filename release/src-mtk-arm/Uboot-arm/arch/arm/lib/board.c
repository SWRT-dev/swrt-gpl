...
# endif /* CONFIG_SYS_FLASH_CHECKSUM */
	...
	if (strcmp("yes", getenv("invaild_env")) == 0)
	{
	     char ethaddr[32] = {0};
            ...
            char env_region[32] = {0};

            strncpy(ethaddr, getenv("ethaddr"), 32);
            strncpy(fenv_model, getenv("fenv_model"), 32);
            strncpy(env_region, getenv("fenv_region"), 32);
            set_default_env("### invaild_env is yes ###\n");

            /* resync tw factory params */
            setenv("ethaddr", ethaddr);
            setenv("fenv_model", fenv_model);
            setenv("fenv_region", env_region);
            saveenv();
	}
