...
int mmd_ioctl(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
    u32 phyad = 0;
    u32 devad = 0;
    u32 regad = 0;
    ...
    if (!memcmp(argv[0], "mmd.r", sizeof("mmd.r")))
    {
        phyad = simple_strtoul(argv[1], NULL, 0);
        devad = simple_strtoul(argv[2], NULL, 0);
        regad = simple_strtoul(argv[3], NULL, 0);
        val = mt7531_mmd_read(phyad, devad, regad);
        printf("Read PHYAD 0x%X DEVAD 0x%04X REG 0x%04X, VAL is 0x%04X\n", phyad, devad, regad, val);
    }
    else if (!memcmp(argv[0], "mmd.w", sizeof("mmd.w")))
    {
        phyad = simple_strtoul(argv[1], NULL, 0);
        devad = simple_strtoul(argv[2], NULL, 0);
        regad = simple_strtoul(argv[3], NULL, 0);
        val = simple_strtoul(argv[4], NULL, 0);
        mt7531_mmd_write(phyad, devad, regad, val);
    }
    return 0;
}
...
U_BOOT_CMD(
    mmd, 4, 1, mmd_ioctl,
    "mmd   - Mediatek MMD PHY register R/W command !!\n",
    "mmd.r [phy_addr(hex)] [devad(hex)] [regad(hex)]- read mmd phy reg \n"
    "mmd.w [phy_addr(hex)] [devad(hex)] [regad(hex)] [value(hex)]- write mmd phy reg\n"
    "");
u32 mt7531_reg_read(u32 reg);
void mt7531_reg_write(u32 reg, u32 val);
int swreg_ioctl(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
    u32 regad = 0;
    u32 val = 0;

    if (!memcmp(argv[0], "swreg.r", sizeof("swreg.r")))
    {
        regad = simple_strtoul(argv[1], NULL, 0);
        val = mt7531_reg_read(regad);
        printf("Read SWREG 0x%04X VAL is 0x%04X\n", regad, val);
    }
    else if (!memcmp(argv[0], "swreg.w", sizeof("swreg.w")))
    {
        regad = simple_strtoul(argv[1], NULL, 0);
        val = simple_strtoul(argv[2], NULL, 0);
        mt7531_reg_write(regad, val);
    }
    return 0;
}

U_BOOT_CMD(
    swreg, 4, 1, swreg_ioctl,
    "swreg   - Mediatek switch register R/W command !!\n",
    "swreg.r [regad(hex)]- read switch reg \n"
    "swreg.w [regad(hex)] [value(hex)]- write switch reg\n"
    "");
