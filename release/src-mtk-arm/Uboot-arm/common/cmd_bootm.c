		...
		int boot_progress)
{
	...
	{
		int skip_load_os = 1;
		char *s , *tag_flag , *check_result;
		s = getenv("skip_load_os");
		tag_flag = getenv("have_tag");
		check_result = getenv("checkimg_result");
		skip_load_os = (int)simple_strtol(s, NULL, 10);
		...
		if(skip_load_os)
		{
			...
			if(ret)
				setenv("check_os_error", "1");
			else
				setenv("check_os_error", "0");
			return ret;
		}
		if(ret)
		{
			printf("\n\nNow restore the kernel image!... Please keep the power on! ...\n\n");
			run_command("run boot_restore_img",0);
		}
	}
#endif
	/* Load the OS */
		...
		ulong *os_len)
{
	...
	if (argc < 1) {
		...
		s = getenv("have_tag");
		if(s && !strcmp(s,"yes"))
			img_addr = load_addr + CONFIG_MAX_NAND_BLOCK_SIZE;
		else
			img_addr = load_addr;
